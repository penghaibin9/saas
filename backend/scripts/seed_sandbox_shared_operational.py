...
def main():
 ...
 from app.services.sandbox_school_shared_operational_seed import seed_shared_operational_coverage
 db=get_sessionmaker()()
 try:
  t=db.get(Tenant,SANDBOX_TID)
  if not t or t.tenant_code!=SANDBOX_CODE:raise RuntimeError('target tenant identity check failed')
  print(json.dumps(seed_shared_operational_coverage(db,SANDBOX_TID),ensure_ascii=False,indent=2));return 0
 finally:db.close()

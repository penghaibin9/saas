...
同一哨兵 Authority 由原 Sandbox 20K gate 与 Control Plane B8-02 topology gate 复用；
两条门禁都只能 prime 一次、reset 后只读 verify，禁止各自实现第二套旁租户保护。
"""
...
ALLOWED_WORKFLOW_NAMES = frozenset({
    "Sandbox 20K Real-School Data Gate",
    "Control Plane B8 20K IAM Topology",
})
...
def _expected_rows() -> dict[str, tuple[str, str, str, str, str]]:
    return {
        f"D-SENT-{index:04d}": (f"旁租户哨兵{index:02d}", "2025", "ENROLLED", "NORMAL", "ACTIVE")
...
def _assert_gate_only() -> None:
    ...
    if workflow not in ALLOWED_WORKFLOW_NAMES:
        raise RuntimeError(f"仅允许 20K Authority gates 调用旁租户哨兵脚本 workflow={workflow!r}")


def _load_actual(db, StudentProfile) -> dict[str, tuple[str, str, str, str, str]]:
    rows = db.execute(select(
        StudentProfile.student_no, StudentProfile.real_name, StudentProfile.grade,
        StudentProfile.current_stage, StudentProfile.student_status, StudentProfile.status,
    ).where(
        StudentProfile.tenant_id == DEMO_TID,
        StudentProfile.is_deleted.is_(False),
    ).order_by(StudentProfile.student_no)).all()
    return {
        str(student_no): (str(real_name), str(grade), str(current_stage), str(student_status), str(status))
...
def _resolve_tenant(db, Tenant, *, create: bool):
    ...
    if by_id is not None and by_id.tenant_code != DEMO_CODE:
        raise RuntimeError(f"固定旁租户ID已被占用 id={DEMO_TID} code={by_id.tenant_code!r}")
    if by_code is not None and int(by_code.id) != DEMO_TID:
        raise RuntimeError(f"demo-school code 已绑定其他租户 id={by_code.id} expected={DEMO_TID}")
...
def verify(db, StudentProfile, Tenant) -> None:
    ...
    if actual != expected:
        ...
        changed = [student_no for student_no in sorted(set(expected) & set(actual)) if expected[student_no] != actual[student_no]][:10]
        raise RuntimeError(
            "demo-school 旁租户哨兵被改变："
            f"expected={SENTINEL_COUNT} actual={len(actual)} missing={missing} extra={extra} changed={changed}"
        )
...
def main() -> int:
    ...
    try:
        if args.prime:
            ...
            verify(db, StudentProfile, Tenant)
            action = "VERIFY"
        print(f"[20k-neighbor-sentinel] PASS action={action} tenant={DEMO_CODE} tenantId={DEMO_TID} students={SENTINEL_COUNT}")
        return 0
    except RuntimeError as exc:
        db.rollback()
        print(f"[20k-neighbor-sentinel] FAIL {exc}")

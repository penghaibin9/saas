"""Bootstrap the minimal AA-011 Selection identities through canonical School IAM.

The helper intentionally uses the production teacher/student XLSX import authority.  It creates
only the identities needed by the existing Academic B browser contract:
- e2e_academic_admin / ACADEMIC_ADMIN
- E2E20260001 / STUDENT
- E2E20260002 / STUDENT

Organization creation is still performed through the real school-admin API; no role/permission
row is inserted directly and no authorization rule is weakened for E2E.
"""
...
CRED_PATH = OUT_DIR / "e2e_academic_aa011_credentials.local.json"

TEACHERS = [
    ["e2e_academic_admin", "E2E AA-011教务管理员", "教务处", "选课管理", "ACADEMIC_ADMIN", "", ""],
]
STUDENTS = [
    ["E2E20260001", "E2E AA-011学生A", COLLEGE_A, MAJOR_A, CLASS_A1, GRADE, "男", ""],
    ["E2E20260002", "E2E AA-011学生B", COLLEGE_A, MAJOR_A, CLASS_A1, GRADE, "女", ""],
]
...
def _admin_login() -> str:
    result = _req(
        "POST",
        "/auth/login",
        headers={"X-Forwarded-For": "10.253.0.10"},
        body={"loginName": ADMIN[0], "password": ADMIN[1], "tenantCode": TENANT},
    )
    if result.get("code") != 0:
        raise SystemExit("AA-011 admin login failed: " + json.dumps(result, ensure_ascii=False))
...
def _canonical_import(token: str, *, kind: str, content: bytes) -> dict:
    filename = f"e2e_academic_aa011_{kind}.xlsx"
    ...
    created = _req(
        ...
        headers={
            ...
            "Idempotency-Key": f"academic-aa011-{kind}-upload-v1",
        },
    )
    if created.get("code") != 0:
        raise SystemExit(f"AA-011 canonical {kind} upload failed: {json.dumps(created, ensure_ascii=False)}")
    item = dict(created.get("data") or {})
    job_id = str(item.get("id") or item.get("jobId") or "").strip()
    if not job_id:
        raise SystemExit(f"AA-011 canonical {kind} upload returned no job id")
    status = str(item.get("status") or "").upper()
    if status not in {"SUCCEEDED", "VALIDATED"}:
        processed = _req("POST", f"/data-exchange/imports/{job_id}/process", token=token)
        if processed.get("code") != 0:
            raise SystemExit(f"AA-011 canonical {kind} process failed: {json.dumps(processed, ensure_ascii=False)}")
    ...
    if status != "VALIDATED":
        raise SystemExit(
            f"AA-011 canonical {kind} did not validate: "
            + json.dumps({
                "jobId": job_id,
    ...
    confirmed = _req(
        ...
        headers={"Idempotency-Key": f"academic-aa011-{kind}-confirm-v1"},
    )
    if confirmed.get("code") != 0:
        raise SystemExit(f"AA-011 canonical {kind} confirm failed: {json.dumps(confirmed, ensure_ascii=False)}")
...
def _normalize_password(token: str, login_name: str, index: int) -> dict:
    ...
    first = _req(
        "POST",
        "/auth/login",
        headers={"X-Forwarded-For": f"10.253.1.{20 + index}"},
    ...
    verified = _req(
        "POST",
        "/auth/login",
        headers={"X-Forwarded-For": f"10.253.2.{20 + index}"},
...
def main() -> int:
    ...
    print("AA-011 org:", json.dumps(org, ensure_ascii=False))
    ...
    print("AA-011 account verification:", json.dumps(results, ensure_ascii=False, indent=2))

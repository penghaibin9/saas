"""写入演示种子数据（幂等：已有该租户学生则跳过）。
规模：1 租户 + 1 品牌 + 8 角色 + 20 用户 + 100 学生 + 30 待办 + 20 审批 + 20 消息 + 50 审计。"""
...
from app.models import (College, Major, Role, SchoolClass, SecurityAuditLog, StudentContact,
                        StudentProfile, StudentStageEvent, Tenant, TenantBrandConfig,
...
if db.scalars(select(StudentProfile).where(StudentProfile.tenant_id == TID)).first():
    print("[seed] demo data already present, skip (先跑 reset_dev_db.py 可重灌)")
    raise SystemExit(0)
...
for i in range(20):
    ...
    db.add(User(tenant_id=TID, login_name=f"user{i+1:02d}", real_name=SURNAMES[i] + GIVEN[i],
                password_hash="mock-not-a-real-hash", user_type="TEACHER" if role_code != "STUDENT" else "STUDENT",
                status="ACTIVE"))
...
for i in range(50):
    db.add(SecurityAuditLog(tenant_id=TID, operator_name=SURNAMES[i % 20] + "老师",
                            ...
                            trace_id=f"seed-{i:04d}",
                            detail_json={"seq": i},
                            created_at=now - timedelta(hours=rng.randint(1, 240))))

# ── demo-school 演示租户（账号密码登录 · 行级隔离验收）──
...
# 密码为 pbkdf2_sha256 预计算哈希：仓库与数据库均不存明文
DEMO_ACCOUNTS = [
    ("admin_demo", "陈演示", "SCHOOL_ADMIN", "pbkdf2_sha256$200000$e3328d353dcd6d6520ec9b30d015d5b6$d25a69f77bb417069675912c3976134c5be31dddb3d53697f632f5808cc2f1aa"),
    ("teacher_demo", "李导师", "TEACHER", "pbkdf2_sha256$200000$5a98601f0dfbdbcc01f4e2214972c72d$d4f8e6e2c16a533071705cbd9e8e0cfd379d5145bce6310899d56ea5edf9a015"),
    ("counselor_demo", "王辅导", "TEACHER", "pbkdf2_sha256$200000$888b4cd504e4020f52945b113906f938$c67b03274c490ba9c4730ba3c361f7a3e2244b623d38ff09f34e2a646a13b32c"),
    ("student_demo", "张同学", "STUDENT", "pbkdf2_sha256$200000$278669d15b64d435317fa81fa201714e$99ef4fd8f0ff515a2bc2a505de8b5b974d4b1864d622d8f64478dd33ad468757"),
]
...
# demo-school 少量数据（证明隔离：demo 账号只见这 5 人，不见主租户 100 人）
for i in range(5):
...
db.add(UnifiedMessage(tenant_id=TID2, receiver_id=1, title="欢迎使用演示租户", message_type="SYSTEM",
                      status="UNREAD"))

db.commit()
print("[seed] demo-school tenant + 4 hashed accounts + 5 students")
print(f"[seed] tenant=1 brand=1 roles={len(ROLES)} users=20 students={len(student_ids)} "
      f"approvals=20 todos=30 messages=20 audits=50")

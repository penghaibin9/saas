"""Bootstrap the single college-level student-affairs account needed by Playwright CI.

Test infrastructure only. Uses the same official identity import, user lookup,
reset-password and change-password APIs as production administration. It does not
write application tables directly, relax throttling or persist credentials.
"""
...
LOGIN_NAME = "e2e_college_admin"
DISPLAY_NAME = "E2E学院管理员"
STABLE_PASSWORD = "E2eTest@2026"


def build_college_teacher_xlsx() -> bytes:
...
def import_college_account(token: str) -> None:
    receipt = _canonical_import(
        token,
        kind="teachers",
        content=build_college_teacher_xlsx(),
        idempotency_namespace="e2e-affairs-college",
    )
    print(
        "[e2e-affairs-college] canonical identity import confirmed",
...
def normalize_password(admin_token: str) -> None:
    ...
    if reset.get("code") != 0:
        raise SystemExit("college reset-password failed: " + json.dumps(reset, ensure_ascii=False))
    temp_password = (reset.get("data") or {}).get("tempPassword")
    if not temp_password:
        raise SystemExit("college reset-password returned no temporary password")
    ...
    if logged.get("code") != 0:
        raise SystemExit("college temporary login failed: " + json.dumps(logged, ensure_ascii=False))
    ...
    if changed.get("code") != 0:
        raise SystemExit("college password normalization failed: " + json.dumps(changed, ensure_ascii=False))
    print("[e2e-affairs-college] password normalized without credential output")


def main() -> int:
    admin_token = login()
    ensure_org(admin_token)
    import_college_account(admin_token)
    normalize_password(admin_token)
    print(f"[e2e-affairs-college] ready: {TENANT}:{LOGIN_NAME}")
    return 0

...
def progress_blocked(sid: str, body: BlockedBody, user=Depends(require_staff)):
...
def dorm_update(sid: str, body: DormBody, user=Depends(require_staff)):
...
def dorm_confirm(body: IdsBody, user=Depends(require_staff)):
    return success(svc.batch_confirm_checkin(body.ids), message="已确认")


@router.post("/dorms/{sid}/exception", summary="标记入住异常（说明≥5字）")
def dorm_exception(sid: str, body: NoteBody, user=Depends(require_staff)):
    return success(svc.mark_dorm_exception(sid, body.note), message="已标记")


@router.post("/exceptions", summary="登记迎新异常（IDENTITY/PAYMENT/MATERIAL/DORM/NO_SHOW，说明≥5字）")
def exception_create(body: ExceptionCreate, user=Depends(require_staff)):
    return success(svc.create_exception(body.studentId, body.exceptionType, body.description,
                                        body.riskLevel), message="已登记")
...
def batch_detail(bid: str, user=Depends(require_staff)):
    return success(svc.get_batch(bid))


@router.post("/batches", summary="新建批次")
def batch_create(body: BatchCreate, user=Depends(require_staff)):
    return success(svc.create_batch(body.model_dump()), message="已新建")


@router.put("/batches/{bid}", summary="编辑批次")
def batch_update(bid: str, body: BatchUpdate, user=Depends(require_staff)):
...
def batch_void(bid: str, body: ReasonBody, user=Depends(require_staff)):
...
def checkin_point_create(body: PointCreate, user=Depends(require_staff)):
    return success(svc.create_checkin_point(body.model_dump()), message="已新增")


@router.put("/checkin-points/{pid}", summary="编辑报到点")
def checkin_point_update(pid: str, body: PointUpdate, user=Depends(require_staff)):
...
def checkin_point_toggle(pid: str, user=Depends(require_staff)):
    return success(svc.toggle_checkin_point(pid), message="已切换")


@router.post("/checkin-points/{pid}/delete", summary="删除报到点")
def checkin_point_delete(pid: str, user=Depends(require_staff)):
...
def flow_config(user=Depends(require_staff)):
...
def notice_create(body: NoticeCreate, user=Depends(require_staff)):
...
def notice_send(nid: str, user=Depends(require_staff)):
...
def archive_create(body: ArchiveCreate, user=Depends(require_staff)):

...
def aid_editable(apply_id: int = Path(...), user=Depends(get_current_user)):
    ...
    with session() as db:
        ...
        result = aid._apply_row(row, student, family, reveal=True)
...
def aid_update_returned(
    ...
    with session() as db:
        ...
        family = aid._family_of(db, row.id)
        if family:
            if "memberCount" in body: family.member_count = body.get("memberCount")
            if "annualIncome" in body: family.income_encrypted = aid.encrypt_field(body.get("annualIncome"))
            if "debt" in body: family.debt_encrypted = aid.encrypt_field(body.get("debt"))
            if "familyMembers" in body: family.family_members_json = json.dumps(body.get("familyMembers") or [], ensure_ascii=False)
            if "specialTags" in body: family.special_flags_json = json.dumps(body.get("specialTags") or [], ensure_ascii=False)
            family.version += 1
        row.version += 1
...
def aid_resubmit(
    ...
    with session() as db:
        ...
        if not workflow:
            ...
            db.add(WorkflowTask(tenant_id=_tid(), instance_id=workflow.id, node_code=first, assignee_id=assignee, status="PENDING"))
        row.status, row.return_reason, row.version = first, None, row.version + 1
        aid._todo_upsert(db, row.id, assignee, row.student_id, f"困难认定重新提交待评议：{student.real_name}")
        aid._audit(db, row.id, "STUDENT_RESUBMIT")
        db.commit(); aid._drain_message_outbox(); db.refresh(row)
...
def funding_resubmit(
    ...
    with session() as db:
        ...
        atomic_claim_version(db, row, body.get("version"))
        snapshot = funding._check_grant(db, row.student_id) if row.project_type == "GRANT" else funding._check_scholarship(db, row.student_id)
        if not snapshot["ok"]:
        ...
        workflow = db.get(WorkflowInstance, int(row.workflow_instance_id)) if row.workflow_instance_id else None
        if not workflow:
            workflow = funding._open_wf(db, row.id, row.project_type, row.student_id, f"{student.real_name} {row.project_type}", first, assignee)
            row.workflow_instance_id = workflow.id
        else:
            workflow.status, workflow.current_node = "RUNNING", first
            db.add(WorkflowTask(tenant_id=_tid(), instance_id=workflow.id, node_code=first, assignee_id=assignee, status="PENDING"))
        ...
        db.commit(); funding._drain_message_outbox(); db.refresh(row)

...
def create_student(body: StudentCreateRequest, user=Depends(get_current_user)):
...
def update_student(student_id: str, body: StudentUpdateRequest, user=Depends(get_current_user)):
...
def void_student(student_id: str, body: StudentVoidRequest, user=Depends(get_current_user)):
...
def student_timeline(student_id: str, user=Depends(get_current_user)):
    return success({"items": svc.get_timeline(student_id)})


@router.get("/{student_id}/risk-summary", summary="学生风险摘要（学业/实习/就业三维信号）")
def student_risk(student_id: str, user=Depends(get_current_user)):

...
def makeup_create(body: MakeupCreate, user=Depends(get_current_user)):
    return success(svc.create_makeup(body.model_dump()), message="已新增")


@router.put("/makeups/{mid}/status", summary="更新补考状态")
def makeup_status(mid: str, body: StatusBody, user=Depends(get_current_user)):
...
def retake_status(rid: str, body: StatusBody, user=Depends(get_current_user)):
...
def warning_detail(wid: str, user=Depends(get_current_user)):
    return success(svc.get_warning_detail(wid))


@router.post("/warnings", summary="新增预警（原因≥5字）")
def warning_create(body: WarningCreate, user=Depends(get_current_user)):
    return success(svc.create_warning(body.model_dump()), message="已创建")


@router.put("/warnings/{wid}/level", summary="调整预警等级（原因≥5字）")
def warning_level(wid: str, body: LevelBody, user=Depends(get_current_user)):
...
def warning_void(wid: str, body: ReasonBody, user=Depends(get_current_user)):
    return success(svc.void_warning(wid, body.reason), message="已作废")


@router.post("/warnings/assign", summary="分配预警跟进人")
def warning_assign(body: AssignBody, user=Depends(get_current_user)):

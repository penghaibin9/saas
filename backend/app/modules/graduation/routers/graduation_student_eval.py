...
def gd_student_evals(page: int = Query(1, ge=1), pageSize: int = Query(20, ge=1, le=200),
    ...
    items, total = svc.list_evals(
        page, pageSize, gd_student_id=gdStudentId, batch_id=require_batch_id(batchId),
    )
    return success(paginate(items, total, page, pageSize))


@router.post("/gd-student-evals/{gd_student_id}", summary="创建/提交导师对学生评价")
def gd_student_eval_create(gd_student_id: str, body: StudentEvalCreate,
                           batchId: int = Query(..., ge=1), user=Depends(get_current_user)):
    result = svc.create_eval(gd_student_id, body.model_dump(), batch_id=require_batch_id(batchId))
...
def gd_student_eval_submit(eval_id: str, batchId: int = Query(..., ge=1),
                           user=Depends(get_current_user)):
    result = svc.submit_eval(eval_id, batch_id=require_batch_id(batchId))

...
def create_intention(body: IntentionCreate, user=Depends(get_current_user)):
...
def intention_template(user=Depends(get_current_user)):
...
async def intention_import_xlsx(file: UploadFile = File(...), user=Depends(get_current_user)):
...
def intention_dry_run(body: IntentionImport, user=Depends(get_current_user)):
    return success(svc.intention_import_dry_run(body.rows))


@router.post("/intentions/import/confirm", summary="意向导入·确认")
def intention_confirm(body: IntentionImport, user=Depends(get_current_user)):
...
def intention_errors_xlsx(body: IntentionImportErrors, user=Depends(get_current_user)):
...
def update_intention(intention_id: str, body: IntentionUpdate, user=Depends(get_current_user)):
...
def submit_intention(intention_id: str, user=Depends(get_current_user)):
...
def withdraw_intention(intention_id: str, user=Depends(get_current_user)):
...
def run_major(user=Depends(get_current_user)):
...
def run_enterprise(user=Depends(get_current_user)):
...
def manual(body: ManualMatchBody, user=Depends(get_current_user)):
...
def batch(body: BatchMatchBody, user=Depends(get_current_user)):
...
def confirm(match_id: str, user=Depends(get_current_user)):
...
def reject(match_id: str, body: MatchActionBody | None = None, user=Depends(get_current_user)):

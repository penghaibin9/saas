...
def plan_get(batch_id: int, user=Depends(get_current_user)):
    return success(plan_svc.get_plan_by_batch(batch_id, user=user))


@router.put("/plans/batch/{batch_id}", summary="保存/更新实习计划书（草稿；任务清单校验）")
def plan_save(batch_id: int, body: dict = Body(...), user=Depends(get_current_user)):
...
def plan_publish(batch_id: int, user=Depends(get_current_user)):
...
def plan_task_summary(batch_id: int, user=Depends(get_current_user)):
...
def plan_task_review(prog_id: int, body: dict = Body(...), user=Depends(get_current_user)):
    b = body or {}

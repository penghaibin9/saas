...
def insurance_list(page: int = Query(1, ge=1), pageSize: int = Query(20, ge=1, le=200),
    ...
    items, total = svc.list_insurances(
...
def insurance_verify(insurance_id: int, body: dict = Body(...),
                     user=Depends(require_permission(_P_VERIFY))):
    payload = body or {}
    return success(svc.verify_insurance(
        insurance_id,
        payload.get("action", ""),
        payload.get("comment", ""),

...
课程均须按当前版本通过且完成承诺。替换后重新计算 blockers/warnings/completeness。
"""
...
def _recompute(result: dict) -> dict:
    ...
    applicable = [item for item in items if item.get("applicable") and item.get("required")]
    done = [item for item in applicable if item.get("status") in ("VALID", "EXEMPTED")]
...
def evaluate_internship_compliance(internship_id, operation="ONBOARD", user=None, db=None):
    result = base.evaluate_internship_compliance(
    ...
    for item in result.get("items") or []:
        ...
        item["evidenceId"] = str(safety["evidenceId"]) if safety.get("evidenceId") else None

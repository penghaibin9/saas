...
def grade_transcript(
    studentId: int = Path(...),
    page: Optional[int] = Query(None, ge=1, le=100000),
    pageSize: int = Query(50, ge=1, le=200),
    user=Depends(require_permission("academicAffairs.grade.view")),
):
    if page is None:
        return success(grade_svc.transcript(studentId, user))
    return success(grade_svc.transcript(studentId, user, page=page, page_size=pageSize))
...
@router.get("/grade-views/audit", summary="成绩操作审计（读侧；按校级、学院和任课关系裁定对象范围）")

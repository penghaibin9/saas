...
def _default_components(task) -> list[dict]:
    ...
    for code, name, weight in (
        ...
        if float(weight or 0) > 0:
            rows.append({"code": code, "name": name, "weight": float(weight), "required": True, "order": len(rows) + 1})
    return rows


def normalize_components(components) -> list[dict]:
    source = list(components or [])
    if len(source) < 2 or len(source) > 12:
        raise AppException("VALIDATION_ERROR", "动态成绩项须为2-12项")
    result = []
    codes = set()
...
def _scheme(db, task, *, create_default=False):
    ...
    if not row and create_default:
        ...
        row = AaGradeSchemeSnapshot(
            tenant_id=_tid(), grade_task_id=task.id, scheme_version=1,
            scheme_json=json.dumps(components, ensure_ascii=False, separators=(",", ":")),
            total_weight=100, status="DRAFT",
        )
        db.add(row)
        db.flush()
    return row
...
def configure_scheme(task_id, user, components) -> dict:
    ...
    with session() as db:
        ...
        if not row:
            row = AaGradeSchemeSnapshot(tenant_id=_tid(), grade_task_id=task.id, scheme_version=1)
            db.add(row)
        else:
        ...
        _grade._legacy._audit(
            db, "AA_GRADE_TASK", task.id, "GRADE_SCHEME_CONFIG",
            f"version={row.scheme_version};components={len(normalized)}",
        )
        db.commit()
        db.refresh(row)
        return {
            "gradeTaskId": str(task.id), "schemeId": str(row.id),
            "schemeVersion": int(row.scheme_version), "status": row.status,
            "components": normalized, "totalWeight": 100,
        }
...
def enter_component_scores(task_id, user, student_id, scores, exception_flag="NORMAL") -> dict:
    ...
    with session() as db:
        ...
        record = db.scalars(select(AaGradeRecord).where(
            AaGradeRecord.tenant_id == _tid(), AaGradeRecord.task_id == task.id,
            AaGradeRecord.student_id == int(student_id), AaGradeRecord.is_deleted.is_(False),
        )).first()
        if not record:
        ...
        if flag != "NORMAL":
            record.usual_score = record.midterm_score = record.final_score = record.total_score = None
            ...
            for component in components:
                if component["code"] not in submitted:
                    continue
                value = _score(submitted[component["code"]], component["name"])
                ...
                if not row:
                    row = AaGradeComponentScore(
                        ...
                        student_id=int(student_id), component_code=component["code"],
                    )
                    db.add(row)
                ...
                component_rows.append({**component, "score": value, "weightedScore": weighted})
        ...
        _grade._legacy._audit(
            db, "AA_GRADE_TASK", task.id, "DYNAMIC_GRADE_ENTER",
            f"student={student_id};schemeVersion={scheme.scheme_version};components={len(component_rows)};flag={flag}",
        )
        db.commit()
        return {
            "gradeTaskId": str(task.id), "studentId": str(student_id),
            "recordId": str(record.id), "schemeVersion": int(scheme.scheme_version or 1),
            "components": component_rows, "totalScore": total,
            "passStatus": record.pass_status, "exceptionFlag": record.exception_flag,
        }
...
def student_component_scores(task_id, user, student_id) -> dict:
    ...
    with session() as db:
        ...
        return {
            "gradeTaskId": str(task.id), "studentId": str(student_id),
            "items": [{
                "componentCode": row.component_code, "componentName": row.component_name,
                "weight": row.weight, "score": row.score, "weightedScore": row.weighted_score,

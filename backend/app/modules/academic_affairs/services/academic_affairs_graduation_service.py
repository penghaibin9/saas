"""13B-P6/Tier1 毕业资格审核（十一项跨域供数三态判定 + 学院初审→教务终审→归档）。

十项核心域 + 费用提醒，每项 PASS/FAIL/UNKNOWN + 证据引用。
终审经 change_student_status 写主档 GRADUATED/COMPLETED/INCOMPLETE（强制二次确认）。
培养方案必须按班级特例/专业年级确定解析；无法唯一解析时关键项 UNKNOWN 并阻断系统通过。
批次生成/预审均幂等，结果覆盖非追加；归档只收敛已终审的 GRADUATED/COMPLETED 行。
"""
...
_REVIEW_ROLES = {"ACADEMIC_ADMIN", "SCHOOL_ADMIN"}
# UNKNOWN 不能被当成 PASS。就业、学工归档、费用目前仍为提醒/人工复核，不自动卡审；
# 其余核心学业与培养过程项未知时必须进入异常队列。
_BLOCKING_UNKNOWN_ITEMS = {
...
def _audit(db, biz_id, action, detail=""):
    ...
    n, r, uid = _op()
    db.add(AffairsAuditTrail(tenant_id=_tid(), biz_type="AA_GRAD_AUDIT",
...
def _program_resolution(db, s):
    ...
    return resolve_student_program(db, s, tenant_id=_tid())


def _program_meta(resolution) -> dict:
    return {
        "programId": str(resolution.program.id) if resolution.program else None,
        "programVersion": resolution.program.version if resolution.program else None,
        "programBindingId": str(resolution.binding.id) if resolution.binding else None,
        "programResolutionRule": resolution.rule,
        "programResolutionStatus": resolution.status,
    }


def _get_program(db, s):
    """按确定规则解析学生方案；歧义/缺失时返回 None，禁止猜第一条 ACTIVE。"""
    return _program_resolution(db, s).program
...
def _check_credit(db, s):
    ...
    resolution = _program_resolution(db, s)
    prog = resolution.program
    acad = _acad_of(db, s)
    meta = _program_meta(resolution)
    if not prog:
        return {"item": "CREDIT", "result": "UNKNOWN", "owner": "AA_STAFF",
                "evidence": resolution.message, **meta}
    if not prog.total_credits:
        return {"item": "CREDIT", "result": "UNKNOWN", "owner": "AA_STAFF",
                "evidence": "适用培养方案未设置毕业总学分", **meta}
    if not acad:
        return {"item": "CREDIT", "result": "UNKNOWN", "owner": "AA_STAFF",
...
def _check_course_required(db, s):
    """必修成绩中无有效未通过记录；无台账/无必修成绩为 UNKNOWN。"""
    from app.models import AcademicGrade
    acad = _acad_of(db, s)
    if not acad:
        return {"item": "COURSE_REQUIRED", "result": "UNKNOWN", "owner": "AA_STAFF",
                "evidence": "无学业记录"}
    ...
    eff_rows = effective_grade_rows(rows)
    failed = [r.course_name for r in eff_rows if r.pass_status == "FAILED"]
    ok = not failed
    ev = (f"必修 {len(eff_rows)} 门全部通过" if ok else
          f"必修未通过 {len(failed)} 门：{'、'.join(failed[:3])}{'…' if len(failed) > 3 else ''}")
    return {"item": "COURSE_REQUIRED", "result": "PASS" if ok else "FAIL",
            "owner": "AA_STAFF", "evidence": ev}


def _check_course_elective(db, s):
    """选修学分按学生确定方案 requirement_json 审核。"""
    from app.models import AcademicGrade
    resolution = _program_resolution(db, s)
    prog = resolution.program
    acad = _acad_of(db, s)
...
def _check_practice(db, s):
    ...
    practice_names = {r.course_name for r in db.scalars(select(AaProgramCourse).where(
        ...
        AaProgramCourse.module.ilike("%实践%"), AaProgramCourse.is_deleted.is_(False))).all()
                      if r.course_name}
...
def _check_discipline(db, s):
    ...
    if not cs:
        return {"item": "DISCIPLINE", "result": "UNKNOWN", "owner": "COUNSELOR",
                "evidence": "无在校服务学生台账，无法核验处分状态", "refId": None}
    ...
    return {"item": "DISCIPLINE", "result": "PASS" if ok else "FAIL", "owner": "COUNSELOR",
            "evidence": f"未解除处分 {len(active_rows)} 条",
            "refId": str(active_rows[0].id) if active_rows else None}


def _check_domain_exists(db, item, model, student_field, s, owner, done_check=None):
    """通用跨域检查；查询异常与无记录均不得伪装 PASS。"""
    try:
        ...
        return {"item": item, "result": "UNKNOWN", "owner": owner,
                "evidence": f"供数查询失败：{type(exc).__name__}", "refId": None}
    if not rows:
        return {"item": item, "result": "UNKNOWN", "owner": owner,
                "evidence": "无该域记录", "refId": None}
...
def _check_fee(db, s) -> dict:
    """费用结清：教材费只作提醒；学费/住宿费无财务对接时不伪造结论。"""
    ...
    if unpaid:
        ...
        return {"item": "FEE", "result": "UNKNOWN", "owner": "FINANCE",
                "evidence": f"教材费台账有未结清 {len(unpaid)} 笔约 {amt:.2f} 元（学费/住宿费待财务回填，本项不阻断）",
                "refId": str(unpaid[0].id)}
    return {"item": "FEE", "result": "UNKNOWN", "owner": "FINANCE",
            "evidence": "待财务回填结清状态（教材费台账无欠费记录；本项不阻断）",
            "refId": None}


def _overall(items) -> str:
    if any(i.get("result") == "FAIL" for i in items):
        return "SYSTEM_ABNORMAL"
    if any(i.get("result") == "UNKNOWN" and i.get("item") in _BLOCKING_UNKNOWN_ITEMS for i in items):
        return "SYSTEM_ABNORMAL"
    ...
    with session() as db:
        ...
        for sid in sids:
            exist = db.scalars(select(AaGraduationAuditResult).where(
                AaGraduationAuditResult.tenant_id == _tid(), AaGraduationAuditResult.batch_id == b.id,
                AaGraduationAuditResult.student_id == sid,
...
def list_results(batch_id, user, status=None, overall=None, item=None, item_result=None,
                 page=1, page_size=50):
    """预审结果列表。item/item_result 供单项透视；COLLEGE_ADMIN 按本院收敛。"""
...
def _college_scope_ids(db, user) -> set[int] | None:
    """None=全校；set=学院范围（可空=fail-closed）。"""
...
def generate_certificates(batch_id, user, body) -> dict:
    ...
    with session() as db:
        ...
        results = db.scalars(select(AaGraduationAuditResult).where(
            ...
            AaGraduationAuditResult.is_deleted.is_(False))).all()
        if not results:
            raise AppException("DATA_CONFLICT", "该批次尚无毕业/结业终审结论，无可生成对象", http_status=409)
        ...
        db.commit()
        return {"batchId": str(b.id), "created": created, "skipped": skipped, "nextSeq": seq}

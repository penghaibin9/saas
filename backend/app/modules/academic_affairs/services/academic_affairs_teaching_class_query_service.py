...
from .academic_affairs_teaching_class_service import backfill_term
...
def _accessible_rows(db, user, rows):
    ...
    if role not in _ADMIN_ROLES:
        ...
        class_ids = {
            int(value) for (value,) in db.query(AaTeachingClassTeacher.teaching_class_id).filter(
                ...
                AaTeachingClassTeacher.teacher_key.in_(list(keys)),
    ...
    tasks = db.query(AaTeachingTask).filter(
        AaTeachingTask.tenant_id == _tid(), AaTeachingTask.id.in_(task_ids or [0]),
    ...
    batches = db.query(AaTeachingTaskBatch).filter(
        AaTeachingTaskBatch.tenant_id == _tid(), AaTeachingTaskBatch.id.in_(batch_ids or [0]),
    ...
    for row in rows:
        ...
        if not task:
            continue
        if scope.class_ids and task.class_id and int(task.class_id) in scope.class_ids:
            allowed.append(row)
            continue
        batch = batch_by_id.get(int(task.batch_id))
        if scope.college_ids and batch and batch.college_id and int(batch.college_id) in scope.college_ids:
            allowed.append(row)
    return allowed
...
def _teacher_dto(row):
    return {
        "teacherRelationId": str(row.id), "teacherId": str(row.teacher_id or ""),
        "teacherKey": row.teacher_key, "teacherName": row.teacher_name or "",
        "roleType": row.role_type, "startWeek": row.start_week, "endWeek": row.end_week,
        "status": row.status,
    }
...
def _class_dto(row, task=None, teachers=None):
    return {
        "teachingClassId": str(row.id), "teachingTaskId": str(row.teaching_task_id),
        "termId": str(row.term_id), "courseId": str(row.course_id),
        "classCode": row.class_code, "className": row.class_name,
        "classType": row.class_type, "capacity": row.capacity,
        "rosterStatus": row.roster_status, "rosterVersionNo": row.current_roster_version_no,
        "currentRosterVersionId": str(row.current_roster_version_id or ""),
        "status": row.status,
        "courseCode": task.course_code if task else "", "courseName": task.course_name if task else "",
        "administrativeClassId": str(task.class_id or "") if task else "",
        "administrativeClassName": task.class_name if task else "",
...
def list_teaching_classes(user, term_id=None, status=None, class_type=None, keyword=None,
    ...
    with session() as db:
        query = db.query(AaTeachingClass).filter(
            AaTeachingClass.tenant_id == _tid(), AaTeachingClass.is_deleted.is_(False),
        )
        if term_id:
        ...
        if keyword:
            ...
            rows = [
                ...
                or key in ((task_by_id.get(int(row.teaching_task_id)).course_name if task_by_id.get(int(row.teaching_task_id)) else "") or "").lower()
            ]
        ...
        items = [
            _class_dto(row, task_by_id.get(int(row.teaching_task_id)), teachers_by_class.get(int(row.id), []))
            for row in selected
        ]
        return items, total
...
def get_teaching_class(user, teaching_class_id: int) -> dict:
    from app.models import (
        ...
        AaTeachingClassTeacher, AaTeachingTask, StudentProfile,
    )
    ...
    with session() as db:
        ...
        if not _accessible_rows(db, user, [row]):
            raise no_data_scope("该教学班不在当前数据范围")
        task = db.query(AaTeachingTask).filter(
            AaTeachingTask.id == row.teaching_task_id, AaTeachingTask.tenant_id == _tid(),
            AaTeachingTask.is_deleted.is_(False),
        ).first()
        ...
        if row.current_roster_version_id:
            ...
            profiles = db.query(StudentProfile).filter(
                StudentProfile.tenant_id == _tid(), StudentProfile.id.in_(profile_ids or [0]),
            ...
            for member in members:
                ...
                current_members.append({
                    "memberId": str(member.id), "studentId": str(member.student_id),
                    ...
                    "sourceType": member.source_type, "sourceId": str(member.source_id or ""),
                    "status": member.status,
                })
        result = _class_dto(row, task, teachers)
        result["currentMembers"] = current_members
        result["rosterVersions"] = [{
            "rosterVersionId": str(version.id), "versionNo": version.version_no,
            "sourceType": version.source_type, "sourceId": str(version.source_id or ""),
            "memberCount": version.member_count, "rosterHash": version.roster_hash,
            "status": version.status, "reason": version.reason or "",
            "lockedAt": _iso(version.locked_at), "lockedBy": version.locked_by or "",
...
def backfill_teaching_classes(user, term_id: int, dry_run=True):
    return backfill_term(user, int(term_id), dry_run=bool(dry_run))

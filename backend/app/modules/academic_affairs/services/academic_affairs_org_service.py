...
def _college_dtos(db, rows) -> list[dict]:
    ...
    college_ids = [c.id for c in rows] or [0]
    major_counts = dict(db.execute(select(Major.college_id, func.count(Major.id)).where(
        Major.tenant_id == _tid(), Major.is_deleted.is_(False), Major.college_id.in_(college_ids)
    ).group_by(Major.college_id)).all())
    result = []
    for c in rows:
        dto = _college_dto(c)
        ...
        dto.update(secretaryName=person.real_name or person.login_name if person else None,
                   ...
                   responsibleName=person.real_name or person.login_name if person else None,
                   majorCount=int(major_counts.get(c.id, 0)),
                   referenceStatus="IN_USE" if major_counts.get(c.id, 0) else "UNUSED")
        result.append(dto)
    return result
...
def list_majors(user, college_id=None, enroll_status=None, keyword=None, page=1, page_size=50):
    ...
    with session() as db:
        ...
        major_ids = [m.id for m in rows] or [0]
        class_rows = db.execute(select(SchoolClass.major_id, func.count(SchoolClass.id),
            func.count(func.distinct(SchoolClass.grade))).where(
            SchoolClass.tenant_id == _tid(), SchoolClass.is_deleted.is_(False),
            SchoolClass.major_id.in_(major_ids)).group_by(SchoolClass.major_id)).all()
        class_metrics = {major: (int(classes), int(grades)) for major, classes, grades in class_rows}
        output = []
        for major in rows:
            dto = _major_dto(major, cmap.get(major.college_id))
            class_count, grade_count = class_metrics.get(major.id, (0, 0))
            dto.update(classCount=class_count, gradeCount=grade_count,
                       referenceStatus="IN_USE" if class_count else "UNUSED")
            output.append(dto)
        return output, total
...
def list_classes(user, major_id=None, grade=None, class_status=None, keyword=None, page=1, page_size=50, *, term_id=None):
    ...
    with session() as db:
        ...
        class_ids = [c.id for c in rows] or [0]
        ...
        teacher_ids = {c.head_teacher_id for c in rows if c.head_teacher_id}
        teachers = {u.id: (u.real_name or u.login_name) for u in db.scalars(select(User).where(
            User.tenant_id == _tid(), User.id.in_(teacher_ids or [0]))).all()}
        output = []
        for school_class in rows:
            dto = _class_dto(school_class, mmap.get(school_class.major_id))
            dto.update(studentCount=int(student_counts.get(school_class.id, 0)),
                       headTeacherName=teachers.get(school_class.head_teacher_id),
                       referenceStatus="IN_USE" if student_counts.get(school_class.id, 0) else "UNUSED")
            output.append(dto)
        return output, total
...
def list_grades(user, college_id=None, major_id=None):
    ...
    with session() as db:
        ...
        grade_major_rows = db.execute(select(
            SchoolClass.grade, Major.major_name, SchoolClass.graduate_year
        ).join(
            Major, and_(Major.id == SchoolClass.major_id, Major.tenant_id == SchoolClass.tenant_id,
                        Major.is_deleted.is_(False))).where(*conds).distinct()).all()
        majors_by_grade = {}
        graduation_years_by_grade = {}
        for grade_value, major_name, graduation_year in grade_major_rows:
            majors_by_grade.setdefault(grade_value, []).append(major_name)
            if graduation_year:
                graduation_years_by_grade.setdefault(grade_value, set()).add(str(graduation_year))
        items = [{"grade": g, "admissionYear": g,
                  "expectedGraduationYear": "、".join(sorted(graduation_years_by_grade.get(g, set()))) or None,
                  "classCount": n, "studentCount": int(smap.get(g, 0)),
                  "majorCount": len(majors_by_grade.get(g, [])),
                  "majorNames": majors_by_grade.get(g, [])} for g, n in rows]
        return {"items": items}
...
def list_teaching_classes(user, term_code=None, batch_id=None, page=1, page_size=50, *, term_id=None, keyword=None):
    ...
    with session() as db:
        ...
        formal_ids = [int(item["teachingClassId"]) for item in items if item.get("teachingClassId")]
        teachers_by_class = {}
        for relation in db.scalars(select(AaTeachingClassTeacher).where(
            AaTeachingClassTeacher.tenant_id == _tid(), AaTeachingClassTeacher.is_deleted.is_(False),
            AaTeachingClassTeacher.status == "ACTIVE",
            AaTeachingClassTeacher.teaching_class_id.in_(formal_ids or [0]))).all():
            teachers_by_class.setdefault(relation.teaching_class_id, []).append(relation.teacher_name or relation.teacher_key)
        formal_by_id = {formal.id: formal for _, _, _, formal in rows if formal}
        for item in items:
            formal_id = int(item["teachingClassId"]) if item.get("teachingClassId") else None
            formal = formal_by_id.get(formal_id)
            item.update({
                "teacherNames": teachers_by_class.get(formal_id, []),
                "rosterVersion": int(formal.current_roster_version_no or 0) if formal else None,
                "rosterStatus": formal.roster_status if formal else "PENDING_FORMALIZATION",
            })
        total = len(items)
        offset = (max(1, page) - 1) * page_size
...
def list_class_students(user, class_id, keyword=None, page=1, page_size=50):
    ...
    from app.models import Major, StudentContact, StudentProfile
    with session() as db:
        ctx = _ctx(user, db)
        ...
        items = [{"id": str(s.id), "studentId": str(s.id), "studentNo": s.student_no, "realName": s.real_name,
                  ...
                  "classId": str(s.class_id) if s.class_id else None, "className": c.class_name,
                  "majorName": major.major_name if major else None, "grade": c.grade,
                  "version": int(s.version or 0)} for s in rows]
        return items, total
...
def org_stats(user):
    ...
    with session() as db:
        ...
        colleges = db.scalars(select(College).where(*col_conds).order_by(College.sort_order, College.id)).all()
        major_counts = dict(db.execute(select(Major.college_id, func.count(Major.id)).where(
            *maj_conds).group_by(Major.college_id)).all())
        class_counts = dict(db.execute(select(Major.college_id, func.count(SchoolClass.id)).join(
            Major, and_(Major.id == SchoolClass.major_id, Major.tenant_id == SchoolClass.tenant_id)).where(
            *cls_conds, Major.is_deleted.is_(False)).group_by(Major.college_id)).all())
        student_counts = dict(db.execute(select(Major.college_id, func.count(StudentProfile.id)).join(
            SchoolClass, and_(SchoolClass.id == StudentProfile.class_id,
                             SchoolClass.tenant_id == StudentProfile.tenant_id)).join(
            Major, and_(Major.id == SchoolClass.major_id, Major.tenant_id == SchoolClass.tenant_id)).where(
            *stu_conds, SchoolClass.is_deleted.is_(False), Major.is_deleted.is_(False)
        ).group_by(Major.college_id)).all())
        result["collegeDistribution"] = [{
            "collegeId": str(college.id), "collegeName": college.college_name,
            "majorCount": int(major_counts.get(college.id, 0)),
            "classCount": int(class_counts.get(college.id, 0)),
            "studentCount": int(student_counts.get(college.id, 0)),

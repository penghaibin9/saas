...
HEADERS = {"楼栋编码": "buildingCode", "楼层": "floorNo", "教室编号": "roomCode",
           ...
           "允许排课": "allowSchedule", "允许排考": "allowExam", "开放借用": "allowBorrow"}
LEGACY_HEADERS = list(HEADERS)[:-3]


def _rule(value, label, default):
...
def generate_preview(body):
    with session() as db:
        ...
        generated = [{"buildingCode": b.building_code, "floorNo": floor,
            ...
            "allowSchedule": body.allowSchedule, "allowExam": body.allowExam, "allowBorrow": body.allowBorrow}
    ...
    with session() as db:
        ...
        for number, raw in enumerate(raw_rows, 2):
            ...
            try:
                ...
                for field, label, default in (("allowSchedule", "允许排课", True), ("allowExam", "允许排考", True), ("allowBorrow", "开放借用", False)):
                    row[field] = _rule(raw.get(field), label, default)
        ...
        with session() as db:
            ...
            for item in payload["items"]:
                ...
                c = AaClassroom(tenant_id=_tid(), building_id=int(r["buildingId"]), floor_no=r["floorNo"],
                    ...
                    allow_schedule=r.get("allowSchedule", True), allow_exam=r.get("allowExam", True),
                    allow_borrow=r.get("allowBorrow", True),  # Preserve previously persisted previews.
...
def import_preview(content):
    ...
    try:
        ...
        if headers not in (list(HEADERS), LEGACY_HEADERS):
        ...
        for line, values in enumerate(iterator, 2):
            ...
            raw.append({HEADERS[label]: values[i] if i < len(values) else None for i, label in enumerate(headers)})

...
def require_renewal_candidate_view(user=Depends(require_platform_principal)):
    """Renewal candidates are shared between commercial and customer-success duties."""
    from app.services import platform_access_governance_service as pam

    duties = pam.effective_platform_duties(user)
    if "*" in duties or "commercial.view" in duties or "customerSuccess.manage" in duties:
        return user
    raise no_permission("平台职责不允许查看商业续费候选")


@router.get("/commercial/tenants/{tenant_id}/renewal-candidates", summary="读取当前代次最晚已付来源续费候选")
def renewal_candidates(
    tenant_id: int,
    withinDays: int = Query(120, ge=1, le=3650),
    page: int = Query(1, ge=1, le=10000),
    pageSize: int = Query(20, ge=1, le=100),
    user=Depends(require_renewal_candidate_view),
):
    from app.services import module_commerce_renewal_service as renewal
    return success(renewal.list_renewal_candidates(
        int(tenant_id), within_days=withinDays, page=page, page_size=pageSize,
    ))


@router.post("/commercial/tenants/{tenant_id}/sources/{source_id}/renewal-followup", summary="把已付来源显式交给现有客户成功续费任务")
def renewal_followup(
    tenant_id: int,
    source_id: int,
    body: dict = Body(...),
    ...
    from app.services import module_commerce_renewal_service as renewal
    return success(
        renewal.ensure_renewal_followup(
            user, int(tenant_id), int(source_id),
            due_at=body.get("dueAt"), owner_name=str(body.get("ownerName") or ""),
...
def install_into_platform_router(target: APIRouter) -> int:
    ...
    for route in router.routes:
        ...
        if not isinstance(route, APIRoute) or not signatures or declared & signatures:
            raise RuntimeError("Invalid or duplicate module-commerce renewal route declaration")
        ...
        if collisions:
            raise RuntimeError(f"Module-commerce renewal route collision: {collisions}")

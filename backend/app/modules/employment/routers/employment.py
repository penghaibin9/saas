...
def student_create(body: StudentCreate, user=Depends(require_permission("employment.student.manage"))):
    return success(svc.create_student(body.model_dump()), message="已新增")


@router.put("/students/{sid}", summary="编辑就业记录")
def student_update(sid: str, body: StudentUpdate, user=Depends(require_permission("employment.student.manage"))):
    return success(svc.update_student(sid, body.model_dump()), message="已保存")


@router.post("/students/{sid}/void", summary="作废就业记录（原因≥5字）")
def student_void(sid: str, body: ReasonBody, user=Depends(require_permission("employment.student.manage"))):
    return success(svc.void_student(sid, body.reason), message="已作废")


@router.post("/students/mark-destination", summary="批量标记去向")
def mark_destination(body: MarkDestBody, user=Depends(require_permission("employment.student.manage"))):
...
def material_return(mid: str, body: ReasonBody, user=Depends(require_permission("employment.material.approve"))):
...
def unemployed_employed(body: IdsBody, user=Depends(require_permission("employment.unemployed.manage"))):
    return success(svc.mark_employed(body.ids), message="已标记")


@router.post("/unemployed/mark-key-help", summary="标记重点帮扶")
def unemployed_key_help(body: IdsBody, user=Depends(require_permission("employment.unemployed.manage"))):
    return success(svc.mark_key_help(body.ids), message="已标记")


@router.post("/unemployed/assign-teacher", summary="分配就业老师")
def unemployed_assign(body: AssignTeacherBody, user=Depends(require_permission("employment.unemployed.assign"))):
...
def followup_create(body: FollowUpCreate, user=Depends(require_permission("employment.followup.manage"))):
    return success(svc.create_followup(body.model_dump()), message="已记录")


@router.post("/followups/{fid}/void", summary="作废跟进（原因≥5字）")
def followup_void(fid: str, body: ReasonBody, user=Depends(require_permission("employment.followup.manage"))):

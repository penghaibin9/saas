...
class AaRegistrationBatch(PKMixin, TenantMixin, CommonMixin, Base):
    ...
    register_type: Mapped[str] = mapped_column(String(20), nullable=False, default="ENROLL",
                                               comment="ENROLL 入学 / ANNUAL 学年 / SEMESTER 学期")
...
class AaGradeTask(PKMixin, TenantMixin, CommonMixin, Base):
    ...
    midterm_ratio: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="期中占比%(0=不启用期中)")
...
class AaGradeRecord(PKMixin, TenantMixin, CommonMixin, Base):
    ...
    midterm_score: Mapped[int | None] = mapped_column(Integer, comment="期中分(期中占比>0时启用)")
    ...
    prev_midterm_score: Mapped[int | None] = mapped_column(Integer)

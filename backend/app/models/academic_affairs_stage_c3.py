...
while every formal evaluation/decision is append-only here.  ``AaArchiveBatch``
remains the operational batch projection, while every successful archive/correction
produces an immutable manifest version.
"""
...
class GraduationDecisionFact(PKMixin, TenantMixin, AuditTimeMixin, Base):
    """Immutable final graduation decision referencing the exact evaluation run used."""

    __tablename__ = "t_aa_graduation_decision_fact"
    __table_args__ = (
        UniqueConstraint("tenant_id", "result_id", name="uk_aa_grad_decision_result"),
...
class PostArchiveCorrectionCase(PKMixin, TenantMixin, CommonMixin, Base):
    ...
    ``GRADUATION``.  Applying a case appends a new fact/correction evidence and a new
    ArchiveManifest version; it never reopens the archived term.
    """

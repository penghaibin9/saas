...
class AaGradeChangeRequest(PKMixin, TenantMixin, CommonMixin, Base):
    ...
    score_snapshot_json: Mapped[str | None] = mapped_column(Text, comment="动态更正原方案、原分项及拟更正分项")
    score_snapshot_hash: Mapped[str | None] = mapped_column(String(64))
    authority_snapshot_json: Mapped[str | None] = mapped_column(Text, comment="申请时正式名单、课程及生效策略证据")
    authority_snapshot_hash: Mapped[str | None] = mapped_column(String(64))
    evidence_manifest_json: Mapped[str | None] = mapped_column(String(4000), comment="申请时正式材料绑定快照")
    evidence_manifest_hash: Mapped[str | None] = mapped_column(String(64), comment="冻结材料清单SHA256")

...
- 现有身份导入、迁移和 Excel 作业通过 adapter_ref 接入，不推倒原业务事务；
- 原始文件与系统生成回执全部进入 FileObject；
- 多实例确认使用 ImportJob 行锁租约 + 原批次租约双重保护；
- 导出/回执具备有效期、撤销、短时下载票据和清理状态。
"""
...
def _export_row(row) -> dict[str, Any]:
    ...
    status = "EXPIRED" if expired and row.status == "SUCCEEDED" else row.status
    return {
        "id": str(row.id),
        "jobType": "EXPORT",
        "moduleCode": row.module_code,
...
def _owned_import(db, job_id: str | int, user: dict, *, lock: bool = False):
    ...
    if actor_id:
        stmt = stmt.where(or_(ImportJob.operator_id == actor_id, ImportJob.operator_id.is_(None)))
    else:
        stmt = stmt.where(ImportJob.created_by.is_(None))
    if lock:
...
def _owned_export(db, job_id: str | int, user: dict, *, lock: bool = False):
    ...
    actor_id = _actor_id(user)
    if actor_id:
        stmt = stmt.where(or_(ExportJob.operator_id == actor_id, ExportJob.operator_id.is_(None)))
    else:
        stmt = stmt.where(ExportJob.created_by.is_(None))
    if lock:
        stmt = stmt.with_for_update()
    row = db.scalars(stmt).first()
    if not row:
...
def confirm_identity_import_job(
    ...
    *,
    ...
    try:
        ...
        if idempotency_key:
            snapshot["confirmIdempotencyHash"] = hashlib.sha256(idempotency_key.encode("utf-8")).hexdigest()
    ...
    *,
    ...
    try:
        ...
        if wanted in {"", "IMPORT"}:
            ...
            if keyword:
                like = f"%{keyword}%"
                stmt = stmt.where(or_(ImportJob.import_type.like(like), ImportJob.adapter_ref.like(like)))
            rows.extend(_import_row(row) for row in db.scalars(stmt.order_by(ImportJob.id.desc())).all())
        if wanted in {"", "EXPORT"}:
            stmt = select(ExportJob).where(
                ExportJob.tenant_id == _tenant_id(),
                ExportJob.is_deleted.is_(False),
            ...
            if keyword:
                like = f"%{keyword}%"
                stmt = stmt.where(or_(ExportJob.export_type.like(like), ExportJob.purpose.like(like)))
            rows.extend(_export_row(row) for row in db.scalars(stmt.order_by(ExportJob.id.desc())).all())
        rows.sort(key=lambda item: (item.get("createdAt") or "", int(item["id"])), reverse=True)
        total = len(rows)
        start = (page - 1) * page_size
        return {"list": rows[start:start + page_size], "total": total, "page": page, "pageSize": page_size}
    finally:
        db.close()


def get_import_job(job_id: str, *, user: dict) -> dict:
    db = get_sessionmaker()()
    try:
        return _import_row(_owned_import(db, job_id, user))
    finally:
        db.close()


def get_export_job(job_id: str, *, user: dict) -> dict:
    db = get_sessionmaker()()
    try:
        return _export_row(_owned_export(db, job_id, user))
    finally:
        db.close()

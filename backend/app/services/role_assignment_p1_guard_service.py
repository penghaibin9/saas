...
The legacy service owns persistence mechanics. This layer closes runtime authorization
holes: disabled-role grants, student/staff role mixing, duplicate authority overwrite,
future grants becoming active too early, and removal of the final active SCHOOL_ADMIN.
All protected mutations keep guard + write + critical audit in one DB transaction.
"""
...
from app.models.role_assignment import SOURCE_MANUAL, SOURCE_TRANSFER, VALIDITY_ACTIVE, RoleAssignmentValidity
...
def _lock_role(db, tenant_id: int, role_code: str) -> Role:
    ...
    role = db.scalars(select(Role).where(
        Role.tenant_id == tenant_id, Role.role_code == code, Role.is_deleted.is_(False),
...
def _is_student_account(db, tenant_id: int, account: User) -> bool:
    if str(account.user_type or "").upper() == "STUDENT":
        return True
    if db.scalars(select(StudentAccountLink.id).where(
...
def _active_role_link_for(db, tenant_id: int, user_id: int, role_id: int):
    """Runtime truth is UserRole, including legacy rows that have no validity extension."""
    return db.scalars(select(UserRole).where(
        UserRole.tenant_id == tenant_id,
...
def grant_assignment(
    user_id: int, role_code: str, *, reason: str, effective_at: Any = None,
    expires_at: Any = None, tenant_id: int | None = None, user: dict | None = None,
) -> dict:
    ...
    try:
        ...
        if _active_role_link_for(db, tid, int(user_id), int(role.id)) is not None:
            raise AppException(
                "DATA_CONFLICT",
                "该账号已持有此角色；请在现有授权记录上复核、转交或回收",
                http_status=409,
            )
        validity, account, role = ras._grant_assignment_in_db(
            db, user_id=int(user_id), role_code=role.role_code, reason=reason,
            start=now, end=end, source_type=SOURCE_MANUAL, source_id=None,
            tenant_id=tid, actor=user,
        )
        assignment_id = int(validity.id)
        from app.services import audit_log
        audit_log.record_critical_in_session(
            db, "ROLE_ASSIGNMENT_GRANT", f"role-assignment:{assignment_id}",
            detail={
                "userId": int(account.id), "roleCode": role.role_code, "reason": reason,
                "effectiveAt": str(now), "expiresAt": str(end or ""),
                "sourceType": SOURCE_MANUAL, "moduleCode": "systemAdmin", "productionGuard": True,
            },
            tenant_id=tid, resource_id=str(assignment_id),
        )
        db.commit()
    except Exception:
        db.rollback()
...
def revoke_assignment(
    ...
    try:
        ...
        from app.services import audit_log
        audit_log.record_critical_in_session(
            db, "ROLE_ASSIGNMENT_REVOKE", f"role-assignment:{assignment_id}",
            detail={
                "userId": old_user_id, "roleCode": row.role_code, "reason": reason,
                "expectedVersion": expected, "moduleCode": "systemAdmin", "productionGuard": True,
            },
            tenant_id=tid, resource_id=str(assignment_id),
        )
        db.commit()
...
def transfer_assignment(
    assignment_id: int, *, to_user_id: int, reason: str, expires_at: Any = None,
    expected_version: int | None, tenant_id: int | None = None, user: dict | None = None,
) -> dict:
    reason = str(reason or "").strip()
    if len(reason) < 5:
    ...
    try:
        ...
        ras._assert_role_delegation_allowed(db, actor=user, role=role, tenant_id=tid)
        if _active_role_link_for(db, tid, target_user_id, int(role.id)) is not None:
            raise AppException("DATA_CONFLICT", "转交目标已经持有该角色", http_status=409)
        new_end = ras._parse_dt(expires_at, "expiresAt") if expires_at is not None else row.expires_at
        now = ras._now()
        if new_end is not None and new_end <= now:
            raise AppException("VALIDATION_ERROR", "到期时间必须晚于转交生效时间")
        ras._revoke_assignment_in_db(
            db, row=row, reason=reason, actor=user, transferred_to_user_id=target_user_id,
        )
        new_validity, _, _ = ras._grant_assignment_in_db(
            db, user_id=target_user_id, role_code=role.role_code, reason=reason,
            start=now, end=new_end, source_type=SOURCE_TRANSFER, source_id=str(assignment_id),
            tenant_id=tid, actor=user,
        )
        ...
        from app.services import audit_log
        audit_log.record_critical_in_session(
            db, "ROLE_ASSIGNMENT_TRANSFER", f"role-assignment:{assignment_id}",
            detail={
                "assignmentId": int(assignment_id), "fromUserId": old_user_id,
                "toUserId": target_user_id, "roleCode": role.role_code, "reason": reason,
                "expectedVersion": expected, "newAssignmentId": new_assignment_id,
                "moduleCode": "systemAdmin", "productionGuard": True,
            },
            tenant_id=tid, resource_id=str(assignment_id),
        )
        db.commit()

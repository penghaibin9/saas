...
def upgrade():
    ...
    if not i.has_table("t_internship_placement_snapshot"):
        cols=[
            sa.Column("id",sa.BigInteger(),primary_key=True,autoincrement=True),sa.Column("tenant_id",sa.BigInteger(),nullable=False),sa.Column("record_id",sa.BigInteger(),nullable=False),sa.Column("placement_seq",sa.Integer(),nullable=False),sa.Column("snapshot_version",sa.Integer(),nullable=False,server_default="1"),sa.Column("application_id",sa.BigInteger()),sa.Column("enterprise_decision_id",sa.BigInteger()),sa.Column("campaign_id",sa.BigInteger()),sa.Column("batch_id",sa.BigInteger(),nullable=False),sa.Column("company_id",sa.BigInteger(),nullable=False),sa.Column("position_id",sa.BigInteger(),nullable=False),
            sa.Column("company_name",sa.String(200),nullable=False),sa.Column("company_credit_code",sa.String(50)),sa.Column("position_title",sa.String(200),nullable=False),sa.Column("position_category",sa.String(50)),sa.Column("work_location",sa.String(200)),sa.Column("work_address",sa.String(300)),sa.Column("work_content",sa.Text()),sa.Column("major_requirement",sa.String(200)),sa.Column("grade_requirement",sa.String(100)),sa.Column("salary_range",sa.String(50)),sa.Column("subsidy",sa.String(50)),sa.Column("remuneration_type",sa.String(30)),sa.Column("remuneration_amount",sa.Float()),sa.Column("remuneration_cycle",sa.String(30)),sa.Column("daily_hours",sa.Float()),sa.Column("weekly_hours",sa.Float()),sa.Column("shift_type",sa.String(30)),sa.Column("night_shift",sa.Boolean()),sa.Column("overtime_allowed",sa.Boolean()),sa.Column("rest_days",sa.String(50)),sa.Column("rest_days_per_week",sa.Float()),sa.Column("accommodation_provided",sa.Boolean()),sa.Column("meal_provided",sa.Boolean()),sa.Column("hazardous_flag",sa.Boolean()),sa.Column("special_equipment",sa.String(200)),sa.Column("prohibited_reason",sa.String(500)),sa.Column("enterprise_mentor_name",sa.String(100)),sa.Column("rights_status",sa.String(30)),sa.Column("rights_rule_version",sa.String(64)),sa.Column("rights_checked_at",sa.DateTime()),sa.Column("position_version",sa.Integer(),nullable=False),sa.Column("position_updated_at",sa.DateTime()),sa.Column("snapshot_json",sa.JSON(),nullable=False),sa.Column("snapshot_hash",sa.String(64),nullable=False),sa.Column("snapshot_sha256",sa.String(64),nullable=False),sa.Column("placement_at",sa.DateTime(),nullable=False),sa.Column("captured_at",sa.DateTime(),nullable=False),sa.Column("captured_by_user_id",sa.BigInteger()),sa.Column("created_at",sa.DateTime(),nullable=False),sa.Column("created_by",sa.BigInteger()),sa.UniqueConstraint("tenant_id","record_id","placement_seq",name="uk_intern_placement_snapshot_seq")]
        op.create_table("t_internship_placement_snapshot",*cols,mysql_engine="InnoDB")
        op.create_index("ix_t_internship_placement_snapshot_tenant_id","t_internship_placement_snapshot",["tenant_id"])
        op.create_index("ix_t_internship_placement_snapshot_snapshot_sha256","t_internship_placement_snapshot",["snapshot_sha256"])
        op.create_index("ix_intern_placement_snapshot_record_time","t_internship_placement_snapshot",["tenant_id","record_id","placement_at"])

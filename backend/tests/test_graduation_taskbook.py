...
def _gd_student_with_mentor(graduation_client, h, no, name, mentor_no):
    ...
    mid = graduation_client.post(GD_MENTOR, headers=h, json={"teacherNo": mentor_no, "teacherName": f"导师{mentor_no}"}).json()["data"]["id"]
...
def _gd_student_with_topic_and_mentor(graduation_client, h, no, name, mentor_no):
    """建档 + 选题确认（stage→TASKBOOK_CONFIRM）+ 分配导师，模拟任务书下达的真实前置条件。"""
    gid = _gd_student_with_mentor(graduation_client, h, no, name, mentor_no)
...
def test_issue_requires_mentor_and_no_duplicate(graduation_client, auth_headers, db_mode):
    h = auth_headers
    sid = graduation_client.post(STU, headers=h, json={"studentNo": "TB001", "realName": "无导师生", "classId": make_org_class()}).json()["data"]["id"]
    gid = graduation_client.post(GD_STU, headers=h, json={"studentId": sid}).json()["data"]["id"]

    no_mentor = graduation_client.post(f"{GD_TASKBOOK}/{gid}/issue", headers=h, json={"objective": "完成毕设选题相关设计", "content": "系统设计与实现"})
    assert no_mentor.json()["code"] != 0

    gid2 = _gd_student_with_mentor(graduation_client, h, "TB002", "有导师生", "MTB1")
    ok = graduation_client.post(f"{GD_TASKBOOK}/{gid2}/issue", headers=h, json={"objective": "完成系统设计文档", "content": "完成系统详细设计"})
    assert ok.json()["code"] == 0
    assert ok.json()["data"]["status"] == "PENDING_CONFIRM"
    assert ok.json()["data"]["taskbookVersion"] == 1

    dup = graduation_client.post(f"{GD_TASKBOOK}/{gid2}/issue", headers=h, json={"objective": "重复下达", "content": "重复"})
    assert dup.json()["code"] != 0


def test_confirm_advances_student_stage_to_guiding(graduation_client, auth_headers, db_mode):
    h = auth_headers
    gid = _gd_student_with_topic_and_mentor(graduation_client, h, "TB101", "小明", "MTB2")
    stu_before = graduation_client.get(f"{GD_STU}/{gid}", headers=h).json()["data"]
    assert stu_before["stage"] == "TASKBOOK_CONFIRM"

    graduation_client.post(f"{GD_TASKBOOK}/{gid}/issue", headers=h, json={"objective": "完成系统设计文档", "content": "完成系统详细设计"})
    confirm = graduation_client.post(f"{GD_TASKBOOK}/{gid}/confirm", headers=h)
    ...
    again = graduation_client.post(f"{GD_TASKBOOK}/{gid}/confirm", headers=h)
    assert again.json()["code"] != 0  # 已确认无需再确认


def test_change_requires_confirmed_and_reason_keeps_history(graduation_client, auth_headers, db_mode):
    h = auth_headers
    gid = _gd_student_with_mentor(graduation_client, h, "TB201", "小红", "MTB3")
    graduation_client.post(f"{GD_TASKBOOK}/{gid}/issue", headers=h, json={"objective": "完成系统设计文档", "content": "完成系统详细设计"})

    too_early = graduation_client.post(f"{GD_TASKBOOK}/{gid}/change", headers=h, json={"objective": "调整目标", "reason": "范围收窄"})
    assert too_early.json()["code"] != 0  # 尚未确认不可变更

    graduation_client.post(f"{GD_TASKBOOK}/{gid}/confirm", headers=h)

    short_reason = graduation_client.post(f"{GD_TASKBOOK}/{gid}/change", headers=h, json={"objective": "调整目标", "reason": "x"})
    assert short_reason.json()["code"] != 0

    chg = graduation_client.post(f"{GD_TASKBOOK}/{gid}/change", headers=h, json={"objective": "调整为移动端方向", "reason": "选题范围调整为移动端"})
    body = chg.json()["data"]
    ...
    reconfirm = graduation_client.post(f"{GD_TASKBOOK}/{gid}/confirm", headers=h)
    assert reconfirm.json()["data"]["status"] == "CONFIRMED"


def test_stats_and_export(graduation_client, auth_headers, db_mode):
    h = auth_headers
    gid = _gd_student_with_mentor(graduation_client, h, "TB301", "小刚", "MTB4")
    graduation_client.post(f"{GD_TASKBOOK}/{gid}/issue", headers=h, json={"objective": "完成系统设计文档", "content": "完成系统详细设计"})

    stats = graduation_client.get(f"{GD_TASKBOOK}/stats", headers=h).json()["data"]
    assert stats["total"] >= 1

    exp = graduation_client.post(f"{GD_TASKBOOK}/export", headers=h)
    body = exp.json()
    assert body["code"] == 0

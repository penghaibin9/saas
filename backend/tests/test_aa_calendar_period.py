...
def test_r7_archive_term_and_role_gate(client, db_mode):
    """校历归档：DRAFT 不可归档(409)；发布后可归档；归档后幂等；教师角色执行发布/归档 403。"""
    hdr = _hdr(client, "school_admin01")
    tid = _term(client, hdr, "2030-2031", 1).json()["data"]["termId"]
    draft_archive = client.post(f"{BASE}/terms/{tid}/archive", headers=hdr)
    assert draft_archive.status_code == 409
    client.post(f"{BASE}/time-slots", headers=hdr, json={"slotNo": 24})
    client.post(f"{BASE}/terms/{tid}/calendar/publish", headers=hdr)
    arch = client.post(f"{BASE}/terms/{tid}/archive", headers=hdr)
    assert arch.status_code == 200 and arch.json()["data"]["status"] == "ARCHIVED"
    arch2 = client.post(f"{BASE}/terms/{tid}/archive", headers=hdr)
    assert arch2.status_code == 200  # 幂等
    detail = client.get(f"{BASE}/terms/{tid}", headers=hdr)
    assert detail.status_code == 200 and detail.json()["data"]["status"] == "ARCHIVED"


def test_r7b_teacher_role_forbidden_publish_archive(client, db_mode):
    """任课教师（ACADEMIC_TEACHER）有 academicAffairs.* 通配权限，但不在超高危角色白名单 → 发布/归档 403。"""
    ...
    arch = client.post(f"{BASE}/terms/{tid}/archive", headers=teacher_hdr)
...
def test_r8_student_forbidden_403(client, db_mode):
    ...
    assert client.post(f"{BASE}/terms/{tid}/archive", headers=hdr).status_code == 403

...
def test_funding_appeal_blocks_publicity_confirm_and_scan(client, db_mode):
    ...
    a = post_versioned(client, f"{BASE}/funding/applications/{app_id}/appeal", headers=hdr,
                    json={"reason": "对公示获奖名单有异议请复核"}).json()
    assert a["code"] == 0
    # 人工确认应被拦
    blocked = post_versioned(client, f"{BASE}/funding/applications/{app_id}/publicity-confirm", headers=hdr, json={}).json()
    ...
    # 复核成立 → 驳回 + 通知字段
    aid = a["data"]["appealId"]
    rv = post_versioned(client, f"{BASE}/funding/appeals/{aid}/review", headers=hdr,
...
def test_funding_appeal_sustained_rejects(client, db_mode):
    ...
    aid = a["data"]["appealId"]
    assert post_versioned(client, f"{BASE}/funding/applications/{app_id}/appeal", headers=hdr,
    ...
    assert post_versioned(client, f"{BASE}/funding/appeals/{aid}/review", headers=hdr,
...
def test_funding_appeal_overruled_and_non_publicity(client, db_mode):
    hdr = _hdr(client, "school_admin01")
    app_id = _seed_app(db_mode["student"], "PUBLICITY")
    aid = post_versioned(client, f"{BASE}/funding/applications/{app_id}/appeal", headers=hdr,
    ...
    assert post_versioned(client, f"{BASE}/funding/applications/{ap2}/appeal", headers=hdr,

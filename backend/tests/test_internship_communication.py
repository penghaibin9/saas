...
def _seed(db_mode):
    """企业 + 两学生实习记录（advisor=刘强/王芳）。"""
    from app.db.session import get_sessionmaker
    from app.models import EmpCompany, InternshipRecord, StudentProfile
    db = get_sessionmaker()()
    try:
        co = EmpCompany(tenant_id=TID, name="沟通测试企业")
        db.add(co); db.flush()
        ids = {"enterprise_id": co.id}
...
def test_create_and_list(client, db_mode):
    ...
    lst = client.get(INT, headers=a).json()["data"]

...
def test_package10_migration_declares_separate_truth_and_atomic_guards():
    source = Path("alembic/versions/20260806_funding_package10_truth.py").read_text("utf-8")
...
def test_package10_mysql_reservation_and_snapshot_are_database_enforced(db_mode):
    ...
    project = batch = first = second = disbursement = None
    try:
        ...
        snapshot = db.execute(text("""
            SELECT amount, approved_amount_snapshot, approved_at_snapshot,
             ...
             WHERE id = :id
        """), {"id": disbursement.id}).mappings().one()
        ...
        try:
            if disbursement is not None and disbursement.id:
            ...
            if first is not None and first.id:
                db.execute(text("DELETE FROM t_affairs_funding_application WHERE id IN (:first, :second)"),
                           {"first": first.id, "second": second.id if second is not None else -1})
            if batch is not None and batch.id:
                db.execute(text("DELETE FROM t_affairs_funding_batch WHERE id = :id"), {"id": batch.id})
            if project is not None and project.id:
                db.execute(text("DELETE FROM t_affairs_funding_project WHERE id = :id"), {"id": project.id})
            db.commit()
        except Exception:
            db.rollback()
        db.close()

...
def _to_guiding(graduation_client, h, sh, gid, name):
...
def _pending_final_id(graduation_client, h, name):
    lst = graduation_client.get(FINAL, headers=h, params={"status": "PENDING_REVIEW"}).json()["data"]
    return next(r for r in lst["items"] if r["studentName"] == name)["id"]


def test_submit_order_draft_then_final(graduation_client, auth_headers, db_mode):
    h = auth_headers
    name = "成果小明"
    gid = _gd_student_with_topic(graduation_client, h, "FN001", name)
    sh = _stu_token(name)
    _to_guiding(graduation_client, h, sh, gid, name)

    # 定稿须先有通过的初稿
    early = graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "定稿", "attachments": [_upload_pdf(graduation_client, sh)]})
    assert early.json()["code"] != 0

    draft = graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "attachments": [_upload_pdf(graduation_client, sh)]})
    assert draft.json()["code"] == 0
    assert draft.json()["data"]["version"] == "v1"

    # 待审不可重复提交
    dup = graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "attachments": [_upload_pdf(graduation_client, sh)]})
    assert dup.json()["code"] != 0

    fid = _pending_final_id(graduation_client, h, name)
    graduation_client.post(f"{FINAL}/{fid}/review", headers=h, json={"action": "APPROVE"})

    # 初稿通过后可交定稿
    fin = graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "定稿", "attachments": [_upload_pdf(graduation_client, sh)]})
    assert fin.json()["code"] == 0
    assert fin.json()["data"]["finalType"] == "定稿"


def test_student_supplied_plagiarism_rate_is_ignored(graduation_client, auth_headers, db_mode):
    h = auth_headers
    name = "查重小红"
    gid = _gd_student_with_topic(graduation_client, h, "FN101", name)
    sh = _stu_token(name)
    _to_guiding(graduation_client, h, sh, gid, name)

    # A student cannot forge an authoritative plagiarism result in the request body.
    graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "plagiarismRate": "35%", "attachments": [_upload_pdf(graduation_client, sh)]})
    fid = _pending_final_id(graduation_client, h, name)
    row = next(
        item for item in graduation_client.get(FINAL, headers=h).json()["data"]["items"] if item["id"] == fid
    )
...
def test_reject_then_resubmit_new_version(graduation_client, auth_headers, db_mode):
    h = auth_headers
    name = "重交小刚"
    gid = _gd_student_with_topic(graduation_client, h, "FN201", name)
    sh = _stu_token(name)
    _to_guiding(graduation_client, h, sh, gid, name)

    graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "attachments": [_upload_pdf(graduation_client, sh)]})
    fid = _pending_final_id(graduation_client, h, name)
    graduation_client.post(f"{FINAL}/{fid}/review", headers=h, json={"action": "REJECT", "comment": "格式需按模板调整"})

    resub = graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "attachments": [_upload_pdf(graduation_client, sh)]})
...
def test_not_submitted_derivation_and_remind(graduation_client, auth_headers, db_mode):
    h = auth_headers
    name = "未交小李"
    gid = _gd_student_with_topic(graduation_client, h, "FN301", name)
    sh = _stu_token(name)
    _to_guiding(graduation_client, h, sh, gid, name)  # GUIDING 且无成果

    nb = graduation_client.get(FINAL, headers=h, params={"status": "NOT_SUBMITTED"}).json()["data"]
    target = next(r for r in nb["items"] if r["studentName"] == name)
    assert target["status"] == "NOT_SUBMITTED"

    remind = graduation_client.post(f"{FINAL}/remind", headers=h, json={"gdStudentId": target["gdStudentId"]})
    assert remind.json()["code"] == 0

    graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "attachments": [_upload_pdf(graduation_client, sh)]})
    remind2 = graduation_client.post(f"{FINAL}/remind", headers=h, json={"gdStudentId": target["gdStudentId"]})
    assert remind2.json()["code"] != 0  # 已提交不可催


def test_export_finals_ledger(graduation_client, auth_headers, db_mode):
    h = auth_headers
    name = "导出小周"
    gid = _gd_student_with_topic(graduation_client, h, "FN401", name)
    sh = _stu_token(name)
    _to_guiding(graduation_client, h, sh, gid, name)
    graduation_client.post(f"{MOBILE}/graduation/final", headers=sh, json={"finalType": "初稿", "attachments": [_upload_pdf(graduation_client, sh)]})

    exp = graduation_client.post(f"{FINAL}/export", headers=h)
    body = exp.json()
    assert body["code"] == 0

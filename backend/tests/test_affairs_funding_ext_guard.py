...
def test_extension_dtos_return_version_and_backend_actions():
    text = source("backend/app/services/affairs_funding_ext_guard.py")
    assert text.count('data["version"] = int(row.version or 0)') >= 4
    assert '"APPLIED": ["APPROVE", "REJECT"]' in text
    assert '"APPROVED": ["ONBOARD", "TERMINATE"]' in text
    assert 'row.status in service._LOAN_NEXT' in text
...
def test_monthly_and_money_inputs_fail_before_mysql():
    ...
    assert "service._scope_or_403(db, record.student_id, user)" in text

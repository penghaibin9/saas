...
def test_graduation_audit_logs_list(graduation_client, auth_headers, db_mode):
    r = graduation_client.get("/api/v1/graduation/audit-logs", headers=auth_headers, params={"page": 1, "pageSize": 10}).json()
    assert r["code"] == 0
    data = r["data"]
...
def test_mobile_peer_tasks_requires_student(graduation_client, auth_headers, db_mode):
    r = graduation_client.get("/api/v1/mobile/graduation/peer-tasks", headers=auth_headers).json()

...
def test_risk_scan_new_codes(graduation_client, auth_headers, db_mode):
    ...
    # 学生 A：选题后无任务书 → GD-R03
    name_a = "风险补齐生A"
    sid_a = graduation_client.post(STU, headers=h, json={"studentNo": "R9A", "realName": name_a, "classId": make_org_class()}).json()["data"]["id"]
    gid_a = graduation_client.post(GD_STU, headers=h, json={"studentId": sid_a, "batchId": bid}).json()["data"]["id"]
    tid_a = graduation_client.post(GD_TOPIC, headers=h, json={"title": f"{name_a}题", "sourceType": "TEACHER",
                       "advisorName": "李老师", "capacity": 1, "submitReview": True, "batchId": bid}).json()["data"]["id"]
    graduation_client.post(f"{GD_TOPIC}/{tid_a}/review", headers=h, json={"action": "APPROVE"})
    graduation_client.post(f"{GD_STU}/{gid_a}/assign-topic", headers=h, json={"topicId": tid_a})

    # 学生 B：选题 + 导师 + 任务书确认 + 开题驳回 → GD-R05
    name_b = "风险补齐生B"
    sno_b = "R9B"
    sid_b = graduation_client.post(STU, headers=h, json={"studentNo": sno_b, "realName": name_b, "classId": make_org_class()}).json()["data"]["id"]
    gid_b = graduation_client.post(GD_STU, headers=h, json={"studentId": sid_b, "batchId": bid}).json()["data"]["id"]
    tid_b = graduation_client.post(GD_TOPIC, headers=h, json={"title": f"{name_b}题", "sourceType": "TEACHER",
                       "advisorName": "李老师", "capacity": 1, "submitReview": True, "batchId": bid}).json()["data"]["id"]
    graduation_client.post(f"{GD_TOPIC}/{tid_b}/review", headers=h, json={"action": "APPROVE"})
    graduation_client.post(f"{GD_STU}/{gid_b}/assign-topic", headers=h, json={"topicId": tid_b})
    mid = graduation_client.post(GD_MENTOR, headers=h, json={"teacherNo": "R9M1", "teacherName": "导师R9"}).json()["data"]["id"]
    graduation_client.post(f"{GD_MENTOR}/{mid}/review", headers=h, json={"action": "APPROVE"})
    graduation_client.post(f"{GD_ASSIGN}/assign", headers=h, json={"gdStudentId": gid_b, "mentorId": mid})
    issue = graduation_client.post(f"{GD_TASKBOOK}/{gid_b}/issue", headers=h, params={"batchId": bid},
    ...
    confirm = graduation_client.post(f"{GD_TASKBOOK}/{gid_b}/confirm", headers=h, params={"batchId": bid},
                          json={"proxyReason": "代确认任务书用于风险扫描回归"})
    assert confirm.json()["code"] == 0, confirm.json()
    sh = _stu_token(name_b, sno_b)
    sub = graduation_client.post(f"{MOBILE}/graduation/proposal", headers=sh,
                      json={"background": "背景足够长用于开题", "plan": "方案足够长用于开题"})
    assert sub.json()["code"] == 0, sub.json()
    pid = next(r for r in graduation_client.get(PROP, headers=h, params={
        "status": "PENDING_REVIEW", "batchId": bid}).json()["data"]["items"]
               if r["studentName"] == name_b)["id"]
    graduation_client.post(f"{PROP}/{pid}/review", headers=h, params={"batchId": bid},
                json={"action": "REJECT", "comment": "方案需细化再交"})

    scan = graduation_client.post(f"{RISK}/scan", headers=h, params={"batchId": bid})
    assert scan.json()["code"] == 0, scan.json()

    rows = graduation_client.get(RISK, headers=h, params={"page": 1, "pageSize": 200, "batchId": bid}).json()["data"]["items"]
...
def test_batch_archive_operations(graduation_client, auth_headers, db_mode):
    h = auth_headers
    bid = graduation_client.post("/api/v1/graduation/batches", headers=h, json={
    ...
    for i in range(2):
        sid = graduation_client.post(STU, headers=h, json={"studentNo": f"R9B{i}", "realName": f"归档生{i}", "classId": make_org_class()}).json()["data"]["id"]
        graduation_client.post(GD_STU, headers=h, json={"studentId": sid, "batchId": bid})

    prev = graduation_client.post(f"{ARCH}/batch-generate/preview", headers=h, params={"batchId": bid}).json()
    assert prev["code"] == 0, prev
    gen = graduation_client.post(f"{ARCH}/batch-generate", headers=h, params={"batchId": bid},
                      json={"previewToken": prev["data"]["previewToken"]})
    assert gen.json()["code"] == 0, gen.json()
    assert gen.json()["data"]["skipped"] >= 2  # 缺材料被跳过

    # 无已提交记录 → 一键核验备案返回 0
    prev_f = graduation_client.post(f"{ARCH}/batch-file/preview", headers=h, params={"batchId": bid}).json()
    assert prev_f["code"] == 0, prev_f
    fil = graduation_client.post(f"{ARCH}/batch-file", headers=h, params={"batchId": bid},

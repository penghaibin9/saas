...
class CommerceRouterTests(unittest.TestCase):
    ...
    @staticmethod
    def signatures(routes):
        return [(method, route.path) for route in routes
        ...
        for route in declared:
            for method in route.methods:
                with self.subTest(method=method, path=route.path):
                    matches = [r for r in self.registered.routes
    ...
    def test_frozen_surface_is_preserved_without_duplicate_signatures(self):
        final = self.signatures(self.canonical.router.routes)
        self.assertEqual(len(final), len(set(final)))
        self.assertTrue(set(self.signatures(self.bundle.router.routes)).issubset(final))

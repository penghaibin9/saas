...
def _mentor(graduation_client, h, teacher_no, teacher_name="张伟"):
...
def test_defense_group_writes_mentor_ids(graduation_client, auth_headers, db_mode):
    h = auth_headers
    bid = _batch(graduation_client, h)
    chair_id = _mentor(graduation_client, h, _uniq("T-CH"), "组长甲")
    sec_id = _mentor(graduation_client, h, _uniq("T-SE"), "秘书甲")
    m1 = _mentor(graduation_client, h, _uniq("T-M1"), "评委甲")
    m2 = _mentor(graduation_client, h, _uniq("T-M2"), "评委乙")
    r = graduation_client.post(DG, headers=h, params={"batchId": bid}, json={
...
def test_same_name_defense_expert_isolated_by_mentor_id(graduation_client, auth_headers, db_mode):
    """同名张伟 A/B：仅组内 mentor_id 对应工号可访问学生。"""
    h = auth_headers
    bid = _batch(graduation_client, h)
    no_a, no_b = _uniq("TA"), _uniq("TB")
    mid_a = _mentor(graduation_client, h, no_a, "张伟")
    mid_b = _mentor(graduation_client, h, no_b, "张伟")
    ...
    sid = graduation_client.post(STU, headers=h, json={"studentNo": _uniq("S"), "realName": "生同名", "classId": make_org_class()}).json()["data"]["id"]
...
def test_sod_review_uses_mentor_id_not_same_name(graduation_client, auth_headers, db_mode):
    """A 为指导老师时：同名 B 可评阅；A 本人不可评阅。"""
    h = auth_headers
    bid = _batch(graduation_client, h)
    no_a, no_b = _uniq("RA"), _uniq("RB")
    mid_a = _mentor(graduation_client, h, no_a, "李华")
    mid_b = _mentor(graduation_client, h, no_b, "李华")
    sid = graduation_client.post(STU, headers=h, json={"studentNo": _uniq("S"), "realName": "生SoD", "classId": make_org_class()}).json()["data"]["id"]
    gid = graduation_client.post(GD_STU, headers=h, json={"studentId": sid, "batchId": bid}).json()["data"]["id"]
    # 绑定指导老师 A
    assign = graduation_client.post("/api/v1/graduation/gd-mentor-assignments/assign", headers=h, json={
    ...
    blocked = graduation_client.post(f"{GD_REVIEW}/assign", headers=h, json={
    ...
    ok = graduation_client.post(f"{GD_REVIEW}/assign", headers=h, json={

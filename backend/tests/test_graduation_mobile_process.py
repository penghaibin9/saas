...
def test_student_taskbook_midterm_grade_and_teacher_guidance(graduation_client, auth_headers, db_mode):
    h = auth_headers
    ...
    mid = graduation_client.post(GD_MENTOR, headers=h, json={"teacherNo": "MBT1", "teacherName": "移动导师"}).json()["data"]["id"]
    ...
    graduation_client.post(f"{GD_TASKBOOK}/{gid}/issue", headers=h, json={"objective": "完成移动端联调", "content": "完成移动端接口联调测试"})

    sh = _stu_token(student_name)
    tb = graduation_client.get(f"{MOBILE}/graduation/taskbook", headers=sh).json()["data"]
    assert tb["hasData"] is True
    assert tb["status"] == "PENDING_CONFIRM"

    confirm = graduation_client.post(f"{MOBILE}/graduation/taskbook/confirm", headers=sh).json()["data"]
    assert confirm["status"] == "CONFIRMED"

    mt = graduation_client.get(f"{MOBILE}/graduation/midterm", headers=sh).json()["data"]
    assert mt["hasData"] is True

    grade_before = graduation_client.get(f"{MOBILE}/graduation/grade", headers=sh).json()["data"]
    assert grade_before["published"] is False

    th = _teacher_token("移动导师")
    my_students = _items(graduation_client.get(f"{MOBILE}/teacher/graduation/my-students", headers=th).json()["data"])
    assert any(s["gdStudentId"] == str(gid) for s in my_students)

    add_guidance = graduation_client.post(f"{MOBILE}/teacher/graduation/{gid}/guidance", headers=th,
                               json={"content": "移动端指导记录测试", "method": "ONLINE"})
    assert add_guidance.json()["code"] == 0

    wrong_teacher = _teacher_token("不是导师")
    denied = graduation_client.post(f"{MOBILE}/teacher/graduation/{gid}/guidance", headers=wrong_teacher,
                        json={"content": "越权测试"})

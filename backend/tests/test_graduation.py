...
def test_students_and_detail(graduation_client, auth_headers, db_mode):
    ids = _seed(db_mode)
    lst = graduation_client.get("/api/v1/graduation/students", headers=auth_headers,
                     params={"batchId": ids["batch"]}).json()
    assert lst["code"] == 0 and lst["data"]["total"] == 1
    assert lst["data"]["items"][0]["stageLabel"] == "成果检查"
    det = graduation_client.get(f"/api/v1/graduation/students/{ids['student']}", headers=auth_headers,
                     params={"batchId": ids["batch"]}).json()
    assert det["code"] == 0 and len(det["data"]["proposals"]) == 1 and len(det["data"]["finals"]) == 1


def test_topics(graduation_client, auth_headers, db_mode):
    _seed(db_mode)
    lst = graduation_client.get("/api/v1/graduation/topics", headers=auth_headers).json()
    assert lst["code"] == 0 and lst["data"]["total"] == 1
...
def test_proposal_review_closed_loop(graduation_client, auth_headers, db_mode):
    ids = _seed(db_mode, stage="TASKBOOK_CONFIRM", taskbook=True)
    # 驳回需原因≥5字
    bad = graduation_client.post(f"/api/v1/graduation/proposals/{ids['proposal']}/review", headers=auth_headers,
                      params={"batchId": ids["batch"]},
                      json={"action": "REJECT", "comment": "x"}).json()
    assert bad["code"] == 422001
    ok = graduation_client.post(f"/api/v1/graduation/proposals/{ids['proposal']}/review", headers=auth_headers,
                     params={"batchId": ids["batch"]},
    ...
    # 重复批阅 409
    dup = graduation_client.post(f"/api/v1/graduation/proposals/{ids['proposal']}/review", headers=auth_headers,
                      params={"batchId": ids["batch"]},
                      json={"action": "APPROVE"}).json()
    assert dup["code"] == 409001


def test_final_review(graduation_client, auth_headers, db_mode):
    ids = _seed(db_mode)
    ok = graduation_client.post(f"/api/v1/graduation/finals/{ids['final']}/review", headers=auth_headers,
                     params={"batchId": ids["batch"]},
                     json={"action": "APPROVE"}).json()
    assert ok["code"] == 0 and ok["data"]["status"] == "APPROVED"


def test_defense_publish_conflict(graduation_client, auth_headers, db_mode):
    ...
    graduation_client.post(f"/api/v1/graduation/defense-groups/{ids['gbad']}/assign", headers=h,
                params={"batchId": ids["batch"]},
                json={"studentIds": [str(ids["student"])]})
    bad = graduation_client.post(f"/api/v1/graduation/defense-groups/{ids['gbad']}/publish", headers=h,
                      params={"batchId": ids["batch"]}).json()
    assert bad["code"] == 422001 and "冲突" in bad["message"]
    # gok：分入无导师冲突的学生（advisor 不在 gok 名单）→ 完整组可发布
    sid = graduation_client.post("/api/v1/students", headers=h,
    ...
    try:
        ...
        db.close()
    graduation_client.post(f"/api/v1/graduation/defense-groups/{ids['gok']}/assign", headers=h,
                params={"batchId": ids["batch"]},
                json={"studentIds": [str(g2)]})
    ok = graduation_client.post(f"/api/v1/graduation/defense-groups/{ids['gok']}/publish", headers=h,
                     params={"batchId": ids["batch"]}).json()
    assert ok["code"] == 0 and ok["data"]["published"] is True
    lst = graduation_client.get("/api/v1/graduation/defense-groups", headers=h,
                     params={"batchId": ids["batch"]}).json()
    pub = [g for g in lst["data"]["items"] if g["id"] == str(ids["gok"])][0]
    assert pub["published"] is True


def test_dashboard_and_audit(graduation_client, auth_headers, db_mode):
    ids = _seed(db_mode)
    dash = graduation_client.get("/api/v1/graduation/dashboard", headers=auth_headers,
                      params={"batchId": ids["batch"]}).json()
    assert dash["code"] == 0 and any(s["label"] == "毕设学生" for s in dash["data"]["stats"])
    graduation_client.post(f"/api/v1/graduation/proposals/{ids['proposal']}/review", headers=auth_headers,
                params={"batchId": ids["batch"]},
                json={"action": "APPROVE"})
    au = graduation_client.get("/api/v1/graduation/audit-logs", headers=auth_headers,
                    params={"bizType": "PROPOSAL", "batchId": ids["batch"]}).json()
    assert au["code"] == 0 and au["data"]["total"] >= 1


def test_requires_login(graduation_client):
    assert graduation_client.get("/api/v1/graduation/dashboard").json()["code"] == 401001

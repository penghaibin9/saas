...
def test_credit_appeal_approve_creates_credit(client, db_mode):
    ...
    r = post_versioned(f"{BASE}/second-class/appeals/{aid}/review", headers=hdr, json={"action": "APPROVE"}).json()
    ...
    # 已审核不可再核
    assert post_versioned(f"{BASE}/second-class/appeals/{aid}/review", headers=hdr,
...
def test_credit_appeal_reject(client, db_mode):
    ...
    # 正常驳回
    r = post_versioned(f"{BASE}/second-class/appeals/{aid}/review", headers=hdr,

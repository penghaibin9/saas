...
def _seed():
    ...
    try:
        ...
        campaign = InternshipRecruitmentCampaign(
            tenant_id=TID,
            batch_id=800001,
            campaign_code="RACE-A01-10",
            campaign_name="A01并发选岗",
            round_no=1,
            status="OPEN",
        ...
        for idx in range(1, 5):
            row = InternshipPosition(
                tenant_id=TID,
                company_id=700000 + idx,
                company_name=f"并发企业{idx}",
                batch_id=800001,
        ...
        db.flush()
        ids = (record.id, campaign.id, [p.id for p in positions])
        db.commit()
        return ids
    finally:
        db.close()
...
def test_mysql_two_stale_writers_single_winner_no_duplicate_or_half_slots(db_mode):
    ...
    def _run(plan):
        ...
        try:
            ...
            rows = [
                {"volunteerNo": i + 1, "positionId": pid, "applicationStatement": f"志愿{i + 1}"}
                for i, pid in enumerate(plan)
            ]
            group, apps, _record = svc.save_or_submit_in_tx(
                db,
                tenant_id=TID,
                student_id=900001,
                record_id=record_id,
            ...
            db.commit()
            with lock:
                results.append(("ok", [a.position_id for a in apps], group.version))
        except AppException as exc:
            db.rollback()
            with lock:
                results.append(("err", exc.http_status, exc.code))
        except Exception as exc:  # noqa: BLE001
            db.rollback()
            with lock:
                results.append(("boom", repr(exc)))
        finally:
            db.close()
            set_tenant(None)

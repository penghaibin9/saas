...
V2-04 正式合同：目标课程必须选择课程库具体版本，稳定身份使用 course_code/version；
学生自助提交→教务审核→写 t_acad_grade(source=RECOGNIZED) 并刷新台账；原成绩<60、
同目标版本重复、目标课程已通过、无课程版本/freetext、无效附件和越权审核均 fail-closed。
"""
...
def _seed(db_mode):
    ...
    from app.models import AaCourse, AcademicGrade, AcademicStudent, College, Major, SchoolClass, StudentProfile
    db = get_sessionmaker()()
    ...
    target = AaCourse(tenant_id=TID, course_code="TGT001", course_name="高等数学", credit=4,
                      nature="REQUIRED", status="ENABLED")
    english = AaCourse(tenant_id=TID, course_code="ENG001", course_name="大学英语", credit=3,
                       nature="REQUIRED", status="ENABLED")
    ...
    db.add(AcademicGrade(
        ...
        attempt_no=1, course_name=english.course_name, nature=english.nature or "REQUIRED",
        score=80, credit_value=3, pass_status="PASSED", source="PUBLISH", record_status="ACTIVE",
    ))
    db.commit()
    ids = {"profile": p.id, "acad": a.id, "target": target.id, "english": english.id}
    db.close()
    return ids
...
def _body(ids, **overrides):
    body = {
        "sourceCourseName": "高等数学A", "sourceScore": 82, "sourceCredit": 4,
        "sourceOrigin": "原电子信息专业", "targetCourseId": str(ids["target"]),
        "targetCourseName": "仅兼容展示，不作为正式身份", "reason": "转专业课程替代",
    }
...
def test_full_chain_recognized_grade_and_aggregates(client, db_mode):
    ...
    r = client.post(f"{BASE}/grade-recognitions/student/submit", headers=stu, json=_body(ids))
    ...
    assert g.course_id == ids["target"] and g.course_code == "TGT001" and int(g.course_version) == 1
    assert g.course_name == "高等数学" and g.score == 82 and g.pass_status == "PASSED"
...
def test_fail_score_rejected(client, db_mode):
    ids = _seed(db_mode)
    stu = _stu_token("认甲", "RN2401")
    r = client.post(f"{BASE}/grade-recognitions/student/submit", headers=stu,
                    json=_body(ids, sourceScore=55))
...
def test_duplicate_pending_409(client, db_mode):
    ...
    stu = _stu_token("认甲", "RN2401")
    body = _body(ids)
    assert client.post(f"{BASE}/grade-recognitions/student/submit", headers=stu, json=body).status_code == 200
...
def test_target_already_passed_409(client, db_mode):
    ...
    stu = _stu_token("认甲", "RN2401")
    r = client.post(f"{BASE}/grade-recognitions/student/submit", headers=stu,
                    json=_body(ids, targetCourseId=str(ids["english"])))
    assert r.status_code == 409, r.text
...
def test_reject_requires_reason_and_student_forbidden(client, db_mode):
    ...
    stu = _stu_token("认甲", "RN2401")
    admin = _hdr(client, "school_admin01")
    submitted = client.post(f"{BASE}/grade-recognitions/student/submit", headers=stu, json=_body(ids))
    assert submitted.status_code == 200, submitted.text
    rid = submitted.json()["data"]["recognitionId"]

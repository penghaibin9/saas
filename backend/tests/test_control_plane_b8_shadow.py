...
def test_active_tenant_universe_never_contains_platform_permissions():
...
def test_b8_convergence_is_idempotent_and_never_rewrites_published_version(db_mode):
    first = shadow.converge_published_system_templates(
        actor_user_id=9802,
        source_commit_sha="b8-idempotent-head",
    )
    second = shadow.converge_published_system_templates(
        actor_user_id=9802,
        source_commit_sha="b8-idempotent-head",
    )
...
def test_custom_resolver_uses_role_permission_only_and_never_falls_back_to_template(db_mode):
    ...
    try:
        role = Role(
            tenant_id=tenant_id,
            role_code="B8_CUSTOM_DIRECT_ONLY",
            role_name="B8 Custom",
            role_type="CUSTOM",
            status="ACTIVE",
        )
        ...
        assert shadow.custom_role_allows(
            db, tenant_id=tenant_id, role_id=role.id,
            permission_code="internship.recruitment.view",
        ) is False

        permission = db.scalars(select(Permission).where(
            Permission.permission_code == "internship.recruitment.view"
        )).first()
        if permission is None:
            permission = Permission(
                permission_code="internship.recruitment.view",
                permission_name="B8 view",
...
def test_b6_product_iam_snapshot_reads_published_normalized_template_truth(db_mode):
    ...
    shadow.converge_published_system_templates(
        actor_user_id=9803,
        source_commit_sha="b8-product-iam-head",
    )

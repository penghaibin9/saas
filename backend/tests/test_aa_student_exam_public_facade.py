...
def test_public_mobile_exam_facade_uses_safe_read_service(monkeypatch):
    ...
    from app.modules.academic_affairs.services import student_exam_read_service as safe_exam

    seen = []
    monkeypatch.setattr(safe_exam, "exam_my", lambda user: seen.append(("exam", user)) or {"items": []})
    monkeypatch.setattr(
        safe_exam,
        "deferrable_courses",
    ...
    # 对外 canonical 始终是 public_service；它再委托 facade，facade 必须进入 safe read service。

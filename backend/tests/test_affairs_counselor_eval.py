...
def test_counselor_eval_full_flow(client, db_mode):
    ...
    # 发布
    p = post_versioned(f"{BASE}/counselor-eval/evals/{eid}/publish", headers=hdr).json()
    ...
    # 申诉
    a = post_versioned(f"{BASE}/counselor-eval/evals/{eid}/appeal", headers=hdr,
    ...
    # 无待复核申诉再复核 → 冲突
    assert post_versioned(f"{BASE}/counselor-eval/evals/{eid}/appeal-review", headers=hdr,

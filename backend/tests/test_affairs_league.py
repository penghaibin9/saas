...
def test_league_dev_terminate_and_validation(client, db_mode):
    ...
    # 终止原因过短 → 拒绝
    assert client.post(f"{BASE}/party-league/dev/{did}/terminate", headers=hdr,
                       json={"reason": "短"}).json()["code"] != 0
    # 正常终止
    t = client.post(f"{BASE}/party-league/dev/{did}/terminate", headers=hdr,
    ...
    r = client.post(f"{BASE}/party-league/dev/{did2}/advance", headers=hdr, json={"toStage": "XXX"})

...
def test_archive_is_real_scoped_and_not_placeholder_success():
    ...
    assert 'context = build_affairs_context(user, db)' in archive
    assert 'allowed_classes = context.allowed_class_ids(db)' in archive
...
def test_second_class_uses_append_only_difference_adjustments():
    text = read("backend/app/services/affairs_activity_service.py")
...
def test_activity_and_volunteer_dtos_return_actions_and_scope():
    ...
    assert 'build_affairs_context(user, db).require_student(db, student_id)' in text
    assert 'reliability._teacher_scope_tokens(db, user)' in text
    assert 'reliability._activity_matches(activity, class_tokens, college_tokens)' in text

...
def test_archive_grade_policy_guard_is_installed():
    """归档策略必须由显式编排安装，禁止再靠 import-order monkey patch。"""
    guard_source = (
    ...
    assert "return _policy.apply_effective_grade_policy_debt" in guard_source
...
def test_policy_debt_changes_grade_archive_result_to_blocked():
    """唯一权威 domain policy 仍必须把策略欠账 fail-closed 为 BLOCKED。"""
    source = (
        ROOT / "backend/app/modules/academic_affairs/services/academic_affairs_archive_domain_policy.py"
    ).read_text(encoding="utf-8")

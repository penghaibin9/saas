...
def test_w74_pc_locks_task_context_for_entire_canonical_mutation():
    ...
    # Verify the queue control semantically instead of requiring Vue attributes to share one line.
    queue_button = workspace.split('v-for="(item, index) in queue"', 1)[1].split("</button>", 1)[0]
    assert ':class="{ \'is-active\': index === currentIndex }"' in queue_button
    assert ':disabled="submitting"' in queue_button
    ...
    auto_toggle = workspace.split('class="gd-review-workspace__auto"', 1)[1].split("</label>", 1)[0]
    assert ':checked="autoNext"' in auto_toggle
    assert ':disabled="submitting"' in auto_toggle

    dossier_button = workspace.split('class="gd-review-workspace__dossier"', 1)[1].split("</button>", 1)[0]
    assert ':disabled="submitting"' in dossier_button

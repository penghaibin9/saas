...
def _submit_proposal(client, sh, background="选题背景说明：面向职校学生的实训管理系统",
                     plan="研究方案：需求分析→设计→实现→测试，共 12 周", outcome="预期成果：可运行系统 + 论文"):
    return client.post(f"{MOBILE}/graduation/proposal", headers=sh,
                       json={"background": background, "plan": plan, "outcome": outcome})


def test_student_submit_and_no_duplicate(client, auth_headers, db_mode):
    h = auth_headers
    name = "开题小明"
    _gd_student_with_topic(client, h, "PR001", name)
    sh = _stu_token(name)

    # 提交前：可提交、无最新版
    view0 = client.get(f"{MOBILE}/graduation/proposal", headers=sh).json()["data"]
    assert view0["hasData"] is True
    assert view0["canSubmit"] is True
    assert view0["latest"] is None

    ok = _submit_proposal(client, sh)
    assert ok.json()["code"] == 0
    assert ok.json()["data"]["status"] == "PENDING_REVIEW"
    assert ok.json()["data"]["version"] == "v1"

    # 待审阅时不可重复提交
    dup = _submit_proposal(client, sh)
    assert dup.json()["code"] != 0

    view1 = client.get(f"{MOBILE}/graduation/proposal", headers=sh).json()["data"]
    assert view1["canSubmit"] is False
    assert view1["latest"]["status"] == "PENDING_REVIEW"


def test_review_approve_advances_stage_and_detail_versions(client, auth_headers, db_mode):
    h = auth_headers
    name = "开题小红"
    gid = _gd_student_with_topic(client, h, "PR101", name)
    sh = _stu_token(name)
    _submit_proposal(client, sh)
    ...
    # 驳回需理由≥5
    bad = client.post(f"{PROP}/{pid}/review", headers=h, json={"action": "REJECT", "comment": "x"})
    assert bad.json()["code"] != 0

    ok = client.post(f"{PROP}/{pid}/review", headers=h, json={"action": "APPROVE", "comment": "选题合理"})
    assert ok.json()["code"] == 0
    ...
    # 已批阅不可再批
    again = client.post(f"{PROP}/{pid}/review", headers=h, json={"action": "APPROVE"})
...
def test_reject_then_resubmit_new_version(client, auth_headers, db_mode):
    h = auth_headers
    name = "开题小刚"
    _gd_student_with_topic(client, h, "PR201", name)
    sh = _stu_token(name)
    _submit_proposal(client, sh)

    lst = client.get(PROP, headers=h, params={"status": "PENDING_REVIEW"}).json()["data"]
    pid = next(r for r in lst["items"] if r["studentName"] == name)["id"]
    client.post(f"{PROP}/{pid}/review", headers=h, json={"action": "REJECT", "comment": "研究方案需细化进度安排"})

    # 被驳回后可重交，版本 v2 + 重交标记
    view = client.get(f"{MOBILE}/graduation/proposal", headers=sh).json()["data"]
    assert view["canSubmit"] is True
    assert view["latest"]["status"] == "REJECTED"

    resub = _submit_proposal(client, sh, background="修订后的选题背景说明：补充可行性论证",
                             plan="修订研究方案：细化到每周里程碑")
    assert resub.json()["code"] == 0
    assert resub.json()["data"]["version"] == "v2"
    assert resub.json()["data"]["isResubmit"] is True

    lst2 = client.get(PROP, headers=h, params={"status": "PENDING_REVIEW"}).json()["data"]
    pid2 = next(r for r in lst2["items"] if r["studentName"] == name)["id"]
    detail = client.get(f"{PROP}/{pid2}", headers=h).json()["data"]
...
def test_not_submitted_derivation_and_remind(client, auth_headers, db_mode):
    h = auth_headers
    name = "未交小李"
    _gd_student_with_topic(client, h, "PR301", name)  # 已确认选题但未提交开题

    # 未提交页签应派生出该生
    nb = client.get(PROP, headers=h, params={"status": "NOT_SUBMITTED"}).json()["data"]
    target = next(r for r in nb["items"] if r["studentName"] == name)
    assert target["status"] == "NOT_SUBMITTED"

    # 催交成功（真实写审计，非 mock）
    remind = client.post(f"{PROP}/remind", headers=h, json={"gdStudentId": target["gdStudentId"]})
    ...
    sh = _stu_token(name)
    _submit_proposal(client, sh)
    remind2 = client.post(f"{PROP}/remind", headers=h, json={"gdStudentId": target["gdStudentId"]})
    assert remind2.json()["code"] != 0


def test_export_proposals_ledger(client, auth_headers, db_mode):
    h = auth_headers
    name = "导出小周"
    _gd_student_with_topic(client, h, "PR401", name)
    sh = _stu_token(name)
    _submit_proposal(client, sh)

    exp = client.post(f"{PROP}/export", headers=h)
    body = exp.json()
    assert body["code"] == 0

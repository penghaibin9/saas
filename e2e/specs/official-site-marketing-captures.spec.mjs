...
test.describe.serial('Official site marketing captures · real isolated school admin', () => {
  test('capture six platform and implementation surfaces from the real system', async ({ page }) => {
    ...
    expect(fs.readdirSync(captureDir).filter((name) => name.endsWith('.png'))).toHaveLength(6)
  })
})

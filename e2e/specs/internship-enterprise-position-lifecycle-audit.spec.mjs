...
test.describe('岗位实习审计：IX-003 企业生命周期 + IX-005 岗位生命周期', () => {
  ...
  test('IX-003：学校真实创建企业、审核通过、暂停并恢复，列表电话保持脱敏', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '暂停合作', exact: true }).click()
    dialog = page.getByRole('dialog')
    await dialog.getByRole('button', { name: '确认暂停', exact: true }).click()
    await expect(page.getByRole('button', { name: '恢复合作', exact: true })).toBeVisible()
    await page.getByRole('button', { name: /返回上一页|返回企业库/ }).click()
    row = companyRow(page, companyName())
    await expect(row).toContainText('已暂停')

    await row.getByRole('link', { name: '合作与资质', exact: true }).click()
    await page.getByRole('button', { name: '恢复合作', exact: true }).click()
    dialog = page.getByRole('dialog')
  ...
  test('IX-003：学校真实邀请企业加入当前招聘季，企业门户真实接受邀请', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: fixture.campaignName })).toBeVisible()

...
// Runs in the existing exact-head School IAM MySQL job; no fixtures intercept these requests.
test('approved system UI: create role → permissions → additive members → durable readback', async ({ page }, testInfo) => {
  const login = new StaffLoginPage(page, config.staffBaseUrl)
  await login.login({ tenant: FIXTURE.iamTenantCode, username: FIXTURE.iamAdminLogin, password: '123456' })
  const token = await login.token()
  expect(token).toBeTruthy()
  const beforeUser = await browserApi(page, token, 'GET', `/system/users/${FIXTURE.iamTargetUserId}`)
  const existingRoles = (beforeUser.roles || []).map(row => row.code).sort()
  const suffix = `${String(process.env.GITHUB_RUN_ID || Date.now()).replace(/\D/g, '').slice(-10)}_${testInfo.retry}`
  const name = `系统界面验收角色${suffix}`
  const roleCode = `UI_ROLE_${suffix}`
  await page.goto(`${config.staffBaseUrl}/admin/system/iam?surface=roles`)
  await page.getByTestId('A012-open').click()
  await page.getByLabel('角色名称', { exact: true }).fill(name)
  await page.getByLabel('角色编码', { exact: true }).fill(roleCode)
  await page.getByLabel('已发布来源模板', { exact: true }).selectOption('SCHOOL_ADMIN')
  const createResponse = page.waitForResponse(r => r.request().method() === 'POST' && new URL(r.url()).pathname.endsWith('/system/roles'))
  await page.getByTestId('A012-save').click()
  const createdJson = await (await createResponse).json()
  expect(createdJson.code).toBe(0)
  const roleId = String(createdJson.data.id)
  await expect(page.getByTestId('role-workbench')).toBeVisible()
  ...
  expect(Object.keys(sent).sort()).toEqual(['expectedVersion', 'permissionCodes', 'reason', 'requestId', 'scopeCode'].sort())
  expect(sent.expectedVersion).toBe(original.version)
  if (addingPermission) expect(sent.permissionCodes).toContain(code)
  else expect(sent.permissionCodes).not.toContain(code)
  expect(sent).not.toHaveProperty('scopeTarget')
  expect(sent).not.toHaveProperty('tenantId')
  expect((await response.json()).code).toBe(0)
  await expect(page.getByTestId('permission-receipt')).toBeVisible()
  await expect(page.getByTestId('A017-review')).toBeDisabled()
  const saved = await browserApi(page, token, 'GET', `/system/roles/${roleId}`)
  if (addingPermission) expect(saved.permissionCodes).toContain(code)
  else expect(saved.permissionCodes).not.toContain(code)
  expect(saved.version).toBeGreaterThan(original.version)
  const evidence = await browserApi(page, token, 'GET', `/system/roles/${roleId}/audit?page=1&pageSize=50`)
  expect(evidence.total).toBeGreaterThan(0)
  await page.getByRole('tab', { name: /^成员/ }).click()
  await page.getByTestId('A022-open').click()
  await page.getByLabel('搜索候选老师').fill(FIXTURE.iamTargetLogin)
  await page.getByTestId('role-members-panel').getByRole('button', { name: '查询', exact: true }).click()
  const candidate = page.getByTestId('role-members-panel').locator('tbody tr').filter({ hasText: FIXTURE.iamTargetLogin })
  await expect(candidate).toHaveCount(1)
  await candidate.getByRole('checkbox').check()
  await page.getByLabel('成员授权原因').fill('真实浏览器验证追加成员不覆盖既有角色')
  const memberResponse = page.waitForResponse(r => r.request().method() === 'POST' && new URL(r.url()).pathname.endsWith(`/system/roles/${roleId}/members/batch`))
  await page.getByTestId('A022-submit').click()
  expect((await (await memberResponse).json()).code).toBe(0)
  await expect(page.getByTestId('member-total')).toContainText('共 1 位成员')
  const members = await browserApi(page, token, 'GET', `/system/roles/${roleId}/members?page=1&pageSize=50`)
  expect(members.items.map(row => String(row.id))).toContain(String(FIXTURE.iamTargetUserId))
  const afterUser = await browserApi(page, token, 'GET', `/system/users/${FIXTURE.iamTargetUserId}`)
  const codes = (afterUser.roles || []).map(row => row.code)
  for (const existing of existingRoles) expect(codes).toContain(existing)
  expect(codes).toContain(roleCode)
  await page.getByRole('tab', { name: '菜单与操作', exact: true }).click()
  await page.reload()
  if (addingPermission) await expect(page.locator(`[data-permission="${code}"] input`)).toBeChecked()
  else await expect(page.locator(`[data-permission="${code}"] input`)).not.toBeChecked()
  await expect(page.locator('.sw-worktitle')).toContainText(name)
  await expect(page.getByTestId('A017-review')).toBeDisabled()
  const screenshot = testInfo.outputPath('system-ui-real-role-workspace.png')
  await page.screenshot({ path: screenshot, fullPage: true })
  await testInfo.attach('compiled-workspace-real-api', { path: screenshot, contentType: 'image/png' })
  const report = { headSha: process.env.E2E_EXPECTED_SHA || '', roleId, roleCode, realBrowserLogin: true,
    realHttp: true, persistenceReadback: true, permissionVersionBefore: original.version,
    permissionVersionAfter: saved.version, memberTotal: members.total, existingRolesPreserved: true,
    auditTotal: evidence.total, unrelatedAccountRoleReplacement: false }
  await testInfo.attach('system-ui-real-api-evidence', { body: Buffer.from(JSON.stringify(report, null, 2)), contentType: 'application/json' })
})

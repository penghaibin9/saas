...
test('imported accounts complete verified bind, change and four authenticated surfaces', async ({ page, browser }) => {
  ...
  for (const item of mobileCases) {
    ...
    const response = mobile.waitForResponse(r => /\/auth\/browser-login(?:\?|$)/.test(r.url()) && r.request().method() === 'POST', { timeout: 20000 })
    await mobile.locator('uni-button.account-button, button.account-button').click()
    expect((await response).status()).toBe(200)

...
try {
  ...
  await check('unconfirmed portal save locks form and readback cannot replay it', async () => {
    ...
    await expect(page.getByLabel('门户名称',{exact:true})).toBeDisabled()
    await expect(page.getByRole('button',{name:'保存配置',exact:true})).toBeDisabled()
    await assert.rejects(() => page.getByLabel('门户名称',{exact:true}).fill('不应写入', {timeout:750}))
    await assert.rejects(() => page.getByRole('button',{name:'保存配置',exact:true}).click({timeout:750}))
    assert.equal((await writes()).length,1)
    await page.getByRole('button',{name:'只读取当前配置',exact:true}).click()
    await expect(page.locator('fieldset')).toHaveAttribute('disabled', '')
    await expect(page.getByLabel('门户名称',{exact:true})).toBeDisabled()
    await expect(page.getByRole('button',{name:'保存配置',exact:true})).toBeDisabled(); assert.equal((await writes()).length,1)
    ...
    await expect(page.getByLabel('门户名称',{exact:true})).toBeEnabled()
  })

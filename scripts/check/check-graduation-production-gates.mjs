...
// Cross-client P0 artifacts.
requireFile('backend/alembic/versions/0141_merge_gd_intern_affairs_heads.py', 'Alembic 多头合并迁移')
requireFile('backend/app/core/mobile_graduation_permissions.py', '教师小程序动作权限门')
requireFile('backend/app/api/v1/mobile_graduation_guard.py', '学生小程序任务书证据路由')
requireFile('backend/app/api/v1/student_portal_graduation_guard.py', '学生门户任务书证据路由')
requireFile('backend/app/modules/graduation/routers/graduation_p0_guard.py', '毕业设计高风险写入口保护')
requireFile('backend/app/modules/graduation/services/graduation_record_resolver.py', '毕业设计当前批次解析器')
requireFile('backend/app/modules/graduation/services/graduation_taskbook_confirmation_service.py', '跨端任务书原子确认服务')
...
if (!routeRegistration.includes('api_router.include_router(graduation_p0_guard.router')
    || routeRegistration.indexOf('api_router.include_router(graduation_p0_guard.router') > routeRegistration.indexOf('graduation, graduation_batch, graduation_student')) {
  failures.push('毕业资格/指导撤销 P0 路由未优先于旧路由注册')
}
...
console.log('毕业设计生产闸门通过：生产 Mock、跨端任务书证据、移动权限、批次解析、迁移合并与高风险写入口均已登记。')
console.log('提示：试点 UAT 签字前不称「已可验收上线」。')

...
// 安全：JWT 密钥必须由环境变量提供。生产环境缺失或过短时直接拒绝启动，
// 避免使用可被预测的默认密钥导致令牌被伪造。
...
if (!JWT_SECRET || JWT_SECRET.length < 32) {
  ...
  JWT_SECRET = JWT_SECRET || 'internship-dev-secret-change-in-production';
}
...
function authMiddleware(req, res, next) {
  ...
  try {
    ...
    // SaaS 安全：令牌所属租户必须与当前请求解析出的租户一致，杜绝跨校令牌复用/身份混淆
...
// 未完成首次改密前，仅放行改密/查看个人信息等少数接口，其余一律拦截。
// 该中间件可在已登录(req.user 存在)后调用。

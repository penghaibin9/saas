...
// 只审计写操作；GET 等读操作不记录，避免日志爆炸。
const AUDIT_METHODS = new Set(['POST', 'PUT', 'PATCH', 'DELETE']);

// 需要脱敏的字段（出现在请求体中时以 *** 替换）。
...
const RESOURCE_LABELS = {
  'users': '用户', 'admin/users': '账号', 'colleges': '学院', 'majors': '专业',
  'classes': '班级', 'enterprises': '企业', 'positions': '岗位',
  ...
  'gd-achievements': '毕设成果', 'quality-checks': '质量检查', 'province': '省厅名单',
  'insurances': '保险', 'agreements': '实习协议', 'defense': '答辩',
  'enterprise-evals': '企业评价', 'quality-report': '质量报告', 'risk': '风险预警',
  'teacher-records': '教师工作记录', 'satisfaction': '满意度', 'complaints': '投诉',
  'emergency-contacts': '紧急联系人', 'audit-logs': '审计日志'
};

// 路径末段子动作 → 中文动词，命中时优先使用。
...
function sanitize(body) {
  ...
  let json;
  try {
    json = JSON.stringify(body, (key, value) => {

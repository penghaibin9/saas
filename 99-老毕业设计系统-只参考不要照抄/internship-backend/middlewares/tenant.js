...
/**
 * 租户解析中间件（挂在 /api 之前，包裹整条请求链）
 * single 模式：恒为默认租户，零开销、零行为变化。
 * multi  模式：按以下优先级解析"学校(租户)"，并将其放入请求级上下文：
 *   1) 显式 X-Tenant 头 / ?tenant= 查询（接口/登录通用）
 *   2) 子域名 schoolA.example.com → schoolA
 *   3) JWT 的 tid 声明（已登录请求自动携带）
...
async function tenantMiddleware(req, res, next) {
  ...
  if (!code) {
    ...
    // 形如 schoolA.example.com 才取子域名；裸 IP / localhost 不取

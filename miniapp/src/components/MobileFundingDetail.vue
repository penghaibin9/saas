<template>
  <view class="fund-result">
    <view class="fund-result__head"><button class="fund-result__head-action" @click="$emit('close')">返回申请</button><text>我的奖助申请</text><button class="fund-result__head-action" :disabled="loading" @click="load">刷新</button></view>
    ...
    <view v-if="detail?.allowedActions?.includes('EDIT_RETURNED')" class="fund-result__footer"><button class="fund-result__footer-action" @click="$emit('edit',detail)">按意见修改并重提</button></view>
  </view>
</template>
<script setup>
...
.fund-result__head { display:flex; justify-content:space-between; align-items:center; padding:calc(8px + env(safe-area-inset-top)) 12px 8px; background:var(--bg-card,#fff); border-bottom:1px solid var(--border-light,#e3e8e6); flex-shrink:0; font-weight:600; }.fund-result__head-action { margin:0; padding:0 8px; line-height:36px; min-height:36px; background:transparent; color:var(--brand-primary,#278578); font-size:13px; }

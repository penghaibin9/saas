<template>
  <view class="mobile-global-state">
    ...
    <view v-else class="mgs-panel">
      <view class="mgs-icon" :class="`mgs-icon--${state}`">
        <text>{{ meta.icon }}</text>
      </view>
      ...
      <view class="mgs-actions">
        <slot name="actions">
          <button v-if="meta.retry" class="mgs-btn mgs-btn--primary" @click="$emit('retry')">
            {{ meta.retry }}
          </button>
...
<script>
/**
 * MobileGlobalState 移动端全局页面状态容器
 * 与 PC 端 AppGlobalState 状态一致：
 * ready | loading | empty | error | forbidden | offline | readonly | noLicense
...
export default {
  ...
  props: {
    state: {
      ...
      default: 'ready',
      validator: (v) =>
        ['ready', 'loading', 'empty', 'error', 'forbidden', 'offline', 'readonly', 'noLicense'].includes(v)
    },

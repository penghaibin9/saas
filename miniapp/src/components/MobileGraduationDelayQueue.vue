<template>
  <view v-if="visible" class="gddq">
    <view class="gddq__head">
      ...
      <view class="gddq__count"><text class="gddq__count-value">{{ rows.length }}</text><text class="gddq__count-label">待处理</text></view>
    </view>
    ...
    <view v-for="row in rows" :key="row.id" class="gddq__card">
      ...
      <view class="gddq__reason"><text class="gddq__label">申请理由</text><text class="gddq__reason-text">{{ row.reason }}</text></view>
      <view v-if="row.allowedActions && row.allowedActions.advisorReview" class="gddq__actions">
        <button class="btn btn-primary gddq__action" :disabled="busyId === row.id" @click="review(row, 'APPROVE')">审核通过</button>
        <button class="btn btn-ghost gddq__action" :disabled="busyId === row.id" @click="review(row, 'REJECT')">驳回</button>
      </view>

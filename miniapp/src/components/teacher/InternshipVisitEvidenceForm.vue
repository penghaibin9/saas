<template>
  <view class="visit-evidence card">
    ...
    <view class="ve__actions">
      ...
      <button class="ve__submit flex-1" :class="{ 'is-disabled': submitDisabled }" :disabled="submitDisabled" plain @click="submit">{{ submitting ? '保存中…' : '保存执行证据' }}</button>
    </view>
  </view>
</template>

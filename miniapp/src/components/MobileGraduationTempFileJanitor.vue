...
export default {
  ...
  data() {
    return { active: false, known: new Set(), initialized: false, timer: null }
  },
  mounted() {
    ...
    if (this.active) {
      this.sync()
      this.timer = setInterval(this.sync, 600)
    }
  },
  beforeUnmount() {
  ...
  methods: {
    ...
    sync() {
      ...
      if (this.initialized) {
        this.known.forEach((id) => { if (!current.has(id)) this.abandon(id) })
      }
      this.known = current
    ...
    abandon(fileId) {
      realRequest(`/mobile/graduation/materials/${fileId}/abandon`, { method: 'POST', data: {} })
        .catch(() => { /* 已提交并绑定时后端 409 是正确保护，不提示误导学生 */ })
    }
  }
}
</script>

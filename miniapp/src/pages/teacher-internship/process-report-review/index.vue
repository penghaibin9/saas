...
export default {
  data() {
    return {
      ...
      batches: [], batchId: '', batchIndex: 0, page: 1, hasMore: false, loadingMore: false, loadSeq: 0
    }
  },
  computed: {
  ...
  methods: {
    ...
    async load(done) {
      ...
      this.state = 'loading'
      this.list = []; this.loadingMore = false; this.loadingDetail = ''
      this.page = 1
      this.hasMore = false
      try {
    ...
    async loadMore() {
      ...
      try {
        ...
        this.page = nextPage
        this.hasMore = !!data?.hasMore
      } catch (e) { if (sequence === this.loadSeq) toast(e?.message || '报告加载失败，请重试') }
    ...
    review(r, action) {
      ...
      uni.showModal({
        ...
        content: '',
        success: async (m) => {
          if (!m.confirm || this.actingId || sequence !== this.loadSeq) return

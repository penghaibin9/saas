<template>
  <view class="page-wrap selection-page" :class="{ 'has-safe-bar': mode === 'detail' || mode === 'volunteers' }">
    <MobileNavBar variant="brand" :title="navTitle" :show-back="mode === 'list'">
      ...
      <template #right>
        <text v-if="catalogReady && mode !== 'volunteers'" class="nav-action" @click.stop="openVolunteers">志愿 {{ selectedCount }}/3</text>
      </template>
    </MobileNavBar>

    <view v-if="mode === 'list'" class="page-pad">
      <MobileGlobalState v-if="contextState !== 'ready'" :state="contextState" title="招聘季信息加载失败" :description="contextError" @retry="loadContext" />
      <view v-else-if="context.catalogState === 'NO_OPEN_CAMPAIGN'" class="preparation-card">
        <text class="preparation-kicker">选岗前准备</text>
        <text class="preparation-title">等待学校开放选岗</text>
        <text class="preparation-description">{{ context.blockReason }}</text>
        <button class="preparation-primary" @click="openPreparation">查看实习准备</button>
        <button class="preparation-refresh" @click="loadContext">重新检查</button>
      </view>
      ...
      <view v-if="context" class="context-card">
        ...
        <view class="context-metrics">
          ...
          <view><text>{{ selectedCount }}/3</text><text>已选志愿</text></view>
        </view>
      </view>
      ...
      <MobileGlobalState :state="listState" @retry="loadPositions">
        <view class="list-summary"><text>学校认可岗位</text><text>共 {{ total }} 个 · 每页 20 条</text></view>
        ...
        <view v-else class="position-stack">
          <view v-for="p in positions" :key="p.id" class="position-card" @click="openPosition(p)">
            <view class="position-top"><text class="position-title">{{ p.title }}</text><text class="position-salary">{{ p.remuneration }}</text></view>
            <text class="position-company">{{ p.companyName }}</text>
            <view class="position-meta"><text>{{ p.workLocation }}</text><text>剩余 {{ p.remaining }} 个</text></view>
    ...
    <view v-else class="page-pad volunteer-wrap">
      <MobileGlobalState :state="volunteerState" :description="volunteerError" @retry="loadVolunteers">
        <view class="profile-core card"><view><text class="section-title">实习档案</text><text class="profile-status">{{ profileCompleteness.ready ? '材料齐备' : '待完善' }}</text></view><text class="profile-percent">完成度 {{ profileCompleteness.percent }}%</text><text class="profile-line">{{ profileCoreText }}</text><button :disabled="volunteerBusy" @click="openProfile">完善实习档案与材料</button></view>
        <MobileInlineAlert v-if="group.status === 'LOCKED'" type="warning" title="志愿已锁定" :description="lockedDescription" />
        <MobileInlineAlert v-else-if="group.status === 'NEEDS_REVISION'" type="info" title="学校已退回补正" description="请按学校意见补充材料。重新投递还需满足当前资格和选岗时间要求，旧企业意见保留为历史。" />
        <MobileInlineAlert v-if="context && !context.canSelect" type="warning" title="当前暂不可投递" :description="context.blockReason || '请先核对当前选岗条件'" />
    ...
    <MobileSafeAreaBar v-else-if="mode === 'volunteers'"><button v-if="confirmOpen" class="safe-primary full" :disabled="volunteerBusy || !consentConfirmed || !materialPreview.previewHash" @click="submitVolunteers">{{ volunteerBusy ? '提交中…' : '确认整组投递' }}</button><button v-else-if="group.status === 'SUBMITTED'" class="safe-secondary full" :disabled="volunteerBusy" @click="withdrawVolunteers">整组撤回并修改</button><button v-else-if="group.status === 'LOCKED'" class="safe-secondary full" :disabled="volunteerBusy" @click="requestUnlock">申请改志愿</button><button v-else-if="volunteerFinalized" class="safe-secondary full" disabled>学校已确认</button><button v-else class="safe-primary full" :disabled="volunteerBusy || !volunteerEditable || activeSlots.length < 1 || !profileCompleteness.ready" @click="prepareSubmit">投递确认</button></MobileSafeAreaBar>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    async loadContext() {
      this.detailSeq++; this.companySeq++; this.mode = 'list'; this.detail = null; this.company = null;
      this.volunteerBusy = false; this.confirmOpen = false; this.consentConfirmed = false; this.materialPreview = normalizeMobileMaterialPreview(); this.profile = null; this.profileCompleteness = { percent: 0, ready: false };
      const sequence = ++this.contextSeq
      this.requestSeq++; this.volunteerSeq++
      this.contextState = 'loading'; this.contextError = ''
      this.positions = []; this.total = 0
      this.group = normalizeMobileVolunteerGroup({ status: 'UNAVAILABLE' }); this.slots = this.group.slots
      try {
        const data = await this.selectionApi.context()
        if (sequence !== this.contextSeq) return
        this.context = normalizeMobileSelectionContext(data || {})
        this.profileScope = data?.recordId ? selectionScope({ batchId: data.batchId, campaignId: data.campaignId, recordId: data.recordId }) : selectionScope(this.roundScope)
        this.contextState = 'ready'
        if (this.catalogReady) await Promise.all([this.loadPositions(), this.loadVolunteers()])
        else { this.mode = 'list'; this.volunteerState = 'ready' }
      } catch (error) {
        if (sequence !== this.contextSeq) return
        this.contextState = 'error'; this.contextError = error?.message || '请稍后重试'
    ...
    openPosition(position) { const sequence = ++this.detailSeq; this.mode = 'detail'; this.detailId = position.id; this.detail = normalizeMobilePositionDetail(position); this.detailState = 'loading'; this.selectionApi.position(position.id).then((data) => { if (sequence !== this.detailSeq) return; this.detail = normalizeMobilePositionDetail(data || position); this.detailState = 'ready' }).catch(() => { if (sequence === this.detailSeq) this.detailState = 'error' }) }, reloadDetail() { if (this.detailId) this.openPosition({ ...(this.detail || {}), id: this.detailId }) },
    openCompany() { if (!this.detail?.companyId) return; const sequence = ++this.companySeq; this.companyId = this.detail.companyId; this.mode = 'company'; this.companyState = 'loading'; this.company = null; this.selectionApi.company(this.companyId).then((data) => { if (sequence !== this.companySeq) return; this.company = normalizeMobilePublicCompany(data || {}); this.companyState = 'ready' }).catch(() => { if (sequence === this.companySeq) this.companyState = 'error' }) }, reloadCompany() { if (this.companyId) { const id = this.companyId; this.mode = 'detail'; this.detail = this.detail || { companyId: id }; this.openCompany() } },
    loadVolunteers() { if (!this.catalogReady) return Promise.resolve(); const sequence = ++this.volunteerSeq; this.volunteerState = 'loading'; this.volunteerError = ''; this.profile = null; this.profileCompleteness = { percent: 0, ready: false }; const unavailable = normalizeMobileVolunteerGroup({ status: 'UNAVAILABLE' }); this.group = unavailable; this.slots = unavailable.slots; return Promise.allSettled([this.selectionApi.volunteers(), this.selectionApi.profile(), this.selectionApi.profileCompleteness()]).then((results) => { if (sequence !== this.volunteerSeq) return; if (results[0].status === 'fulfilled') { const group = normalizeMobileVolunteerGroup(results[0].value || {}); this.group = group; this.slots = group.slots } else this.volunteerError = results[0].reason?.message || '暂时无法读取学校志愿记录，请稍后重试；系统不会用本地数据替代学校记录。'; if (results[1].status === 'fulfilled') this.profile = results[1].value || null; if (results[2].status === 'fulfilled') { const raw = results[2].value || {}; const blockers = Array.isArray(raw.blockers) ? raw.blockers : []; const percent = Math.max(0, Math.min(100, Number(raw.percent ?? raw.completeness ?? 0) || 0)); this.profileCompleteness = { percent, ready: Boolean(raw.readyToSubmit ?? raw.canSubmit ?? (percent === 100 && blockers.length === 0)) } } const failure = results.find(result => result.status === 'rejected'); if (failure && !this.volunteerError) this.volunteerError = failure.reason?.message || '档案与完整度读取失败，请重试'; this.volunteerState = failure ? 'error' : 'ready' }) },
    openVolunteers() { if (!this.catalogReady) return; this.mode = 'volunteers'; this.confirmOpen = false; this.consentConfirmed = false; this.loadVolunteers() },
    persistSlots(next) { if (!this.volunteerEditable || this.volunteerBusy) return; const previous = this.slots; this.slots = next; const sequence = this.contextSeq; this.volunteerBusy = true; this.volunteerError = ''; let payload; try { payload = buildMobileVolunteerSaveRequest(this.group, next) } catch (e) { this.slots = previous; this.volunteerBusy = false; this.volunteerError = e.message; return } this.selectionApi.saveVolunteers(payload).then((data) => { if (sequence !== this.contextSeq) return; if (data) { const group = normalizeMobileVolunteerGroup(data); this.group = group; this.slots = group.slots } else this.loadVolunteers(); this.confirmOpen = false; this.materialPreview = normalizeMobileMaterialPreview() }).catch((e) => { if (sequence !== this.contextSeq) return; this.slots = previous; this.volunteerError = e?.message || '志愿整组保存失败' }).finally(() => { if (sequence === this.contextSeq) this.volunteerBusy = false }) },
    ...
    prepareSubmit() { if (!this.volunteerEditable || this.volunteerBusy) return; const sequence = this.contextSeq; this.volunteerBusy = true; this.volunteerError = ''; this.consentConfirmed = false; this.selectionApi.materialPreview().then((data) => { if (sequence !== this.contextSeq) return; this.materialPreview = normalizeMobileMaterialPreview(data || {}); if (!this.materialPreview.previewHash) throw new Error('请先完善实习档案并生成企业视角材料预览'); this.confirmOpen = true }).catch((e) => { if (sequence !== this.contextSeq) return; this.volunteerError = e?.message || '投递材料预览失败' }).finally(() => { if (sequence === this.contextSeq) this.volunteerBusy = false }) }, changeContactMode(e) { this.contactMode = CONTACT_MODES[Number(e.detail.value || 0)] || 'MASKED_ONLY' },
    submitVolunteers() { if (!this.consentConfirmed || !this.volunteerEditable || this.volunteerBusy) return; const sequence = this.contextSeq; this.volunteerBusy = true; this.volunteerError = ''; let payload; try { payload = buildMobileVolunteerSubmitRequest(this.group, this.materialPreview, this.contactMode) } catch (e) { this.volunteerBusy = false; this.volunteerError = e.message; return } this.selectionApi.submitVolunteers(payload).then(() => { if (sequence !== this.contextSeq) return; this.confirmOpen = false; this.consentConfirmed = false; return this.loadContext() }).catch((e) => { if (sequence !== this.contextSeq) return; const items = e?.details?.invalidItems || []; this.volunteerError = items.length ? items.map((item) => `第${item.volunteerNo}志愿：${item.reason || '岗位不可提交'}`).join('；') : (e?.message || '整组投递失败') }).finally(() => { if (sequence === this.contextSeq) this.volunteerBusy = false }) },
    withdrawVolunteers() { if (this.volunteerBusy) return; const sequence = this.contextSeq; this.volunteerBusy = true; this.selectionApi.withdrawVolunteers({ expectedGroupVersion: this.group.version }).then(() => { if (sequence !== this.contextSeq) return; this.confirmOpen = false; return this.loadVolunteers() }).catch((e) => { if (sequence !== this.contextSeq) return; this.volunteerError = e?.message || '整组撤回失败' }).finally(() => { if (sequence === this.contextSeq) this.volunteerBusy = false }) }, requestUnlock() { if (this.volunteerBusy) return; const sequence = this.contextSeq; this.volunteerBusy = true; this.selectionApi.requestUnlock({ expectedGroupVersion: this.group.version, reason: '学生申请重新调整岗位志愿' }).then(() => sequence === this.contextSeq ? this.loadVolunteers() : undefined).catch((e) => { if (sequence !== this.contextSeq) return; this.volunteerError = e?.message || '申请改志愿失败' }).finally(() => { if (sequence === this.contextSeq) this.volunteerBusy = false }) }
  }
}
</script>

<style scoped>
.preparation-card { display:flex; flex-direction:column; padding:48rpx 36rpx; background:var(--bg-card); border:1px solid var(--border-light); border-radius:24rpx; margin-top:20rpx; }
.preparation-kicker { color:var(--text-secondary); font-size:24rpx; margin-bottom:16rpx; }
.preparation-title { color:var(--text-primary); font-size:38rpx; font-weight:700; }
.preparation-description { color:var(--text-secondary); font-size:28rpx; line-height:1.8; margin:24rpx 0 40rpx; }
.preparation-primary, .preparation-refresh { width:100%; font-size:28rpx; border-radius:14rpx; }
.preparation-primary { background:var(--brand-primary); color:#fff; }
.preparation-refresh { color:var(--brand-primary); background:transparent; margin-top:16rpx; }
.preparation-refresh::after { border:0; }

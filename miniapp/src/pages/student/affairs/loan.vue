...
export default {
  ...
  methods: {
    ...
    async withdraw(item) { if (this.busy) return; this.busy = `withdraw-${item.loanId}`; try { await studentApi.withdrawLoan(item.loanId, item.version); toast('回执已撤回'); await this.load() } catch (e) { toast(normalizeError(e).text || '撤回失败') } finally { this.busy = '' } },

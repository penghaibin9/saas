    ...
    <AcademicPageState :state="state" @retry="load">
      <view class="page-pad stack" v-if="d">
        ...
        <view class="list-group" v-if="d.openExams && d.openExams.length">
          <view v-for="e in d.openExams.slice(0, listLimit)" :key="e.examId">
        ...
        <button v-if="d.openExams.length > listLimit" class="btn" @click="listLimit += 20">查看更多考试</button>

        <template v-if="d.myRegs && d.myRegs.length">
          <view class="section-head"><text class="section-head__title">我的报名</text></view>
          <view class="list-group">
            <view v-for="r in d.myRegs.slice(0, listLimit)" :key="r.regId" class="list-row lv__item">
              <view class="flex-1">
          ...
          <button v-if="d.myRegs.length > listLimit" class="btn" @click="listLimit += 20">查看更多报名记录</button>
        </template>
      </view>
    </AcademicPageState>
...
export default {
  ...
  data() { return { d: null, state: 'loading', detailId: '', confirmed: false } },
  onLoad() { this.load() },
  onHide() { this.confirmed = false },
  methods: {
    catLabel(c) { return CAT[c] || '等级考试' },
    resetAcademicContext() { this.clearApplicationContext(); this.detailId = ''; this.confirmed = false },
    clearForbiddenLevelExam() {
      const hadPending = this.protectPendingReference()
      this.d = null; this.detailId = ''; this.confirmed = false; this.submitting = false
    ...
    load() {
      return this.readAcademic(async () => {
        ...
        try { return await studentApi.getMyLevelExam() }
        ...
        if (!Array.isArray(d.openExams) || !Array.isArray(d.myRegs)) throw new Error('报名记录无法核对')
        this.d = d
        this.acceptApplication(d.myRegs, 'examId', (row, body, kind) => String(this.pendingApplication?.returnedId || '') === String(body.examId) && row.status === (kind === 'cancel' ? 'CANCELLED' : 'REGISTERED'))
      })
    },
    isRegistered(id) { return (this.d.myRegs || []).some(r => String(r.examId) === String(id) && ['REGISTERED','SCORED'].includes(r.status)) },
    examName(id) { return (this.d.openExams || []).find(e => String(e.examId) === String(id))?.examName || (this.d.myRegs || []).find(r => String(r.examId) === String(id))?.examName || '等级考试' },
    canCancel(r) { return r.status === 'REGISTERED' && (this.d.openExams || []).some(e => String(e.examId) === String(r.examId)) },
    register(e) {
    ...
    cancel(r) {
      ...
      return this.sendApplication({ title: '确认取消：' + this.examName(r.examId), kind: 'cancel', body: { examId: r.examId }, existingId: r.examId,

    ...
    <AcademicPageState :state="state" @retry="load">
      <view class="page-pad stack" v-if="d">
        ...
        <view class="list-group" v-if="d.list && d.list.length">
          <view v-for="t in d.list.slice(0, listLimit)" :key="t.taskId" class="list-row ev__item" :class="{ 'is-target': String(t.taskId) === targetId }">
            <view class="flex-1">
        ...
        <button v-if="d.list.length > listLimit" class="btn" @click="listLimit += 20">查看更多课程</button>
      </view>
    </AcademicPageState>
...
mixins: [academicApplicationPage],
  ...
  data() { return { d: null, state: 'loading', active: null, score: '', comment: '', targetId: '', drafts: {}, academicDraftFields: ['active', 'score', 'comment', 'drafts'] } },
  computed: {
  ...
  methods: {
    ...
    resetAcademicContext() { this.clearApplicationContext(); this.active = null; this.drafts = {}; this.score = ''; this.comment = ''; this.targetId = '' },
    ...
    load() {
      ...
      return this.readAcademic(async () => {
        ...
        try {
          return await studentApi.getMyEvaluationTasks()
        } catch (error) {
        ...
        if (!Array.isArray(d.list)) throw new Error('评教信息无法核对')
        this.d = d
        if (this.active) {

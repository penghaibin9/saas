<template>
  <view class="page-wrap">
    ...
    <MobileGlobalState :state="state" @retry="load">
      <view class="page-pad">
        <view v-if="!recordId && !['checkout', 'recheck'].includes(tab)" class="provider-card">
        ...
        <view v-if="!recordId && !['checkout', 'recheck'].includes(tab)" class="presence-summary">
        ...
        <view v-if="!recordId && !['checkout', 'recheck'].includes(tab)" class="allocation-summary">
        ...
        <view v-if="!recordId" class="seg">
          <button class="seg__btn" :class="{ on: tab === 'transfer' }" @click="tab = 'transfer'; load()">调宿待审</button>
          <button class="seg__btn" :class="{ on: tab === 'exception' }" @click="tab = 'exception'; load()">异常待处置</button>
...
export default {
  ...
  methods: {
    ...
    async startRoomRectification(row) {
      ...
      this.acting = true
      try { await realRequest(`/mobile/teacher/affairs/dorm/rectifications/${row.rectificationId}/start`, { method: 'POST', data: { expectedVersion: version } }); await this.load() }
      catch (e) { this.showError(e, '开始整改失败') } finally { this.acting = false }
    },
    async uploadRoomRectification(row) {
      ...
      try {
        ...
        this.roomRectFiles[row.rectificationId] = { fileId: String(uploaded.fileId || uploaded.id), fileName: uploaded.fileName || selected.name || '整改照片' }
      } catch (e) { this.showError(e, '整改照片上传失败') } finally { this.acting = false }
    },
    async submitRoomRectification(row) {
      ...
      const version = this.versionOf(row); if (version === null) return
      const payload = JSON.stringify({ note, fileId: file.fileId })
      if (this.roomRectRequests[id]?.payload !== payload) this.roomRectRequests[id] = { payload, key: createClientRequestId('dorm-rectify') }
      this.acting = true
      try {
        await realRequest(`/mobile/teacher/affairs/dorm/rectifications/${id}/submit`, { method: 'POST', data: { expectedVersion: version, note, fileIds: [file.fileId], clientRequestId: this.roomRectRequests[id].key } })
      ...
      } catch (e) { this.showError(e, '整改提交失败') } finally { this.acting = false }
    },
    ...
    confirmCheckout(row) {
      ...
      this.actionDlg = {
        ...
        submit: async () => {
          ...
          this.acting = true; this.checkoutReceipt = ''
          try {
            const result = await realRequest(`/student-affairs/dorm/checkout-requests/${row.requestId}/confirm`, { method: 'POST', data: { version } })
            this.checkoutReceipt = `${result.studentName || row.studentName}：${this.checkoutLabel(result.status)} · ${result.bedLabel || row.bedLabel}`
            await this.load()
          } catch (e) { this.showError(e, '退宿确认失败，床位结果请刷新核对') }
    ...
    async selectRoom(room) { this.inspection.roomId = room.roomId; this.inspection.studentId = ''; this.inspection.clientRequestId = createClientRequestId('dorm-record'); this.occupants = []; try { const data = await affairsContractApi.getDormInspectionBeds(room.roomId); const seen = new Set(); this.occupants = (data.items || []).filter((bed) => bed.studentId && !seen.has(String(bed.studentId)) && seen.add(String(bed.studentId))).map((bed) => ({ studentId: bed.studentId, realName: bed.studentName || bed.occupantName || bed.realName, studentNo: bed.studentNo })) } catch (e) { this.showError(e, '房间住宿学生加载失败') } },

    ...
    <MobileGlobalState :state="state" @retry="load">
      <view class="page-pad">
        ...
        <view class="stack" v-else>
          <view v-for="x in list" :key="x.deferId" class="card ed">
...
export default {
  ...
  methods: {
    ...
    _err(e, label) {
      ...
      if (code === 'APPROVAL_VERSION_CONFLICT') { toast((e && e.message) || '该申请已处理，正在刷新'); this.load() }
      ...
      uni.showModal({
        ...
        success: (r) => {
          ...
          teacherApi.reviewAcademicDefer(x.deferId, action, reason)
            .then(() => { toast(action === 'APPROVE' ? '已通过' : action === 'RETURN' ? '已退回' : '已驳回'); this.load() })

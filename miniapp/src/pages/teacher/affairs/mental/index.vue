  ...
  <view class="page-wrap">
    ...
    <MobileGlobalState :state="state" @retry="load">
      <view class="page-pad" v-if="!active && loaded">
        <MobileInlineAlert type="info" title="明细受限" description="名单仅展示业务身份、关注等级和状态；查看转介事由及明细必须填写原因并成功写入敏感访问审计。" />
        ...
        <view class="card stack-sm" v-if="showForm">
          ...
          <button class="btn btn-primary" :disabled="creating || !canCreate" @click="createReferral">{{ creating ? '登记中…' : '登记转介' }}</button>
        ...
        <view class="list-group" v-else>
          <view v-for="r in list" :key="r.referralId" class="list-row" @click="openDetail(r)">
            <view class="flex-1"><text class="t-md">{{ r.realName }}</text><text class="mt__sub">{{ r.studentNo }}</text></view>
      ...
      <view class="page-pad" v-if="active">
        ...
        <template v-if="active.noteMasked">
          ...
          <view class="card stack-sm">
            <input class="mt__input" v-model="viewReason" maxlength="200" placeholder="查看原因（5-200字，将被审计留痕）" placeholder-class="mt__ph" />
            <button class="btn btn-primary" :disabled="viewReason.trim().length < 5 || viewReason.trim().length > 200" @click="revealDetail">查看明细</button>
          </view>
        </template>
        <template v-else>
          ...
          <template v-if="active.status !== 'CLOSED'">
            ...
            <view class="card stack-sm">
              <textarea class="mt__textarea" v-model="actionContent" :maxlength="300" placeholder="记录内容（回访、升级原因或关闭结论，5-300字）" placeholder-class="mt__ph" />
              <text class="mt__counter">{{ actionContent.trim().length }}/300</text>
              <button v-if="canAct('FOLLOW')" class="btn btn-ghost" :disabled="acting || actionContent.trim().length < 5" @click="doFollow">回访（≥5字）</button>
              <button v-if="canAct('ESCALATE')" class="btn btn-ghost mt__danger" :disabled="acting || actionContent.trim().length < 5" @click="doEscalate">升级为危机（说明≥5字）</button>
              <button v-if="canAct('CLOSE')" class="btn btn-ghost" :disabled="acting || actionContent.trim().length < 5" @click="doClose">关闭（结论≥5字）</button>
              <text v-if="!canAct('FOLLOW') && !canAct('ESCALATE') && !canAct('CLOSE')" class="mt__hint">当前状态暂无可执行动作。</text>
            </view>
          </template>
        </template>
      </view>
...
export default {
  ...
  computed: {
    ...
    levelLabel() { const o = LEVEL_OPTS.find((x) => x.value === this.form.level); return o ? o.label : '选择等级' },
  ...
  methods: {
    ...
    toggleForm() { this.showForm = !this.showForm; if (this.showForm && !this.students.length) this.loadCandidates() },
    loadCandidates() {
    ...
    revealDetail() {
      const reason = this.viewReason.trim(); if (reason.length < 5 || reason.length > 200) return
      teacherApi.getMentalDetail(this.active.referralId, reason).then((d) => { this.active = d }).catch((e) => this.showError(e, '查看失败'))
    },
    version() { if (this.active.version === undefined || this.active.version === null) { toast('记录缺少版本号，请重新打开详情'); return null }; return this.active.version },
    doFollow() {
      const content = this.actionContent.trim(); if (this.acting || !this.canAct('FOLLOW') || content.length < 5) return
      const version = this.version(); if (version === null) return
      this.acting = true
      affairsContractApi.followMental(this.active.referralId, content, version).then((d) => { toast('回访已记录'); this.active = d; this.actionContent = '' }).catch((e) => this.showError(e, '操作失败')).finally(() => { this.acting = false })
    },
    doEscalate() {
      const content = this.actionContent.trim(); if (this.acting || !this.canAct('ESCALATE') || content.length < 5) return
      const version = this.version(); if (version === null) return
      uni.showModal({ title: '确认升级为危机', content: `升级原因：${content}\n\n将接入风险中枢并生成危机记录，请确认信息准确。`, confirmText: '确认升级', success: (r) => {
        if (!r.confirm) return
        this.acting = true
        affairsContractApi.escalateMental(this.active.referralId, content, version).then((d) => { toast('已升级'); this.active = d; this.actionContent = '' }).catch((e) => this.showError(e, '操作失败')).finally(() => { this.acting = false })
      } })
    },
    doClose() {
      const content = this.actionContent.trim(); if (this.acting || !this.canAct('CLOSE') || content.length < 5) return
      const version = this.version(); if (version === null) return
      uni.showModal({ title: '确认关闭心理关注', content: `关闭结论：${content}\n\n关闭后该记录转为只读，确认已完成必要处置。`, confirmText: '确认关闭', success: (r) => {
        if (!r.confirm) return
        this.acting = true
        affairsContractApi.closeMental(this.active.referralId, content, version).then((d) => { toast('已关闭'); this.active = d; this.actionContent = '' }).catch((e) => this.showError(e, '操作失败')).finally(() => { this.acting = false })
      } })
    }
  }
}

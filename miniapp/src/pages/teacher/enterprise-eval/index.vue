<template>
  <view class="page-wrap">
    <MobileNavBar variant="teacher" title="企业评价" subtitle="纸质材料代录 · 退回重交 · 独立审核" show-back />

    <view class="ee__tabs">
      <view class="ee__tab" :class="{ 'is-on': tab === 'list' }" @click="showList">评价台账<text v-if="tab === 'list'" class="ee__tab-u" /></view>
    ...
    <MobileGlobalState :state="state" @retry="load">
      <view class="page-pad stack">
        <view class="card ee__batch" v-if="batches.length">
          ...
          <picker class="ee__picker" mode="selector" :disabled="!!editingEval" :range="batchLabels" :value="batchIndex" @change="onBatch">
            <view class="ee__pick-val">{{ batchLabels[batchIndex] || '请选择批次' }}<text v-if="!editingEval" class="ee__arrow">▾</text></view>
          </picker>
        </view>
        ...
        <template v-if="tab === 'list'">
          ...
          <view v-for="item in list" :key="item.id" class="card ee">
            ...
            <text v-if="item.reviewComment" class="ee__return">审核意见：{{ item.reviewComment }}</text>
            ...
            <view v-else-if="canReview && item.reviewStatus === 'PENDING'" class="ee__actions">
              <button class="btn btn-ghost flex-1" :disabled="submitting || isOwnRecord(item)" @click="review(item, 'RETURN')">{{ isOwnRecord(item) ? '录入人不可自审' : '退回' }}</button>
        ...
        <template v-else-if="canCreate">
          ...
          <view v-else class="card ee__form">
            <view class="ee__row"><text class="ee__label">实习学生 *</text><template v-if="editingEval"><text class="ee__readonly">{{ editingEval.studentName }}（{{ editingEval.studentNo }}）</text></template><picker v-else class="ee__picker" mode="selector" :range="studentLabels" :value="studentIndex" @change="(e) => studentIndex = Number(e.detail.value)"><view class="ee__pick-val">{{ studentLabels[studentIndex] || '请选择' }}<text class="ee__arrow">▾</text></view></picker></view>
            ...
            <view class="ee__file"><view><text class="ee__label">企业盖章材料 *</text><text class="ee__file-name">{{ sourceFileName || (editingEval ? '退回重交必须上传新的企业材料' : '支持图片、PDF、Word等文件') }}</text></view><button class="btn btn-ghost" :disabled="uploading" @click="pickFile">{{ uploading ? '上传中…' : (form.sourceFileId ? '重新上传' : '选择并上传') }}</button></view>
          </view>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      else { body.internshipId = student.id; body.batchId = this.batchId }
...
<style scoped>
.ee__tabs{display:flex;gap:var(--space-6);padding:var(--space-3) var(--page-padding-mobile) 0;background:var(--bg-card)}.ee__tab{position:relative;padding-bottom:var(--space-3);color:var(--text-tertiary)}.ee__tab.is-on{color:var(--text-primary);font-weight:var(--font-weight-semibold)}.ee__tab-u{position:absolute;left:50%;bottom:0;transform:translateX(-50%);width:22px;height:3px;border-radius:2px;background:var(--teacher-600)}.ee__batch,.ee__row,.ee__file{display:flex;align-items:center;min-height:50px;border-bottom:1px solid var(--border-light)}.ee__label{width:110px;flex-shrink:0;color:var(--text-secondary);font-size:var(--font-size-sm)}.ee__picker,.ee__input{flex:1}.ee__pick-val,.ee__input,.ee__readonly{text-align:right;color:var(--text-primary);font-size:var(--font-size-sm)}.ee__readonly{flex:1}.ee__arrow{margin-left:4px;color:var(--text-tertiary)}.ee{display:flex;flex-direction:column;gap:var(--space-2)}.ee__sub{display:block;margin-top:3px;color:var(--text-tertiary);font-size:var(--font-size-xs)}.ee__scores{display:flex;flex-wrap:wrap;gap:6px 12px;padding:var(--space-2);background:var(--gray-50);border-radius:var(--radius-md);font-size:var(--font-size-xs)}.ee__avg{color:var(--teacher-700);font-weight:var(--font-weight-semibold)}.ee__return{padding:var(--space-2);border-radius:var(--radius-md);background:var(--danger-50);color:var(--danger-700);font-size:var(--font-size-xs)}.ee__source{display:flex;flex-wrap:wrap;gap:8px 14px;color:var(--text-tertiary);font-size:var(--font-size-xs)}.ee__actions{display:flex;gap:var(--space-2)}.ee__form{display:flex;flex-direction:column}.ee__file{justify-content:space-between;gap:var(--space-2);padding:var(--space-2) 0}.ee__file-name{display:block;margin-top:4px;color:var(--text-tertiary);font-size:var(--font-size-xs)}
</style>

...
test('W4 teacher graduation preview returns to the same review context and revalidates canonical version before actions', () => {
  ...
  assert.match(teacher, /this\.reviewKind !== kind \|\| this\.queueIndex !== queueIndex/)
  assert.match(teacher, /const fresh = await api\(\)/)
  assert.match(teacher, /freshIdentity !== beforeIdentity \|\| fresh\.reviewReady !== true/)
  assert.match(teacher, /this\.detail = fresh/)
  assert.match(teacher, /this\.detail\.reviewReady === true && !this\.previewVersionConflict/)

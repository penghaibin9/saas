...
> 依据：《_13-需求输入-V1.1.md》《_13-现有系统集成事实速查.md》；页面编号与路由基准：《13B-教务中心页面树与路由设计.md》；移动端逐页规格：《13B-教务中心移动端入口设计.md》（本文引用不重复展开）；跨端一致性：《13A-13B-跨端一致性与数据联动矩阵.md》。
...
|---|---|---|---|---|---|
| 教务首页（教务处/学院教务/教师三视图） | `/` | 教务处/学院教务/教师 | ★ | Dashboard preset、stats 聚合 | docs/modules/13B-教务中心页面树与路由设计.md §2-01、backend/app/services/stats_service.py |
| 学年学期与校历 7 页（列表/新增/编辑/校历/节次/发布/归档☆） | `/terms/*` | 教务处（学院看） | ★（6 页） | 状态机 DRAFT/PUBLISHED/FROZEN/ARCHIVED；发布同步课表/工作台/学生端 | 同上 §3.1 |
| 学籍管理 10 页（档案/注册批次/核对/异动列表/详情/代录/审批/统计；学年注册☆） | `/roll/*` | 教务处/学院教务/辅导员（本班只读）/节点人 | ★（8 页） | t_student_profile 单一写入口 + t_aa_status_change 异动流水 + Workflow | backend/app/models/student.py、backend/app/models/approval.py、docs/modules/13B-教务中心与现有系统融合设计.md §1 |
| 专业/班级一览（只读复用） | `/majors`、`/classes` | 教务处/学院教务 | ★ | t_college/t_major/t_class 直读 | backend/app/models/org.py |
| 培养方案 8 页（列表/新增/详情/编制/课程配置/审核/发布；版本对比☆） | `/programs/*` | 教务处/学院教务/专业负责人 | ★（7 页） | 版本化方案表；审批走 Workflow | docs/modules/13B-教务中心页面树与路由设计.md §3.3 |
| 课程库 6 页（列表/新增/详情/编辑/审核/导入） | `/courses/*` | 课程负责人/学院教务/教务处 | ★ | 导入管线 dry-run→confirm | backend/app/services/domain_import_service.py |
| 教学任务 7 页（批次/生成/核对/教师确认/审核；统计☆） | `/teaching-tasks/*` | 教务处/学院教务/专业负责人/教师 | ★（6 页） | Workflow + t_unified_todo | docs/modules/13B-教务中心页面树与路由设计.md §3.4 |
...
| 成绩管理（V1：学生成绩单/成绩统计；录入/审核/发布/更正☆P2） | `/grades/*` | 教务处/学院教务/教师（录入 P2） | ★（2 页） | **读写既有 t_acad_grade 扩展字段，不建平行表** | backend/app/models/academic.py、docs/modules/13B-教务中心页面树与路由设计.md §2-12 |
...
| 教材/教室/评价/等级考试/教学计划/归档 | `/textbooks/*` 等 | 教务处/学院教务 | ☆P2/P3 | 复用导出/水印/审计管线 | docs/modules/13B-教务中心页面树与路由设计.md §2-16~21 |
...
|---|---|---|---|---|---|
| 教务首页·教务处视图 | /admin/academic-affairs | 教务处管理员 | ★ | Dashboard preset（任务/排课/成绩/异动/预警/毕业指标卡） | docs/source-design/09A（6.3）、docs/modules/13B-教务中心页面树与路由设计.md §2-01 |
...
| 教师课表 | /admin/academic-affairs/schedule/teacher/:teacherId | 任课教师（本人）/教务 | ★ | 读已发布课表 | docs/modules/13B-教务中心页面树与路由设计.md §3.4 |
...
| 调停课发起/审批 | /course-adjustments/* | 教师/学院/教务 | ☆P2 | Workflow | docs/modules/13B-教务中心页面树与路由设计.md §2-09 |
...
| 我的考试/缓考申请 | /portal/academic-affairs/exams | 学生本人 | ◇（且待 P2 考务） | — | docs/source-design/09B（09B.8） |
| 学业预警（只读知悉） | /portal/academic-affairs/warnings | 学生本人 | ◇ | StatusTag | — |
| 毕业进度红绿灯 | /portal/academic-affairs/graduation-progress | 学生本人 | ◇ | 与小程序同口径同端点 | docs/modules/13B-教务中心移动端入口设计.md §2.9 |
...
|---|---|---|---|---|---|---|
| 2.1 | 我的课表（周视图） | pages/student/academic-affairs/schedule | 学生本人 | ★只读 | 校历/发布数据驱动；变更角标（调/停/补） | docs/modules/13B-教务中心移动端入口设计.md §2.1 |
...
| # | 入口 | 页面路径 | 角色 | V1 | 复用组件 | 证据路径 |
|---|---|---|---|---|---|---|
| 3.1 | 今日课程 | pages/teacher/academic-affairs/today-courses | 任课教师 | ★ | 既有教师工作台页扩展「今日课程」卡直达；读已发布课表 | miniapp/src/pages/teacher/workbench、docs/modules/13B-教务中心移动端入口设计.md §3.1 |

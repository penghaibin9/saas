...
> 需求依据：`docs/modules/_13-需求输入-V1.1.md` §2.x；实现依据：`docs/modules/_13-现有系统集成事实速查.md`（响应/错误码/鉴权/Workflow/待办消息/导入导出管线全部复用既有实现）。
...
业务错误绝不 500；列表空数据返回成功 + 空 list；范围内无数据→空列表，越权访问具体资源→403002（判定标准见 docs/api/04 §三）。

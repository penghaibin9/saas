...
## 附录三：2026-07-10 PC 三级目录审计与 12 域收敛方案

> 本附录是 PC 菜单与页面信息架构的当前裁定，覆盖本文旧版“18 个二级模块”和代码旧版“27 个二级模块”的菜单组织口径；旧业务设计、状态机、API 契约和路由仍保留，不因目录收敛而删除。
> 本轮只做页面树、菜单、状态和兼容审计，不开发教务后端，不创建空页面，不把 mock 页面包装成已实现。

### 三.1 对标边界与共性结论

本轮只参考公开资料中的通用产品原则，不进入第三方系统、不抓取受限页面、不复制界面、文案、图标、字段布局或私有流程。

| 对标材料 | 公开信息可验证的产品侧重点 | 本项目只吸收的共性 | 明确不复制 |
|---|---|---|---|
| 强智教学微服务平台公开产品介绍 | 教、学、管理、服务、决策一体化；教学、教材、质量、实践、移动服务按需组合 | 多角色工作台、教学运行主线、服务与决策分层、能力按租户授权 | 微服务拆法、页面布局、品牌、专有交互和文案 |
| 正方软件公开产品目录与已部署教学管理平台 | 教学管理服务平台作为智慧校园业务平台，与门户/认证协同 | 统一入口、角色上下文、菜单与权限分离、旧系统兼容 | 登录页、导航版式、字段命名、专有流程 |
| 青果公开官网与学校投用案例 | 教学计划、教学安排、考试、成绩、学籍、教材、质量、毕业处理等形成完整教务域 | 按真实教学运行分域，计划→任务→排课→考试→成绩→归档闭环 | 12 模块原名、第三方菜单层级、图标与页面结构 |

公开参考仅用于共性验证：

- 强智教学微服务平台：<https://heec.cahe.edu.cn/company/product/152.html>
- 正方软件产品目录：<https://www.zfsoft.com/>
- 青果软件官网：<https://www.kingosoft.com/>
- 青果教务系统学校投用案例：<https://www.yeu.edu.cn/2014/0902/c770a23042/page.htm>

原创落地结论：教务中心不按数据库实体或审批动作逐个拆二级菜单，而按“今天要处理什么”和“业务结果流向哪里”组织为 12 个业务域；高频任务前置，详情、指标、时间线、归档材料下沉。

### 三.2 当前代码事实与状态裁定

| 审计项 | 结论 |
|---|---|
| PC 左侧一级菜单组件 | `frontend/src/layouts/BasePortalLayout.vue` |
| 一级菜单真实数据源 | `frontend/src/config/adminMenu.js` |
| 二/三级菜单真实数据源 | `frontend/src/config/navPlan.js` |
| router meta 是否直接生成菜单 | 否，只用于路由权限/标题契约 |
| Pinia 是否生成菜单 | 否 |
| 后端菜单接口/数据库是否生成菜单 | 否 |
| localStorage/sessionStorage 菜单缓存 | 无；localStorage 仅存主题偏好，sessionStorage 存登录令牌 |
| 旧二级模块数 | 27 |
| 旧三级叶子数 | 295（6 个标 implemented、289 个 planned；implemented 标记经本轮审计不成立） |
| 真实注册路由 | 8 条：工作台、学生列表/详情、成绩、学分、补考重修、预警列表/详情 |
| 页面真实状态 | 8 条均为 partial；`academic.api.js` 仍有 mock fallback，不能达到 implemented 定义 |
| 新二级模块数 | 12 |
| 新三级页面库存 | 118 |
| 独立菜单页 | 87 |
| 非菜单页 | 31（详情、抽屉/tab、指标下钻、时间线） |
| 新状态统计 | implemented 0 / partial 8 / planned 110 |

### 三.3 新二级模块与三级页面全量清单

状态图例：`[P]` = partial，可访问旧页但未达生产闭环；`[N]` = planned，无 path、不可点击；`[H]` = 非菜单页面库存。

1. **教务工作台**（5 个菜单页 + 3 个非菜单页）
   - `[P]` 教务总览（现有学业过程承接）
   - `[N]` 我的待办、教学运行监控、今日课表、运行异常台账
   - `[H]` 运行提醒指标组、教务数据趋势、快捷入口面板
2. **教学计划**（7 + 2）
   - `[N]` 学年学期、培养方案、专业教学计划、开课计划、教学任务生成、计划调整、计划审核
   - `[H]` 培养方案详情、版本对比与审核记录
3. **课程与排课**（10 + 3）
   - `[N]` 课程库、开课管理、排课任务、手动排课、课表管理、调停课申请、调停课审批、教室占用、排课冲突、课表发布
   - `[H]` 课程详情与大纲、排课冲突明细、课表发布与回退记录
4. **选课与教学班**（7 + 2）
   - `[N]` 选课批次、选课规则、学生选课、退补选、选课名单、教学班管理、重修报名
   - `[H]` 选课批次详情、教学班详情与合班记录
5. **教师教学任务**（6 + 2）
   - `[N]` 教师任务列表、任课安排、教师课表、教学班学生、任务确认、教学材料
   - `[H]` 教学任务详情、教师工作量指标
6. **考务管理**（8 + 3）
   - `[N]` 考试批次、排考管理、考场安排、监考安排、准考名单、缓考管理、考试异常
   - `[P]` 补考与重修（现有承接）
   - `[H]` 考试批次详情、考试安排详情、考务统计指标
7. **成绩管理**（9 + 3）
   - `[N]` 成绩录入、成绩提交、成绩审核、成绩发布、成绩更正、成绩复核、成绩单
   - `[P]` 成绩查询（现有课程成绩）、GPA 与学分修读（现有承接）
   - `[H]` 成绩任务详情、成绩更正详情、成绩单详情
8. **学籍管理**（8 + 3）
   - `[P]` 学生学籍（现有学业台账承接）
   - `[N]` 学籍注册、学籍异动、休学复学、转专业、转学退学、毕业资格预审、学籍台账
   - `[H/P]` 学生学业详情（现有）；`[H/N]` 学籍异动审批详情、毕业资格预审详情
9. **学业预警**（6 + 3）
   - `[P]` 预警学生（现有承接）
   - `[N]` 预警规则、预警分派、预警处置、跟进记录、预警统计
   - `[H/P]` 预警跟进详情（现有）；`[H/N]` 预警来源指标、预警处置时间线
10. **教材与教学资源**（6 + 2）
    - `[N]` 教材目录、教材选用、教材征订、教材发放、教室资源、实训室资源
    - `[H]` 教材选用详情、教学资源详情
11. **教学质量**（7 + 2）
    - `[N]` 听课评价、学生评教、督导评价、课堂巡查、教学事故、质量问题、整改跟进
    - `[H]` 评价结果详情、质量问题整改详情
12. **教务统计与归档**（8 + 3）
    - `[N]` 教务统计、业务归档中心、教师工作量统计、学院汇总、校级汇总、教务数据驾驶舱、导出任务、验收台账
    - `[H]` 归档包详情、统计指标下钻、导出与下载审计详情

### 三.4 成熟商用 PC 页面体验蓝图

每个二级模块首屏采用统一的“业务结论优先”结构，不把多个 CRUD 表格简单堆叠：

1. **上下文条**：当前学年学期、当前激活角色、数据范围、业务阶段、最后刷新时间；权限来自后端上下文，不写死角色名。
2. **结论区**：4–6 个可解释指标卡，只显示当前数据范围；指标写清分子、分母、阈值、更新时间和下钻目标。
3. **今日任务区**：待提交、待审核、待发布、临期、超期五类，按风险与截止时间排序。
4. **异常区**：冲突、缺失、越权拒绝、发布失败、数据不同步；每条异常有来源、责任人、状态、恢复动作和审计入口。
5. **主工作区**：列表/看板按业务阶段切换；大列表必须服务端分页，筛选条件默认记忆，日期默认不限。
6. **低频配置**：规则、模板、版本、归档策略进入“更多/配置”，不与高频任务同权重平铺。
7. **状态反馈**：loading、empty、error、no permission、network error、validation error、submit success、submit failed 完整覆盖。
8. **高风险操作**：发布、撤回、更正、异动生效、批量导出必须二次确认、原因、权限、状态机和审计。
9. **Excel**：正式导入导出统一 xlsx，按当前筛选和数据范围裁剪；敏感字段脱敏，下载与导出任务可追溯。
10. **视觉密度**：桌面端采用克制的蓝灰中后台风格；12–14px 正文、稳定表格行高、清晰层级与留白；不使用第三方品牌色、图标和专有版式。

推荐的模块首页信息结构：

| 模块 | 首屏业务结论 | 高频区 | 异常/风险区 | 低频下沉 |
|---|---|---|---|---|
| 教务工作台 | 今日教学是否正常、待办是否超期 | 今日课表、我的待办、待发布 | 冲突/未排/未发布/接口异常 | 快捷入口配置 |
| 教学计划 | 本学期计划完成度与审核进度 | 方案、开课、任务生成 | 版本冲突、缺课、未审核 | 模板、归档就绪检查 |
| 课程与排课 | 已排率、冲突数、课表发布状态 | 手工排课、调停课、课表 | 教师/班级/教室/容量冲突 | 规则、历史发布批次 |
| 选课与教学班 | 批次阶段、容量、异常选课 | 退补选、名单、教学班 | 超员、先修不满足、时间冲突 | 规则版本、归档 |
| 教师教学任务 | 待确认任务、材料完整率 | 任课安排、课表、学生 | 未确认、任务冲突、材料缺失 | 工作量口径 |
| 考务管理 | 排考完成率、监考缺口、发布状态 | 排考、考场、监考、异常 | 时间/考场/人员冲突 | 统计、归档 |
| 成绩管理 | 录入/审核/发布进度与职责分离 | 录入、提交、审核、发布 | 未交、退回、异常分布、更正 | 成绩单、归档 |
| 学籍管理 | 注册率、异动在办、毕业预审异常 | 注册、异动、预审 | 状态冲突、材料缺失、超期 | 学籍台账、归档 |
| 学业预警 | 新增风险、超期、闭环率 | 分派、处置、跟进 | 挂科/学分/GPA/缺课/毕业风险 | 规则、统计 |
| 教材与资源 | 征订/发放进度、资源可用率 | 选用、征订、发放、占用 | 缺书、冲突、停用、维修 | 台账、统计 |
| 教学质量 | 评价覆盖率、整改完成率 | 听课、评教、巡查、整改 | 事故、低分、超期整改 | 指标模板、归档 |
| 统计与归档 | 数据完整率、归档率、验收项 | 汇总、归档包、导出任务 | 缺失材料、口径不一致、导出失败 | 归档策略、口径版本 |

### 三.5 教务旧页面到新三级页面映射表

| 旧模块 | 旧页/职责 | 新二级模块 | 新三级页面/承接形态 | 处理方式 | 旧路由 | 新路由 | 兼容 | 当前状态 |
|---|---|---|---|---|---|---|---|---|
| aa-dashboard | 教务看板 | 教务工作台 | 教务总览 | 保留菜单 | `/admin/academic` | 本轮沿用 | 是 | partial |
| aa-terms | 学年学期 | 教学计划 | 学年学期 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-calendar | 校历节次 | 教学计划 | 学年学期内校历/节次 tab | 移为 tab | 无 | planned | 不适用 | planned |
| aa-student-status | 学籍管理 | 学籍管理 | 学生学籍 | 合并菜单 | `/admin/academic/students` | 本轮沿用 | 是 | partial |
| aa-registration | 注册管理 | 学籍管理 | 学籍注册 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-status-change | 学籍异动 | 学籍管理 | 学籍异动/休复学/转专业 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-orgs | 学院专业班级 | 学籍管理 | 组织主数据 external-link/Picker | external-link | 无 | 系统组织中心待确认 | 不适用 | planned |
| aa-training | 培养方案 | 教学计划 | 培养方案 | 保留菜单 | 无 | planned | 不适用 | planned |
| aa-courses | 课程库 | 课程与排课 | 课程库；详情/大纲下沉 | 移为详情页 | 无 | planned | 不适用 | planned |
| aa-teaching-plan | 教学计划 | 教学计划 | 专业教学计划/开课计划 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-teaching-tasks | 教学任务 | 教学计划/教师教学任务 | 任务生成；教师任务列表 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-scheduling | 排课管理 | 课程与排课 | 排课任务/手动排课 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-schedule | 课表管理 | 课程与排课 | 课表管理/发布 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-schedule-change | 调停课 | 课程与排课 | 调停课申请/审批 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-course-selection | 选课管理 | 选课与教学班 | 选课批次/规则/名单 | 保留菜单 | 无 | planned | 不适用 | planned |
| aa-exam | 考务管理 | 考务管理 | 考试批次/排考/监考 | 保留菜单 | 无 | planned | 不适用 | planned |
| aa-makeup | 补缓重免 | 考务管理 | 缓考管理/重修报名/补考与重修 | 合并菜单 | `/admin/academic/makeup-retake` | 本轮沿用 | 是 | partial |
| aa-grades | 成绩管理 | 成绩管理 | 成绩查询/GPA 与学分修读 | 保留菜单 | `/admin/academic/grades`、`/credits` | 本轮沿用 | 是 | partial |
| aa-grade-review | 审核发布更正 | 成绩管理 | 提交/审核/发布/更正/复核 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-warning | 学业预警 | 学业预警 | 预警学生/处置/跟进 | 保留菜单 | `/admin/academic/warnings` | 本轮沿用 | 是 | partial |
| aa-graduation-qual | 毕业资格 | 学籍管理 | 毕业资格预审 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-textbooks | 教材管理 | 教材与教学资源 | 教材目录/选用/征订/发放 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-resources | 教学资源 | 教材与教学资源 | 教室/实训室资源 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-evaluation | 教学评价 | 教学质量 | 听课/评教/督导评价 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-quality | 教学质量 | 教学质量 | 巡查/事故/整改 | 合并菜单 | 无 | planned | 不适用 | planned |
| aa-archive | 教务归档 | 教务统计与归档 | 业务归档中心；分域归档为 tab | 移为 tab | 无 | planned | 不适用 | planned |
| aa-stats | 教务统计 | 教务统计与归档 | 统计/汇总/驾驶舱 | 合并菜单 | 无 | planned | 不适用 | planned |

现有详情路由不挂菜单：`/admin/academic/students/:id` → 学籍管理/学生学业详情；`/admin/academic/warnings/:id` → 学业预警/预警跟进详情。两者均保留兼容且保持 partial。

### 三.6 教务业务流程到菜单落位表

| 业务主线 | 业务动作 | 二级模块 | 三级页面 | 状态 | 上游数据 | 下游回流 | 风险触发 | 统计指标 | 归档材料 |
|---|---|---|---|---|---|---|---|---|---|
| 学年学期与参数 | 建立→发布→冻结 | 教学计划 | 学年学期 | planned | 学校参数 | 计划/排课/考试 | 周次冲突、未发布 | 当前学期、教学周 | 校历、节次版本 |
| 培养方案与计划 | 编制→审核→发布→调整 | 教学计划 | 培养方案/专业教学计划 | planned | 课程库、专业 | 开课计划、毕业要求 | 学分不平、版本冲突 | 发布率、调整数 | 方案版本、审核记录 |
| 开课与任务 | 开课→生成任务→确认 | 教学计划/教师教学任务 | 开课计划/任务生成/任务确认 | planned | 培养方案、教师 | 排课、教学班 | 缺教师、未确认 | 任务生成率/确认率 | 任务批次、确认单 |
| 排课与发布 | 排课→冲突处理→发布 | 课程与排课 | 排课任务/手动排课/课表发布 | planned | 任务、教室、校历 | 师生课表、考试 | 四类冲突、发布失败 | 已排率、冲突率 | 发布批次、冲突报告 |
| 选课与教学班 | 开批次→选退补→成班 | 选课与教学班 | 选课批次/退补选/教学班管理 | planned | 任务、规则、学生 | 名单、排课、成绩 | 超员、先修/时间冲突 | 选课率、满班率 | 批次、名单、异常单 |
| 教师教学任务 | 确认→授课→交材料 | 教师教学任务 | 教师任务/教学材料 | planned | 教学任务 | 成绩、质量 | 未确认、材料缺失 | 完成率、工作量 | 任务确认、材料目录 |
| 考务安排 | 建批次→排考→监考→发布 | 考务管理 | 考试批次/排考/监考 | planned | 课表、课程、教室 | 成绩任务 | 人员/考场/时间冲突 | 排考率、监考缺口 | 考务批次、考场表 |
| 成绩主线 | 录入→提交→审核→发布 | 成绩管理 | 录入/提交/审核/发布 | planned | 教学班、考试 | 预警、毕业审核 | 未交、退回、职责冲突 | 提交率、发布率 | 成绩任务、发布批次 |
| 成绩更正 | 申请→复核→更正→留痕 | 成绩管理 | 成绩更正/复核 | planned | 已发布成绩 | 成绩结果层 | 自审、无原因、越状态 | 更正率、复核时长 | 更正单、前后值、审计 |
| 学籍注册异动 | 注册→申请→审批→生效 | 学籍管理 | 注册/异动/休复学/转专业 | planned | 学生主档、组织 | 全域资格与范围 | 材料缺失、状态冲突 | 注册率、异动在办 | 注册批次、异动单 |
| 学业预警 | 生成→分派→处置→关闭 | 学业预警 | 预警学生/分派/处置/跟进 | partial/planned | 成绩、学分、缺课 | 学工风险、学生画像 | 超期、升级、重复预警 | 新增数、闭环率 | 处置记录、关闭说明 |
| 教材与资源 | 选用→征订→发放→占用 | 教材与教学资源 | 教材选用/征订/发放/资源 | planned | 课程、任务 | 教学运行 | 缺书、冲突、停用 | 到书率、资源利用率 | 选用单、发放台账 |
| 教学质量 | 评价→问题→整改→复核 | 教学质量 | 听课/评教/质量问题/整改 | planned | 教学任务、课表 | 教师任务、领导汇总 | 事故、低分、超期 | 覆盖率、整改率 | 评价表、整改证据 |
| 统计归档验收 | 汇总→核验→封存→导出 | 教务统计与归档 | 统计/归档中心/验收台账 | planned | 全部终态业务 | 领导驾驶舱 | 口径不一致、材料缺失 | 完整率、归档率 | 归档包、导出审计 |

### 三.7 权限与权责边界

| 角色/边界 | 可见业务 | 数据范围 | 禁止事项 |
|---|---|---|---|
| 教务处管理员 | 全校教务运行、发布、归档 | SCHOOL | 替教师录成绩、绕流程改学籍、删审计 |
| 学院教务员/教学秘书 | 本院计划、任务、完整性审核 | COLLEGE | 跨院、无授权校级发布 |
| 专业负责人 | 本专业方案/计划/预审 | MAJOR | 获得全校教务或跨专业数据 |
| 课程负责人 | 本课程大纲、材料、教学班运行 | COURSE/TEACH_CLASS | 自动获得成绩审核/发布权 |
| 任课教师 | 本人任务、课表、教学班、成绩提交 | TEACH_CLASS | 审核本人提交、发布成绩、改学籍 |
| 监考/阅卷/复核教师 | 分配给本人的任务必要字段 | TASK/SELF | 查看完整成绩或学籍隐私、处理未分配任务 |
| 教学督导 | 质量、听课、评教、巡查 | 授权质量范围 | 修改成绩、学籍、学生核心档案 |
| 学生 | 本人课表、选课、成绩、学籍摘要 | SELF | 查看他人数据或内部风险明细 |

所有角色共同受以下约束：第一层 `module.academicAffairs.enabled` 授权、第二层菜单/按钮权限、当前激活角色、数据范围、真实业务关系、敏感字段权限、审批节点与审计必须合取；前端隐藏不替代后端 403。当前旧模块尚未完整实现上述后端闸门，因此所有承接页保持 partial，权限没有因本轮菜单重构扩大。

### 三.8 路由兼容与后续 canonical 路由原则

本轮不删除、不改写 8 条旧路由，避免已完成页面、书签和审计跳转失效：
...
后续真实开发新页时，统一使用 `/admin/academic-affairs/<domain>/<page>`，旧 `/admin/academic/*` 在新页面完成真实接口、权限、状态机、审计和测试后再 redirect；未完成前不得反向用 planned 空页替换 partial 旧页。

### 三.9 性能与菜单渲染裁定

1. 27 个二级、295 个三级叶子收敛为 12 个二级、87 个菜单页，减少开发者视角一次性 DOM 数量。
2. 31 个详情/指标节点标 `hidden`，不进入侧栏和顶部搜索。
3. `NAV_PLAN` 在模块加载时拍平索引；搜索有 64 条查询缓存和 20 条结果上限；可见树按 planner 视角缓存。
4. planned 无 path、不注册路由；普通业务角色隐藏，管理员/开发环境可见但不可点击。
5. 同一路由多叶子仍按唯一 leafKey 高亮；不使用 path 前缀让多个叶子同时高亮。
6. 后续大表格必须服务端分页；统计接口并行请求，避免模块首屏串行阻塞。

...
- **依据**：
  ...
  - `docs/security/00-等保三级安全基线总册.md`（引用，不重写）
  - `docs/database/00-数据库设计冻结总册.md`（引用，不重写）
  - `docs/api/00-API契约冻结总册.md`（引用，不重写）
...
- **前端安全底座**：`frontend/src/security/`（含 `constants/security.constants.js`：会话策略、敏感分级、上传/导出策略、审计事件类型、CSRF 策略等），详见 `docs/security/00-等保三级安全基线总册.md`（本文档只引用，不重写）。
- **数据库层权限相关表结构**：以 `docs/database/00-数据库设计冻结总册.md` 为准，本文档不重写表结构。

...
> 依据：共享事实底稿 §2；`miniapp/src/services/studentApi.js`、`teacherApi.js`、`request.js`；`miniapp/src/config/roles.config.js`、`env.js`；`miniapp/src/pages/student/service-apply/index.vue`、`weekly-report/index.vue`；引用历史冻结 `docs/05-数据接口权限与安全/api/00-API契约冻结总册.md`、`04-待办审批消息API.md`、`05-岗位实习API.md`

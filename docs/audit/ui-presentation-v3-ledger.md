...
| PORTAL-DATA-01 | `EmploymentView.vue` | 签约材料与就业回访显式列和真实 DTO 不一致 | 材料绑定 `type/fileName/status`；回访绑定 `way/content/time` 并把方式中文业务化 | `ui-presentation-v3-data-contract.test.mjs` | DONE |

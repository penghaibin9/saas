...
| 编号 | 文件（相对项目根） |
|---|---|
| C1 | docs/03-业务模块设计/跨模块融合/_13-现有系统集成事实速查.md |
| C2 | docs/03-业务模块设计/跨模块融合/_13-需求输入-V1.1.md |
| C3 | docs/03-业务模块设计/跨模块融合/13A-13B-数据表与迁移策略草案.md（表结构唯一基线） |
| C4 | docs/03-业务模块设计/跨模块融合/13A-13B-V1范围冻结表.md（范围唯一基线） |
| C5 | docs/05-数据接口权限与安全/database/00-数据库设计冻结总册.md（公共字段/横切表/枚举原则） |

**13A 专属文档（A 组，docs/03-业务模块设计/跨模块融合/）**：
...
**13B 专属文档（B 组，docs/03-业务模块设计/跨模块融合/）**：
...
**source-design 参考件（S 组，docs/08-历史记录与归档/source-design/，按阶段选读）**：
...
- **允许修改范围**：仅 `docs/03-业务模块设计/跨模块融合/`（新增 13A 契约冻结文档 + 勘误记录）。
...
- **允许修改范围**：仅 `docs/03-业务模块设计/跨模块融合/`（13B 契约冻结文档 + 勘误对照附录）。

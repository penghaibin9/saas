...
- 生产代码修改：**否**
- 允许目录外修改：**0**
- 当前阶段：**浏览器累计 546 / 870；学工与毕业设计契约已收口；岗位实习审计输入未变化；定向页面族回归工具已落盘并通过隔离夹具**
...
- `tools/run-page-family-regression.mjs`
- `tools/page-family-regression-report.md`

能力：

- `student-affairs-key` 固定 11 页 / 33 次；
- `student-affairs-all` 固定 15 页 / 45 次；
- `graduation` 固定 8 页 / 24 次；
- 支持自定义目录前缀或精确 HTML；
- 复制临时原型快照并生成临时 Manifest，再调用正式浏览器执行器；
- 不修改仓库原始 Manifest 和 HTML；
- 总 Manifest 数量、页面族固定数量、HTML 存在性和空筛选均 fail-fast；
- 默认不复制 `.git`、`node_modules`，执行后自动清理临时快照。

已完成：

- 工具 JavaScript 语法检查 PASS；
- 隔离同构夹具准确选择毕业设计 8 页；
- 临时 Manifest 生成、临时快照复制、参数转发、子执行器调用和清理链路 PASS。

尚未完成：完整真实 PR 分支上的学工 33 次和毕业设计 24 次 Chrome / Chromium 回归。因此累计仍为 546 / 870，不增加。
...
- PR 当前全部变更文件均位于 `docs/ui/teacher-pc-v2-html/`；
...
- 毕业设计机器审计和页面族定向回归能力均已写入总 Manifest 与专项记录。
...
当前执行容器不能解析 GitHub 域名，无法直接克隆或下载完整分支；GitHub 连接器可以读取和维护仓库。当前 Actions 没有可下载的分支归档 Artifact，且现有生产 CI 不执行 docs 内冻结工具。工具落盘不等于最终 HEAD 已经通过。

## 下一步

1. 在可材料化完整分支快照的环境中运行一致性检查和毕业设计机器审计，重点确认旧 4 页无残留引用、新 4 页无孤儿或断链；
2. 使用页面族执行器完成学工 11 页 `11 × 3 = 33` 次回归；
3. 使用页面族执行器完成毕业设计现行 8 页 `8 × 3 = 24` 次回归；

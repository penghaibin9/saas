  ...
  <div class="gd-workbench">
    ...
    <template v-else>
      <section v-if="!my.hasData" class="gd-empty-notice">
        ...
        <p>{{ my.note || my.message || '管理员尚未将你加入毕业设计批次。建档后各步骤会显示办理状态和待办。' }}</p>
      </section>
      <template v-else>
      ...
      <section class="gd-focus" aria-label="当前要做">
        <div class="gd-focus__flag">当前要做</div>
        <div class="gd-focus__body">
          <span>第 {{ currentTask.order }} 步 · {{ currentTask.title }}</span>
          <strong>{{ currentTask.status }}</strong>
          <p>{{ currentTask.action ? currentTask.detail || currentTask.description : currentTask.detail || currentTask.description }}</p>
          <small v-if="!currentTask.action">{{ currentTask.actionHint || '当前无需提交，完成条件满足后系统会自动开放下一步。' }}</small>
        </div>
        <button v-if="currentTask.action" class="sp-btn" :disabled="busy" @click="openCurrentTask">{{ currentTask.action }}</button>
      </section>
        ...
        <li v-for="step in steps" :key="step.key" class="gd-step" :class="'is-' + step.tone" :data-step-key="step.key">
          ...
          <article>
            ...
            <div v-if="step.files?.length" class="gd-files">
              <span>{{ step.key === 'proposal' ? '开题材料：' : step.key === 'final' ? '论文成果：' : '相关材料：' }}</span>
              <span v-for="file in step.files" :key="file.fileId" class="gd-file-actions">
                <button class="gd-file" :disabled="busy" @click="openMaterialReader(file)">在线查看</button>
      ...
      <section v-if="hasPeerWork" class="gd-peer sp-panel">
        <h2>成果互查</h2>
        <p class="sp-muted">互查任务只开放学校分配的正式定稿。文件仅可在站内阅读，不能下载，也不能查看对方其他材料。</p>
        <div v-for="p in (peer.toReview || [])" :key="'r-' + p.id" class="gd-peer__item">
          ...
          <p v-else-if="p.taskValid !== false" class="gd-step__comment">该任务没有可访问的正式定稿，请联系管理员核对任务。</p>
        ...
        <div v-for="p in (peer.myRectify || [])" :key="'x-' + p.id" class="gd-peer__item">
          ...
          <div v-if="p.attachmentsList?.length" class="gd-files">
            <span v-for="file in p.attachmentsList" :key="file.fileId" class="gd-file-actions">
              <button class="gd-file" :disabled="busy || file.canPreview === false" @click="openPeerReader(p, file)">查看学校分配的定稿</button>
            </span>
          </div>
...
const finalFiles = computed(() => (final.value.items || []).flatMap((item) => item.attachmentsList || []))
...
const steps = computed(() => [
  ...
  },
  {
    ...
    primary: ['PENDING_CONFIRM', 'CHANGE_PENDING'].includes(taskbook.value.status)
  },
  {
    ...
    primary: Boolean(proposal.value.canSubmit)
  },
  {
  ...
  },
  {
    ...
    primary: midterm.value.status === 'RECTIFYING'
  },
  {
    ...
    primary: Boolean(final.value.canSubmitDraft || final.value.canSubmitFinal)
  },
  {
...
async function openCurrentTask() {
  if (!currentTask.value?.key) return
  await handleAction(currentTask.value.key)
  await nextTick()
  document.querySelector(`[data-step-key="${currentTask.value.key}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}
...
.gd-focus { display:grid; grid-template-columns:auto minmax(0,1fr) auto; gap:14px; align-items:center; margin:14px 0 8px; padding:15px 17px; border:1px solid rgba(22,119,255,.2); border-radius:12px; background:linear-gradient(120deg,rgba(22,119,255,.08),#fff); box-shadow:0 12px 26px -24px rgba(22,119,255,.8); }.gd-focus__flag { align-self:start; padding:4px 8px; border-radius:999px; background:var(--sp-primary); color:#fff; font-size:11px; font-weight:700; white-space:nowrap; }.gd-focus__body { min-width:0; display:grid; gap:3px; }.gd-focus__body > span { color:var(--sp-primary); font-size:12px; font-weight:600; }.gd-focus__body > strong { color:#1d2129; font-size:16px; }.gd-focus__body p { margin:0; color:#4e5969; font-size:13px; line-height:1.55; }.gd-focus__body small { color:#86909c; font-size:12px; }.gd-focus .sp-btn { margin:0; white-space:nowrap; }
.gd-rule-note { margin:0 0 16px; color:#86909c; font-size:12px; line-height:1.6; }
...
@media (max-width: 760px) { .gd-hero { display:block; }.gd-hero .sp-btn { margin-top:12px; }.gd-summary { grid-template-columns:repeat(2, 1fr); }.gd-focus { grid-template-columns:1fr; }.gd-focus__flag { width:max-content; }.gd-step { grid-template-columns:40px minmax(0, 1fr); }.gd-step article { margin-left:12px; }.gd-step__head { display:block; }.gd-step__head .sp-tag { margin-top:8px; }.gd-topic-filters { grid-template-columns:1fr; }.gd-topic-filters__scope { grid-column:auto; } }
</style>

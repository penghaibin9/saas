    ...
    <template v-else>
      <div v-if="actionable" class="w75__action">
        ...
        <div v-if="actionable.resubmitTarget?.kind === 'PROPOSAL'" class="w75__form">
          ...
          <label>选题背景与研究依据<textarea v-model.trim="proposalForm.background" :disabled="busy" /></label>
          <label>研究方案与进度计划<textarea v-model.trim="proposalForm.plan" :disabled="busy" /></label>
          <label>预期成果<textarea v-model.trim="proposalForm.outcome" :disabled="busy" /></label>
          <label>替换主文档（可选）<input type="file" accept=".pdf,.doc,.docx,.zip" :disabled="busy" @change="pickFile('proposal', $event)" /></label>
          <GraduationUploadStatus v-if="proposalUpload" v-model:file="proposalUpload" class="w75__upload" :locked="busy" preview-label="提交前预览" @preview="openPending(proposalUpload)" />
          <button class="sp-btn" :disabled="busy || !proposalForm.background || !proposalForm.plan || (proposalUpload && !graduationUploadReady(proposalUpload))" @click="resubmitProposal">
            整改完成，重新提交开题报告
          </button>
        </div>
        ...
        <div v-else-if="actionable.resubmitTarget?.kind === 'FINAL' && actionable.resubmitTarget.finalType" class="w75__form">
          ...
          <label>修改后主文档<input type="file" accept=".pdf,.doc,.docx,.zip" :disabled="busy" @change="pickFile('final', $event)" /></label>
          <GraduationUploadStatus v-if="finalUpload" v-model:file="finalUpload" class="w75__upload" :locked="busy" preview-label="提交前预览" @preview="openPending(finalUpload)" />
          <button class="sp-btn" :disabled="busy || !graduationUploadReady(finalUpload)" @click="resubmitFinal">
...
async function load() {
  ...
  try {
    ...
    if (previousAction && previousAction !== String(actionable.value?.id || '')) {
...
async function pickFile(kind, event) {
  ...
  busy.value = true
  const upload = kind === 'proposal' ? proposalUpload : finalUpload
  upload.value = null
  try {
    upload.value = await graduationW75Api.upload(file)
  } catch (e) {
...
async function checkedUpload(upload) {
  const expected = { ...upload.value }
  const fresh = await readGraduationUpload(expected, (fileId) => fileSdk.metadata(fileId))
  if (String(upload.value?.fileId || '') !== String(expected.fileId)) {
    throw new Error('待重交文件已变化，请核对后重新提交')
  }
  upload.value = fresh
...
async function resubmitProposal() {
  if (
    ...
    (proposalUpload.value && !graduationUploadReady(proposalUpload.value))
  ) return
  ...
  busy.value = true
  try {
    const fresh = proposalUpload.value ? await checkedUpload(proposalUpload) : null
...
async function resubmitFinal() {
  ...
  if (busy.value || target?.kind !== 'FINAL' || !target.finalType || !graduationUploadReady(finalUpload.value)) return
  const finalType = target.finalType
  const expectedVersion = materialVersion(finalType === '定稿' ? 'THESIS_FINAL' : 'THESIS_DRAFT')
  busy.value = true
  try {
    const fresh = await checkedUpload(finalUpload)
    await graduationW75Api.submitFinal({
      finalType,
      attachments: [fresh.fileId],
      expectedVersion,
    })
    finalUpload.value = null
...
function openPending(file) {
  if (!graduationUploadReady(file) || file?.canPreview !== true || busy.value) return

<template>
  <div class="gsm-page">
    <header class="gsm-hero">
      <div><p>毕业设计材料库</p><h1>查看材料状态并按退回意见重交</h1><span>论文、作品和源代码等大文件请使用本页面上传；完成后的文件版本和审核状态刷新后仍可恢复。</span></div>
      <div><RouterLink class="gsm-btn" to="/graduation">返回毕业设计工作台</RouterLink><button class="gsm-btn gsm-btn--primary" :disabled="loading || busy" @click="load">刷新材料</button></div>
    </header>
    ...
    <template v-else-if="library">
      <section class="gsm-summary"><article><span>材料总数</span><strong>{{ library.total || 0 }}</strong></article><article><span>缺失</span><strong class="is-danger">{{ count('MISSING') }}</strong></article><article><span>待审核</span><strong class="is-warning">{{ reviewCount('PENDING') }}</strong></article><article><span>被退回</span><strong class="is-danger">{{ reviewCount('RETURNED') }}</strong></article><article><span>已通过</span><strong class="is-success">{{ reviewCount('APPROVED') }}</strong></article></section>
      <section class="gsm-profile"><div><span>姓名</span><strong>{{ library.studentName }}</strong></div><div><span>学号</span><strong>{{ library.studentNo }}</strong></div><div><span>课题</span><strong>{{ library.topicTitle || '待分配' }}</strong></div><div><span>指导教师</span><strong>{{ library.advisorName || '待分配' }}</strong></div></section>
        ...
        <article v-for="material in group.items" :key="material.materialId" class="gsm-item">
          ...
          <details v-if="material.versions?.length" class="gsm-history">
            ...
            <div v-for="item in material.versions" :key="item.fileVersionId" class="gsm-version">
              <span>v{{ item.versionNo }} · {{ item.fileName }}</span><span>{{ item.isCurrent ? '当前版本' : '历史只读' }} · {{ item.submittedAt || '-' }}</span>
              <button v-if="item.fileId" class="gsm-btn" :disabled="busy" @click="openReader(material, item)">{{ item.isCurrent ? '查看' : '查看历史版' }}</button>
          ...
          <div v-if="canUpload(material)" class="gsm-upload">
            ...
            <GraduationUploadStatus
              v-if="pending[material.materialCode]"
              v-model:file="pending[material.materialCode]"
              class="gsm-preflight"
              :locked="busy"
              @preview="openPendingReader(pending[material.materialCode])"
            />
            <button class="gsm-btn gsm-btn--primary" :disabled="busy || !graduationUploadReady(pending[material.materialCode])" @click="submit(material)">{{ material.businessStatus === 'RETURNED' ? '重交新版本' : '提交材料版本' }}</button>
            <small>文件需先完成安全扫描；状态会自动更新，提交前可站内预览本次文件。</small>
          </div>
        </article>
      </section>
    ...
    <StudentDocumentViewer v-if="readerFile" :file="readerFile" :versions="readerVersions" :load-preview="loadReaderPreview" :read-only="readerFile.isCurrent === false" @download="download" @close="closeReader" />
  </div>
</template>
...
async function selectFile(material, event) {
  const file = event.target.files?.[0]
  event.target.value = ''
  if (!file || busy.value || !canUpload(material)) return
  busy.value = true
  delete pending[material.materialCode]
  try {
    pending[material.materialCode] = await portalApi.uploadGraduationMaterial(file)
  } catch (e) {
    ui.notify(e?.message || '文件上传失败')
  } finally { busy.value = false }
}
async function submit(material) {
  if (busy.value || !canUpload(material)) return
  const file = pending[material.materialCode]
  if (!graduationUploadReady(file)) return
  const target = Object.freeze({ materialCode: material.materialCode, version: material.version, file: { ...file } })
  busy.value = true
  try {
    const fresh = await readGraduationUpload(target.file, (fileId) => fileSdk.metadata(fileId))
    if (String(pending[target.materialCode]?.fileId || '') !== String(target.file.fileId)) {
      throw new Error('待提交文件已变化，请核对后重新提交')
    }
    pending[target.materialCode] = fresh
    if (!graduationUploadReady(fresh)) throw new Error('文件尚未通过安全检查，请等待或重新检查文件状态')
    await portalApi.submitGraduationMaterial(target.materialCode, target.file.fileId, target.version)
    delete pending[target.materialCode]
    ui.notify('材料新版本已提交')
    await load()
  } catch (e) {
    ui.notify(e?.message || '材料提交失败')
  } finally { busy.value = false }
}

function readerKey(item = {}) { return String(item.fileVersionId || item.versionId || item.fileId || '') }
function versionsFor(material) {
  const current = material.currentVersion ? { ...material.currentVersion, fileVersionId: material.currentVersion.fileVersionId || material.currentVersionId, isCurrent: true } : null
  const all = [current, ...(material.versions || []).map((item) => ({ ...item, isCurrent: item.isCurrent === true || readerKey(item) === String(material.currentVersionId || '') }))].filter(Boolean)
  const seen = new Set()
  return all.filter((item) => { const key = readerKey(item); if (!key || seen.has(key)) return false; seen.add(key); return true })
}
function openReader(material, file) { readerVersions.value = versionsFor(material); const normalized = readerVersions.value.find((item) => readerKey(item) === readerKey(file)) || file; readerFile.value = { ...normalized, isCurrent: normalized.isCurrent !== false && readerKey(normalized) === String(material.currentVersionId || readerKey(material.currentVersion)) } }
function openPendingReader(file) { if (busy.value || !graduationUploadReady(file) || !file.canPreview) return; readerVersions.value = []; readerFile.value = { ...file, temporaryPreview: true, isCurrent: true, versionNo: '待提交' } }
function closeReader() { readerFile.value = null; readerVersions.value = [] }
async function loadReaderPreview(file, options) { if (file.temporaryPreview || file.temporary) return fileSdk.fetchPreviewBlob(file.fileId, options); const ticket = await portalApi.issueGraduationMaterialTicket(file.fileId, 'preview'); return fileSdk.fetchPreviewBlobFrom(ticket, options) }
async function download(file) { busy.value = true; try { if (file.temporaryPreview || file.temporary) await fileSdk.download(file.fileId, file.fileName); else await portalApi.downloadGraduationMaterial(file.fileId, file.fileName) } catch (e) { ui.notify(e?.message || '文件暂不可下载') } finally { busy.value = false } }

      ...
      <section v-for="group in library.groups || []" :key="group.name" class="gsm-group">
        ...
        <article v-for="material in group.items" :key="material.materialId" class="gsm-item">
          ...
          <div v-if="canUpload(material)" class="gsm-upload">
            ...
            <div v-if="pending[material.materialCode]" class="gsm-preflight">
              <p>待提交：{{ pending[material.materialCode].fileName }} · {{ pending[material.materialCode].statusText }}</p>
              <button v-if="pending[material.materialCode].readyForBusiness && pending[material.materialCode].canPreview" class="gsm-btn" :disabled="busy" @click="openPendingReader(pending[material.materialCode])">预览我将提交的文件</button>
            </div>
            <button class="gsm-btn gsm-btn--primary" :disabled="busy || !pending[material.materialCode] || !pending[material.materialCode].readyForBusiness" @click="submit(material)">{{ material.businessStatus === 'RETURNED' ? '重交新版本' : '提交材料版本' }}</button>
            <small>文件需先完成安全扫描；提交前可站内预览本次文件，确认无误后再提交。</small>
          </div>
        </article>
      </section>
...
async function selectFile(material, event) { const file = event.target.files?.[0]; event.target.value = ''; if (!file) return; busy.value = true; try { const uploaded = await portalApi.uploadGraduationMaterial(file); pending[material.materialCode] = uploaded; ui.notify(uploaded.readyForBusiness ? '上传完成，可以先预览确认后再提交' : '上传完成，等待安全扫描后再提交') } catch (e) { ui.notify(e?.message || '文件上传失败') } finally { busy.value = false } }
...
function openPendingReader(file) { readerVersions.value = []; readerFile.value = { ...file, temporaryPreview: true, isCurrent: true, versionNo: '待提交' } }

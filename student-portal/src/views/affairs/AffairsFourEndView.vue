<template>
  <div class="sp-page">
    <nav class="sp-tabs">
      <button v-for="item in tabs" :key="item.key" class="sp-tab" :class="{ 'is-active': tab === item.key }" @click="tab = item.key">
        {{ item.label }}
      </button>
    </nav>
    ...
    <template v-else>
      <div v-if="tabError" class="domain-error">
        <strong>当前业务暂不可用</strong>
        <span>{{ tabError }}</span>
        <button class="sp-btn sp-btn--ghost" @click="reload">重新加载</button>
      </div>
      ...
      <div v-if="tab === 'leave'" class="two">
        <section class="sp-card">
          ...
          <div class="form-grid">
            ...
            <label><span>开始日期</span><input v-model="leaveForm.startTime" type="date" class="sp-inp" /></label>
            <label><span>结束日期</span><input v-model="leaveForm.endTime" type="date" class="sp-inp" /></label>
            <label class="wide"><span>请假事由</span><textarea v-model.trim="leaveForm.reason" class="sp-inp" placeholder="不少于5字" /></label>
          </div>
        ...
        <section class="sp-card">
          ...
          <article v-for="item in (leave.items || [])" :key="item.leaveId" class="record">
            <div class="record-head">
              <div><strong>{{ item.leaveTypeLabel || item.leaveType }}</strong><div class="sp-muted">{{ fmt(item.startTime) }} 至 {{ fmt(item.endTime) }} · {{ item.days }}天</div><div v-if="item.returnReason" class="warn">退回意见：{{ item.returnReason }}</div></div>
              <StatusTag :text="item.affairsStatusLabel || item.statusLabel || item.status" tone="default" />
            </div>
            <div class="actions">
              <button v-if="item.canResubmit" class="sp-btn sp-btn--ghost" :disabled="busy" @click="editLeave(item)">修改后重提</button>
              <button v-if="item.canCancel" class="sp-btn" :disabled="busy" @click="cancelLeave(item)">申请销假</button>
              <button v-if="item.canExtend" class="sp-btn sp-btn--ghost" :disabled="busy" @click="openExtend(item)">申请续假</button>
            </div>
            <div v-if="extendId === item.leaveId" class="inline-form">
              <label><span>新结束日期</span><input v-model="extendForm.newEndTime" type="date" class="sp-inp" /></label>
              <label><span>续假事由</span><textarea v-model.trim="extendForm.reason" class="sp-inp" /></label>
              <button class="sp-btn" :disabled="busy || extendForm.reason.length < 5" @click="submitExtend(item)">提交续假</button>
            </div>
          </article>
        </section>
      </div>
      ...
      <section v-else-if="tab === 'aid'" class="sp-card">
        ...
        <p class="sp-muted">申请、家庭经济信息、本人确认、工作流和老师待办在同一事务提交；任何一步失败都会整体回滚。</p>
        <div class="form-grid compact">
          ...
          <label><span>家庭成员数</span><input v-model.number="aidForm.memberCount" type="number" min="1" class="sp-inp" /></label>
          <label><span>家庭年收入（元）</span><input v-model.number="aidForm.annualIncome" type="number" min="0" class="sp-inp" /></label>
          <label><span>家庭债务（元）</span><input v-model.number="aidForm.debt" type="number" min="0" class="sp-inp" /></label>
          <label class="wide"><span>特殊情况标签</span><input v-model.trim="aidForm.specialTags" class="sp-inp" placeholder="低保、孤残、重大疾病等，用逗号分隔" /></label>
          <label class="wide"><span>困难情况说明（10-500字）</span><textarea v-model.trim="aidForm.statement" class="sp-inp" /></label>
        </div>
        ...
        <article v-for="item in (aid.items || [])" :key="item.applyId" class="record">
          ...
          <div v-if="item.canObject" class="inline-form"><textarea v-model.trim="aidObjections[item.applyId]" class="sp-inp" placeholder="公示异议理由（至少5字）" /><button class="sp-btn" :disabled="busy || (aidObjections[item.applyId] || '').length < 5" @click="submitAidObjection(item)">提交异议</button></div>
      ...
      <section v-else-if="tab === 'funding'" class="sp-card">
        ...
        <div class="form-grid compact">
          <label><span>类型</span><select v-model="fundForm.projectType" class="sp-inp"><option value="SCHOLARSHIP">奖学金</option><option value="GRANT">助学金</option></select></label>
          <label><span>开放批次</span><select v-model="fundForm.batchId" class="sp-inp"><option value="">请选择</option><option v-for="b in filteredFundingBatches" :key="b.batchId" :value="b.batchId">{{ b.batchName || b.schoolYear }}</option></select></label>
          <label class="wide"><span>申请理由（至少5字）</span><textarea v-model.trim="fundForm.statement" class="sp-inp" /></label>
        </div>
        ...
        <article v-for="item in (funding.items || [])" :key="item.applicationId" class="record">
          <div class="record-head"><div><strong>{{ fundingLabel(item.projectType) }}</strong><div class="sp-muted">{{ item.statusLabel || item.status }}</div><div v-if="item.returnReason" class="warn">意见：{{ item.returnReason }}</div></div><StatusTag :text="item.hasPendingAppeal ? '申诉待复核' : (item.statusLabel || item.status)" :tone="item.hasPendingAppeal ? 'warn' : 'default'" /></div>
          <div class="actions"><button v-if="item.canResubmit" class="sp-btn sp-btn--ghost" :disabled="busy" @click="editFunding(item)">修改后重提</button></div>
          <div v-if="item.canAppeal" class="inline-form"><textarea v-model.trim="fundAppeals[item.applicationId]" class="sp-inp" placeholder="公示申诉理由（至少5字）" /><button class="sp-btn" :disabled="busy || (fundAppeals[item.applicationId] || '').length < 5" @click="submitFundingAppeal(item)">提交申诉</button></div>
        </article>
      </section>
      ...
      <section v-else-if="tab === 'dorm'" class="sp-card">
        ...
        <template v-else>
          ...
          <button class="sp-btn sp-btn--ghost" :disabled="busy" @click="loadDormOptions">申请调宿</button>
        </template>
        <div v-if="dormForm.visible" class="inline-form dorm-form">
          ...
          <select v-model="dormForm.bedId" class="sp-inp" :disabled="!dormForm.roomId"><option value="">选择床位</option><option v-for="b in dormBeds.filter(x => x.status === 'VACANT' && !x.isCurrent)" :key="b.bedId" :value="b.bedId">{{ b.bedNo }}</option></select>
          <textarea v-model.trim="dormForm.reason" class="sp-inp" placeholder="调宿原因（至少5字）" />
          <button class="sp-btn" :disabled="busy || !dormForm.bedId || dormForm.reason.length < 5" @click="submitDormTransfer">提交调宿申请</button>
        </div>
        <div class="section-title">调宿申请记录</div>
        <AutoTable :rows="dormTransfers" empty="暂无调宿申请" />
      </section>

      <!-- 处分申诉 -->
      <section v-else-if="tab === 'discipline'" class="sp-card">
        <div class="sp-panel__head">处分申诉</div>
        <p class="sp-muted">本入口用于处分生效后的申诉，不冒充处分决定前的陈述申辩。具体期限以学校处分决定书与规章为准。</p>
        <StateBlock v-if="!discipline.activeCount" type="empty" text="暂无生效处分记录" />
        <article v-for="item in (discipline.items || [])" :key="item.caseId" class="record">
          <div class="record-head"><div><strong>{{ item.discTypeLabel || item.discType }}</strong><div class="sp-muted">{{ fmt(item.effectiveAt) }} 生效</div><div v-if="item.appealReviewOpinion" class="sp-muted">复核意见：{{ item.appealReviewOpinion }}</div></div><StatusTag :text="appealLabel(item.appealStatus)" tone="default" /></div>
          <div v-if="item.canAppeal" class="inline-form"><textarea v-model.trim="disciplineAppeals[item.caseId]" class="sp-inp" placeholder="处分申诉理由（至少5字）" /><button class="sp-btn" :disabled="busy || (disciplineAppeals[item.caseId] || '').length < 5" @click="submitDisciplineAppeal(item)">提交处分申诉</button></div>
        </article>
      </section>

      <!-- 心理自评 -->
      <div v-else-if="tab === 'psy'" class="two">
        <section class="sp-card"><div class="sp-panel__head">心理健康自评</div><p class="sp-muted">结果仅本人与心理中心按授权查看，系统不作自动诊断。</p><StateBlock v-if="!(psy.questions || []).length" type="empty" text="暂无自评问卷" /><div v-for="(q, index) in (psy.questions || [])" :key="q.key" class="question"><strong>{{ index + 1 }}. {{ q.text }}</strong><div class="options"><button v-for="(option, oi) in q.options" :key="oi" class="seg" :class="{ on: psyAnswers[q.key] === oi }" @click="psyAnswers[q.key] = oi">{{ option }}</button></div></div><button class="sp-btn" :disabled="busy || !psyComplete" @click="submitPsy">提交自评</button></section>
        <section class="sp-card"><div class="sp-panel__head">历史测评</div><AutoTable :rows="psyHistory.items || []" empty="暂无测评记录" /></section>
      </div>
      ...
      <section v-else-if="tab === 'activity'">
        ...
        <div class="two">
          <section class="sp-card"><div class="sp-panel__head">可报名活动</div><StateBlock v-if="!(activities.available || []).length" type="empty" text="暂无可报名活动" /><article v-for="item in (activities.available || [])" :key="item.activityId" class="record"><div class="record-head"><div><strong>{{ item.activityName }}</strong><div class="sp-muted">{{ fmt(item.startAt) }} · {{ item.location || '未填写地点' }}</div></div><button v-if="!item.mySignupStatus || item.mySignupStatus === 'CANCELLED'" class="sp-btn" :disabled="busy" @click="enroll(item)">报名</button><StatusTag v-else :text="item.mySignupStatus" tone="default" /></div></article></section>
          <section class="sp-card"><div class="sp-panel__head">入账明细</div><StateBlock v-if="!(secondClass.items || []).length" type="empty" text="暂无已确认入账记录" /><article v-for="item in (secondClass.items || [])" :key="`${item.activityId}-${item.grantedAt}`" class="record"><div class="record-head"><div><strong>{{ item.remark || '第二课堂记录' }}</strong><div class="sp-muted">{{ creditLabel(item.creditType) }} · {{ fmt(item.grantedAt) }}</div></div><div><strong>+{{ item.creditValue }}</strong><button class="link" @click="openCreditAppeal(item)">记错申诉</button></div></div></article><button class="sp-btn sp-btn--ghost" @click="openCreditAppeal(null)">有活动缺记？提交缺记申诉</button></section>
        </div>
    ...
    <div v-if="modal.type" class="mask" @click.self="closeModal">
      <section class="sp-card modal">
        <div class="sp-panel__head">{{ modal.title }}</div>
        <p v-if="modal.notice" class="warn">{{ modal.notice }}</p>
        <template v-if="modal.type === 'leave'"><div class="form-grid"><label><span>类型</span><select v-model="modal.form.leaveType" class="sp-inp"><option value="PERSONAL">事假</option><option value="SICK">病假</option><option value="HOME">探亲假</option><option value="HOSPITAL">住院假</option><option value="GOOUT">外出</option><option value="OTHER">其他</option></select></label><label><span>开始日期</span><input v-model="modal.form.startTime" type="date" class="sp-inp" /></label><label><span>结束日期</span><input v-model="modal.form.endTime" type="date" class="sp-inp" /></label><label class="wide"><span>事由</span><textarea v-model.trim="modal.form.reason" class="sp-inp" /></label></div></template>
        <template v-else-if="modal.type === 'aid'"><div class="form-grid"><label><span>等级</span><select v-model="modal.form.applyLevel" class="sp-inp"><option value="GENERAL">一般困难</option><option value="DIFFICULT">困难</option><option value="SPECIAL">特别困难</option></select></label><label><span>成员数</span><input v-model.number="modal.form.memberCount" type="number" class="sp-inp" /></label><label><span>年收入</span><input v-model.number="modal.form.annualIncome" type="number" class="sp-inp" /></label><label><span>债务</span><input v-model.number="modal.form.debt" type="number" class="sp-inp" /></label><label class="wide"><span>情况说明</span><textarea v-model.trim="modal.form.statement" class="sp-inp" /></label></div></template>
        <template v-else-if="modal.type === 'funding'"><label><span>申请理由</span><textarea v-model.trim="modal.form.statement" class="sp-inp" /></label></template>
        <template v-else-if="modal.type === 'credit'"><p class="sp-muted">{{ modal.form.activityName ? `涉及活动：${modal.form.activityName}` : '缺记申诉可不指定活动' }}</p><select v-model="modal.form.claimCreditType" class="sp-inp"><option value="SECOND_CLASS">第二课堂</option><option value="MORAL">德育积分</option><option value="VOLUNTEER_HOUR">志愿时长</option></select><input v-model.number="modal.form.claimValue" type="number" class="sp-inp" placeholder="主张数值（选填）" /><textarea v-model.trim="modal.form.reason" class="sp-inp" placeholder="申诉理由（至少5字）" /></template>
        <div class="actions"><button class="sp-btn sp-btn--ghost" :disabled="busy" @click="closeModal">取消</button><button class="sp-btn" :disabled="busy" @click="submitModal">保存并提交</button></div>
      </section>
    </div>
  </div>
</template>
...
const ui = useUiStore()
const tabs = [
  { key: 'leave', label: '请假销假' }, { key: 'aid', label: '困难认定' },
  { key: 'funding', label: '奖学金与助学金' }, { key: 'dorm', label: '我的宿舍' },
  { key: 'discipline', label: '处分申诉' }, { key: 'psy', label: '心理自评' },
  { key: 'activity', label: '活动与第二课堂' }, { key: 'talk', label: '谈心谈话' }
]
const tab = ref('leave')
...
const leave = ref({ items: [] }); const aid = ref({ items: [] }); const funding = ref({ items: [] })
const dorm = ref({}); const discipline = ref({ items: [] }); const psy = ref({ questions: [] })
const psyHistory = ref({ items: [] }); const activities = ref({ available: [], mine: [] }); const talk = ref({ items: [] })
const aidBatches = ref([]); const fundingBatches = ref([]); const secondClass = ref({ items: [], byType: [] })
const creditAppeals = ref([]); const dormTransfers = ref([])

const leaveForm = reactive({ leaveType: 'PERSONAL', startTime: '', endTime: '', reason: '' })
const extendId = ref(''); const extendForm = reactive({ newEndTime: '', reason: '' })
const aidForm = reactive({ batchId: '', applyLevel: 'GENERAL', memberCount: null, annualIncome: null, debt: null, specialTags: '', statement: '', confirm: false })
const fundForm = reactive({ projectType: 'SCHOLARSHIP', batchId: '', statement: '', confirm: false })
const aidObjections = reactive({}); const fundAppeals = reactive({}); const disciplineAppeals = reactive({}); const psyAnswers = reactive({})
const dormBuildings = ref([]); const dormRooms = ref([]); const dormBeds = ref([])
const dormForm = reactive({ visible: false, buildingId: '', roomId: '', bedId: '', reason: '' })
const modal = reactive({ type: '', title: '', notice: '', item: null, form: {} })

const validLeave = computed(() => leaveForm.startTime && leaveForm.endTime && leaveForm.reason.length >= 5)
const validAid = computed(() => aidForm.batchId && aidForm.statement.length >= 10 && aidForm.confirm)
const validFunding = computed(() => fundForm.batchId && fundForm.statement.length >= 5 && fundForm.confirm)
...
async function reload() {
  ...
  const tasks = {
    leave: portalApi.affairsLeave(), aid: portalApi.affairsAid(), funding: portalApi.affairsFunding(),
    discipline: portalApi.affairsDiscipline(), psy: portalApi.affairsPsyQuestions(), psyHistory: portalApi.affairsPsyHistory(),
    activities: portalApi.affairsActivitiesMy(), aidBatches: portalApi.affairsAidBatches(), fundingBatches: portalApi.affairsFundingBatches(),
    talk: portalApi.affairsTalk(), dorm: portalApi.affairsDorm(), secondClass: affairsFourEndApi.secondClassReport(),
    creditAppeals: affairsFourEndApi.myCreditAppeals(), dormTransfers: affairsFourEndApi.myDormTransfers()
  }
  ...
  results.forEach((result, index) => {
    ...
    if (result.status === 'rejected') errors[key === 'psyHistory' ? 'psy' : key === 'aidBatches' ? 'aid' : key === 'fundingBatches' ? 'funding' : key === 'secondClass' || key === 'creditAppeals' ? 'activity' : key === 'dormTransfers' ? 'dorm' : key] = result.reason?.message || '数据加载失败'
      ...
      if (key === 'leave') leave.value = value
      else if (key === 'aid') aid.value = value
      else if (key === 'funding') funding.value = value
      else if (key === 'discipline') discipline.value = value
      else if (key === 'psy') psy.value = value
      else if (key === 'psyHistory') psyHistory.value = value
      else if (key === 'activities') activities.value = value
      else if (key === 'aidBatches') aidBatches.value = value.items || []
      else if (key === 'fundingBatches') fundingBatches.value = value.items || []
      else if (key === 'talk') talk.value = value
      else if (key === 'dorm') dorm.value = value
      else if (key === 'secondClass') secondClass.value = { items: [], byType: [], ...value }
      else if (key === 'creditAppeals') creditAppeals.value = value.items || []
      else if (key === 'dormTransfers') dormTransfers.value = value.items || []
    }
  })
  loading.value = false
}

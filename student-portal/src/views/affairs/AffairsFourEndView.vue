  ...
  <div class="sp-page">
    ...
    <template v-else>
      ...
      <div v-if="tab === 'leave'" class="two">
        ...
        <section class="sp-card">
          ...
          <article v-for="item in (leave.items || [])" :key="item.leaveId" class="record">
            ...
            <div v-if="extendId === item.leaveId" class="inline-form"><label><span>新结束日期</span><input v-model="extendForm.newEndTime" type="date" class="sp-inp" /></label><label><span>续假事由</span><textarea v-model.trim="extendForm.reason" class="sp-inp" /></label><button class="sp-btn" :disabled="busy || extendForm.reason.length < 5" @click="submitExtend(item)">提交续假</button></div>
          </article>
        </section>
      </div>
      ...
      <section v-else-if="tab === 'dorm'" class="sp-card">
        ...
        <template v-else><div class="bed-grid"><div><span>楼栋</span><strong>{{ dorm.myBed?.building }}</strong></div><div><span>房间</span><strong>{{ dorm.myBed?.room }}</strong></div><div><span>床位</span><strong>{{ dorm.myBed?.bedNo }}</strong></div><div><span>入住时间</span><strong>{{ fmt(dorm.myBed?.occupiedAt) }}</strong></div></div><p class="sp-muted">已有床位时只能提交正式调宿申请；审批完成前原床保持不变。</p><button class="sp-btn sp-btn--ghost" :disabled="busy" @click="loadDormOptions">申请调宿</button></template>
        <div v-if="dormForm.visible" class="inline-form dorm-form"><select v-model="dormForm.buildingId" class="sp-inp" @change="loadRooms"><option value="">选择目标楼栋</option><option v-for="b in dormBuildings" :key="b.buildingId" :value="b.buildingId">{{ b.buildingName }}（空{{ b.vacantBeds }}）</option></select><select v-model="dormForm.roomId" class="sp-inp" :disabled="!dormForm.buildingId" @change="loadBeds"><option value="">选择房间</option><option v-for="r in dormRooms" :key="r.roomId" :value="r.roomId">{{ r.floorNo }}层 {{ r.roomNo }}（空{{ r.vacantBeds }}）</option></select><select v-model="dormForm.bedId" class="sp-inp" :disabled="!dormForm.roomId"><option value="">选择床位</option><option v-for="b in dormBeds.filter(x => x.status === 'VACANT' && !x.isCurrent)" :key="b.bedId" :value="b.bedId">{{ b.bedNo }}</option></select><textarea v-model.trim="dormForm.reason" class="sp-inp" placeholder="调宿原因（至少5字）" /><button class="sp-btn" :disabled="busy || !dormForm.bedId || dormForm.reason.length < 5" @click="submitDormTransfer">提交调宿申请</button></div>
      ...
      <section v-else-if="tab === 'discipline'" class="sp-card">
        <div class="sp-panel__head">处分申诉</div><p class="sp-muted">本入口用于处分生效后的申诉，不冒充处分决定前的陈述申辩。具体期限以学校处分决定书与规章为准。</p><StateBlock v-if="!discipline.activeCount" type="empty" text="暂无生效处分记录" />
        <article v-for="item in (discipline.items || [])" :key="item.caseId" class="record"><div class="record-head"><div><strong>{{ item.discTypeLabel || item.discType }}</strong><div class="sp-muted">{{ fmt(item.effectiveAt) }} 生效</div><div v-if="item.appealReviewOpinion" class="sp-muted">复核意见：{{ item.appealReviewOpinion }}</div></div><StatusTag :text="appealLabel(item.appealStatus)" tone="default" /></div><div v-if="item.canAppeal" class="inline-form"><textarea v-model.trim="disciplineAppeals[item.caseId]" class="sp-inp" placeholder="处分申诉理由（至少5字）" /><button class="sp-btn" :disabled="busy || (disciplineAppeals[item.caseId] || '').length < 5" @click="submitDisciplineAppeal(item)">提交处分申诉</button></div></article>
      </section>
...
const validLeave = computed(() => leaveForm.startTime && leaveForm.endTime && leaveForm.reason.length >= 5); const validAid = computed(() => aidForm.batchId && aidForm.statement.length >= 10 && aidForm.confirm); const validFunding = computed(() => fundForm.batchId && fundForm.statement.length >= 5 && fundForm.confirm); const filteredFundingBatches = computed(() => fundingBatches.value.filter((x) => x.projectType === fundForm.projectType)); const psyComplete = computed(() => (psy.value.questions || []).length > 0 && (psy.value.questions || []).every((q) => psyAnswers[q.key] != null))
watch(filteredFundingBatches, (list) => { if (!list.some((x) => x.batchId === fundForm.batchId)) fundForm.batchId = list[0]?.batchId || '' }); watch(aidBatches, (list) => { if (!list.some((x) => x.batchId === aidForm.batchId)) aidForm.batchId = list[0]?.batchId || '' })
const fmt = (value) => (value || '').slice(0, 10); const fundingLabel = (type) => ({ SCHOLARSHIP: '奖学金', GRANT: '助学金', WORK_STUDY: '勤工助学', LOAN: '助学贷款', TUITION_REDUCTION: '学费减免', TEMPORARY_AID: '临时补助' }[type] || type); const appealLabel = (status) => ({ SUBMITTED: '申诉已提交', REVIEWING: '复核中', UPHELD: '维持原处分', REVISED: '处分已变更', REVOKED: '处分已撤销' }[status] || status || '未申诉'); const creditLabel = (type) => ({ SECOND_CLASS: '第二课堂', MORAL: '德育积分', VOLUNTEER_HOUR: '志愿时长' }[type] || type); const notifyError = (e, fallback) => ui.notify(e?.message || fallback)
...
async function applyLeave() { const result = await run(() => portalApi.affairsServiceApply({ serviceKey: 'LEAVE', ...leaveForm }), '请假已提交', '请假提交失败'); if (result.ok) leaveForm.reason = '' }
...
async function submitExtend(item) { const result = await run(() => affairsFourEndApi.extendLeave(item.leaveId, item.version, extendForm.newEndTime, extendForm.reason), '续假申请已提交', '续假提交失败'); if (result.ok) extendId.value = '' }
...
async function submitAidObjection(item) { const result = await run(() => portalApi.affairsAidObjection({ applyId: item.applyId, reason: aidObjections[item.applyId] }), '异议已提交并进入老师待办', '异议提交失败'); if (result.ok) aidObjections[item.applyId] = '' }
async function submitFunding() { const result = await run(() => portalApi.affairsFundingApply({ batchId: fundForm.batchId, statement: fundForm.statement, confirm: true }), '奖助申请已提交', '提交失败'); if (result.ok) { fundForm.statement = ''; fundForm.confirm = false } }
...
async function submitFundingAppeal(item) { const result = await run(() => portalApi.affairsFundingAppeal({ applicationId: item.applicationId, reason: fundAppeals[item.applicationId] }), '申诉已提交并进入老师待办', '申诉提交失败'); if (result.ok) fundAppeals[item.applicationId] = '' }
...
async function submitDormTransfer() { const result = await run(() => affairsFourEndApi.submitDormTransfer(dormForm.bedId, dormForm.reason), '调宿申请已提交', '调宿提交失败'); if (result.ok) Object.assign(dormForm, { visible: false, buildingId: '', roomId: '', bedId: '', reason: '' }) }
async function submitDisciplineAppeal(item) { const result = await run(() => portalApi.affairsDisciplineAppeal({ caseId: item.caseId, reason: disciplineAppeals[item.caseId] }), '处分申诉已提交', '申诉提交失败'); if (result.ok) disciplineAppeals[item.caseId] = '' }
...
async function submitUpdatedAndResubmit({ update, resubmit, success }) {
  busy.value = true
  try {
    const updated = await update(); modal.item = { ...(modal.item || {}), ...(updated || {}), version: updated?.version ?? modal.item?.version }
    try { await resubmit(modal.item.version) } catch (e) { modal.notice = `修改已保存，但重新提交失败：${e?.message || '请保留当前内容后重试'}`; notifyError(e, '重新提交失败'); return false }
    ui.notify(success); setTimeout(() => closeModal(), 0); await reload(); return true
  } catch (e) { notifyError(e, '保存修改失败'); return false } finally { busy.value = false }
}
async function submitModal() {
  ...
  else if (modal.type === 'aid') { const id = modal.item.applyId; await submitUpdatedAndResubmit({ update: () => affairsFourEndApi.updateReturnedAid(id, { ...modal.form, version: modal.item.version }), resubmit: (version) => affairsFourEndApi.resubmitAid(id, version), success: '认定申请已修改并重新提交' }) }
  ...
  else if (modal.type === 'credit') { if ((modal.form.reason || '').length < 5) return ui.notify('申诉理由至少5字'); const result = await run(() => affairsFourEndApi.submitCreditAppeal(modal.form), '积分申诉已提交', '申诉提交失败'); if (result.ok) closeModal() }
}
onMounted(reload)
</script>

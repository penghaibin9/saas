...
async function submitDormAction() { if (!validDormAction.value) return ui.notify(dorm.value.hasBed ? '请选择目标床位并填写5-300字调宿原因' : '请选择床位'); const copy = dorm.value.hasBed ? `确认提交调宿？\n目标：${selectedDormTarget.value}\n审批完成前原床保持不变。` : `确认床位？\n目标：${selectedDormTarget.value}\n确认后床位将为你预留，变更须走正式调宿。`; if (!window.confirm(copy)) return; const result = await run(() => dorm.value.hasBed ? affairsFourEndApi.submitDormTransfer(dormForm.bedId, dormForm.reason) : affairsFourEndApi.selfSelectDormBed(dormForm.bedId), dorm.value.hasBed ? '调宿申请已提交' : '床位已确认', dorm.value.hasBed ? '调宿提交失败' : '选床失败', 'dorm'); if (result.ok) closeDormForm() }

...
async function confirmConsent() { if (!consentDetail.value || busy.value || compliance.value.historyMode) return; if (!await systemConfirm({ title:'确认实习知情书', message:`确认已阅读正文版本 ${consentDetail.value.contentVersion} 并同意按学校实习要求执行？`, confirmText:'确认已阅读' })) return; busy.value = true; try { await api.consentConfirm(consentDetail.value.id,{expectedVersion:consentDetail.value.version,contentVersion:consentDetail.value.contentVersion,contentHash:consentDetail.value.contentHash,deviceDigest:deviceDigest()}); ui.notify('知情确认已完成'); await load(); selectedConsent.value=null; consentDetail.value=null } catch(e){ ui.notify(e?.message||'确认失败，请刷新后重试'); if(String(e?.code||'').includes('CONFLICT')) await refreshConsent() } finally { busy.value=false } }
async function rejectConsent() { if (!consentDetail.value || busy.value || compliance.value.historyMode) return; const reason=(await systemPrompt({ title:'填写无法确认原因', message:'请说明无法确认的具体原因。', placeholder:'至少 5 个字', minLength:5, confirmText:'提交原因' })||'').trim(); if(reason.length<5) return; busy.value=true; try{ await api.consentReject(consentDetail.value.id,{expectedVersion:consentDetail.value.version,reason}); ui.notify('拒绝原因已提交'); await load(); selectedConsent.value=null; consentDetail.value=null }catch(e){ ui.notify(e?.message||'提交失败，请重试'); if(String(e?.code||'').includes('CONFLICT')) await refreshConsent() }finally{busy.value=false} }

    ...
    <template v-else>
      ...
      <template v-else>
        ...
        <section v-if="compliance.currentTask" class="sp-notice" :class="compliance.passed ? 'is-success' : 'is-warning'">
          <div>
            <strong>{{ compliance.passed ? '当前无阻断项' : `当前待办：${compliance.currentTask.label}` }}</strong>
            <p class="sp-muted">{{ compliance.currentTask.reason || '请按学校要求完成当前事项。' }}</p>
          </div>
            ...
            <section v-for="item in visibleItems" :key="item.code" class="sp-card compliance-item">
              <div class="compliance-item__head">
                <div>
                  ...
                  <span v-if="item.required" class="required">必需</span>
                </div>
                <StatusTag :text="item.statusLabel" :tone="tone(item.status)" />
              </div>
        ...
        <template v-else-if="section === 'consent'">
          <div class="two">
            <section class="sp-card">
              <div class="sp-panel__head">知情确认任务</div>
              <StateBlock v-if="!consents.length" type="empty" text="暂无知情确认任务" />
              <div v-else class="task-list">
                <button v-for="item in consents" :key="item.id" class="task" :class="{'is-active':selectedConsent?.id===item.id}"
                  @click="selectConsent(item)">
                  <div>
                    <strong>{{ item.consentType === 'GUARDIAN' ? '监护人知情确认' : '学生知情确认' }}</strong>
                    <small>正文版本 {{ item.contentVersion || '—' }}</small>
                  </div>
            ...
            <section class="sp-card">
              ...
              <template v-else-if="selectedConsent.consentType === 'GUARDIAN'">
                ...
                <dl class="kv">
                  <div><dt>关系</dt><dd>{{ selectedConsent.participantRelation || '监护人' }}</dd></div>
                  <div><dt>联系方式</dt><dd>{{ selectedConsent.contactMasked || '已脱敏' }}</dd></div>
                  <div><dt>状态</dt><dd>{{ consentStatus(selectedConsent.status) }}</dd></div>
                </dl>
              </template>
              <template v-else-if="consentDetail">
                ...
                <div v-if="consentDetail.status === 'PENDING'" class="actions">
        ...
        <template v-else-if="section === 'safety'">
          <div class="two">
            <section class="sp-card">
              ...
              <div v-else class="task-list">
                <button v-for="course in safetyCourses" :key="course.id" class="task" :class="{'is-active':selectedCourse?.id===course.id}"
                  @click="selectCourse(course)">
                  <div>
                    <strong>{{ course.title }}</strong>
                    <small>版本 {{ course.courseVersion }} · {{ course.requiredMinutes }} 分钟</small>
                  </div>
            ...
            <section class="sp-card">
              ...
              <template v-else-if="courseDetail">
                <div class="course-meta">
                  <span>要求 {{ courseDetail.requiredMinutes }} 分钟</span>
                  <span>已学习 {{ trustedMinutes }} 分钟</span>
                  <span>剩余尝试 {{ courseDetail.remainingAttempts }}</span>
                </div>
...
const sections = [
  { key: 'overview', label: '合规总览' },
  { key: 'consent', label: '知情确认' },
  { key: 'safety', label: '安全教育' }
]
const compliance = ref({ items: [], blockers: [], warnings: [] })
const consents = ref([])
const selectedConsent = ref(null)
const consentDetail = ref(null)
const safetyCourses = ref([])
const safetyCompletions = ref([])
...
function consentStatus(status) {
  return ({ PENDING: '待确认', VALID: '已确认', REJECTED: '已拒绝', REVOKED: '已作废', SUPERSEDED: '已更新', NOT_APPLICABLE: '不适用' })[status] || status
}
function toneConsent(status) { return status === 'VALID' ? 'success' : ['REJECTED', 'REVOKED'].includes(status) ? 'danger' : 'warn' }
...
async function load() {
  ...
  try {
    const [c, cs, courses, completions] = await Promise.all([
      ...
      portalApi.internshipConsents(),
      portalApi.internshipSafetyCourses(),
      portalApi.internshipSafetyCompletions()
    ])
...
async function selectConsent(item) {
  selectedConsent.value = item
  consentDetail.value = null
  if (item.consentType !== 'STUDENT') return
  try {
    let detail = await portalApi.internshipConsentDetail(item.id)
    if (detail.status === 'PENDING') detail = await portalApi.internshipConsentView(item.id)
...
async function confirmConsent() {
  ...
  busy.value = true
  try {
    await portalApi.internshipConsentConfirm(consentDetail.value.id, {
...
async function rejectConsent() {
  if (!consentDetail.value || busy.value) return
  const reason = (window.prompt('请说明无法确认的具体原因（至少5字）') || '').trim()
  if (reason.length < 5) return ui.notify('拒绝原因至少5个字')
  busy.value = true
  try {
...
async function selectCourse(course) {
  ...
  try { courseDetail.value = await portalApi.internshipSafetyDetail(course.id) }
...
async function commitSafety() {
  ...
  try {
    await portalApi.internshipSafetyCommit(completion.value.id, {
      expectedVersion: completion.value.version,
...
async function submitSafety() {
  ...
  busy.value = true
  try {
    await portalApi.internshipSafetySubmit(selectedCourse.value.id, {
      expectedVersion: completion.value.version,
...
<style scoped>
.hero { display: flex; align-items: center; gap: 24px; margin-bottom: 16px; }
.hero > div:first-child { flex: 1; }
.hero h2 { margin: 4px 0 6px; font-size: 22px; }
.hero__eyebrow { color: var(--pri); font-size: 13px; font-weight: 600; }
.hero__score { text-align: right; }
.hero__score strong { display: block; font-size: 30px; color: var(--pri); }
.hero__score span { color: var(--t3); font-size: 12px; }
.sp-notice { display:flex;justify-content:space-between;align-items:center;gap:16px;padding:16px 18px;border:1px solid var(--line);border-radius:12px;background:#fff;margin-bottom:16px; }
.sp-notice.is-warning { border-color:#FFD591;background:#FFFBE6; }
.sp-notice.is-success { border-color:#B7EB8F;background:#F6FFED; }
.sp-notice p { margin:6px 0 0; }
.compliance-grid { display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px; }
.compliance-item { display:flex;flex-direction:column;gap:12px; }
.compliance-item__head { display:flex;align-items:center;justify-content:space-between;gap:12px; }
.required { margin-left:8px;padding:2px 6px;border-radius:5px;background:#FFF1F0;color:#CF1322;font-size:11px; }
.two { display:grid;grid-template-columns:1fr 1.35fr;gap:18px;align-items:start; }
.task-list { display:flex;flex-direction:column;gap:8px; }
.task { all:unset;box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border:1px solid var(--line);border-radius:10px;cursor:pointer; }
.task:hover,.task.is-active { border-color:var(--pri);background:var(--pri-50); }
.task strong,.task small { display:block; }
.task small { margin-top:4px;color:var(--t3); }
.kv { display:grid;grid-template-columns:1fr 1fr;gap:12px; }
.kv div { padding:12px;background:#F7F8FA;border-radius:8px; }
.kv dt { color:var(--t3);font-size:12px; }
.kv dd { margin:4px 0 0;color:var(--t1); }
.doc-meta { color:var(--t3);font-size:12px;margin-bottom:10px; }
.doc { white-space:pre-wrap;line-height:1.8;color:var(--t2);max-height:420px;overflow:auto;padding:16px;background:#F7F8FA;border-radius:10px; }

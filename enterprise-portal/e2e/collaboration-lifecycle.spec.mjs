...
async function installApi(page){
  ...
  await page.route('**/api/v1/internship/enterprise-portal/**',async route=>{
    ...
    if(path.endsWith('/auth/browser-invite/accept'))return route.fulfill({contentType:'application/json',body:JSON.stringify(ok({accessToken:'collab-access',tokenType:'Bearer',expiresIn:1800,user:{userId:'db-101',realName:'企业HR',userType:'ENTERPRISE_MENTOR'},context:{tenantId:'1',tenantCode:'CSZY',schoolName:'长沙职业技术学院',memberId:'11',companyId:'21',memberRole:'HR'}}))})
    if(path.endsWith('/auth/browser-refresh'))return route.fulfill({contentType:'application/json',body:JSON.stringify(ok({accessToken:'collab-refresh-access',tokenType:'Bearer',expiresIn:1800}))})
    if(path.endsWith('/auth/browser-logout'))return route.fulfill({contentType:'application/json',body:JSON.stringify(ok({invalidated:true}))})

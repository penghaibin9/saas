...
test('A02-9 evaluation tasks are collaboration-gated, server filtered and paged',()=>{
  assert.match(page,/pageSize=50/)
  assert.match(page,/collabReady/)
  assert.match(page,/batchId:batchId\.value/)
  assert.match(page,/evaluationTasks\(\{batchId:batchId\.value,status:tab\.value==='ALL'\?'':tab\.value,page:page\.value,pageSize\}\)/)
  assert.match(page,/上一页/);assert.match(page,/下一页/)
  assert.doesNotMatch(page,/items\.value\.filter/)
})

test('A02-9 five canonical dimensions require explicit integer 0-100 scores instead of default 90',()=>{
  ...
  assert.match(page,/五项评分均需明确填写 0–100 的整数分/)
...
test('A02-9 evaluation payload cannot forge canonical actor source or audit fields',()=>{
  const submitBlock=page.match(/const payload=\{[\s\S]*?\n    \}/)?.[0]||''

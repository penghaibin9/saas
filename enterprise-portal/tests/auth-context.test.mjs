...
test('invite token and tenantCode lock school/company/campaign context through A01 adapter',()=>{assert.match(invite,/邀请 token \+ tenantCode 在服务端校验/);assert.match(invite,/前端不能选择 companyId/);assert.match(api,/auth\/invite\/inspect/);assert.match(api,/auth\/invite\/accept/)})

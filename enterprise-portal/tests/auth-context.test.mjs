...
test('invite token and tenantCode lock school company and campaign context through A01 adapter',()=>{assert.match(invite,/同一 tenantCode \+ 邀请 token 在服务端校验/);assert.match(invite,/不会再次提交 campaignId 或 companyId/);assert.match(api,/auth\/invite\/inspect/);assert.match(api,/auth\/invite\/accept/);assert.match(api,/validatedInvite/)})

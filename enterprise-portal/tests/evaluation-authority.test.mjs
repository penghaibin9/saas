...
test('enterprise evaluation reuses facade and cannot forge canonical audit fields',()=>{assert.match(page,/ENTERPRISE_ONLINE/);assert.match(api,/evaluations\/\$\{id\}\/submit/);for(const forbidden of [/sourceType\s*:/,/actorMemberId\s*:/,/recordedAt\s*:/,/enterpriseContactId\s*:/])assert.doesNotMatch(page,forbidden)})

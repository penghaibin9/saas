...
test('enterprise evaluation UI cannot forge canonical audit fields while facade remains unfrozen',()=>{assert.match(page,/ENTERPRISE_ONLINE/);assert.match(api,/submitEvaluation: \(\) => unavailableFacade\('企业评价提交'\)/);assert.doesNotMatch(api,/evaluation-tasks\/\$\{id\}\/submit/);for(const forbidden of [/sourceType\s*:/,/actorMemberId\s*:/,/recordedAt\s*:/,/enterpriseContactId\s*:/])assert.doesNotMatch(page,forbidden)})

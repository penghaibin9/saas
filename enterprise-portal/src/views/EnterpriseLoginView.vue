...
async function submit(){
  if(contexts.value.length&&!selectedMemberId.value){error.value='请选择要进入的企业上下文';return}
...
<template><main class="auth"><section class="ep-card card"><div class="logo">跃科</div><h1>企业协同中心</h1><p>企业账号必须绑定学校租户；不提供开放式企业自注册。</p><form @submit.prevent="submit"><label>学校编码<input v-model.trim="form.tenantCode" class="ep-input" autocomplete="organization" required></label><label>手机号或登录账号<input v-model.trim="form.loginName" class="ep-input" autocomplete="username" required></label><label>密码<input v-model="form.password" type="password" class="ep-input" autocomplete="current-password" required></label><fieldset v-if="contexts.length" class="context-picker"><legend>选择企业上下文</legend><button v-for="item in contexts" :key="item.memberId" type="button" class="context-option" :class="{selected:selectedMemberId===item.memberId}" @click="selectedMemberId=item.memberId"><span><strong>{{ item.companyName }}</strong><small>{{ item.memberRole }}</small></span><b>{{ selectedMemberId===item.memberId?'已选择':'选择' }}</b></button><p>企业范围仍由 A01 使用所选 memberId 在服务端重新校验；前端不会直接提交 companyId 作为 Authority。</p></fieldset><div v-if="error" class="ep-error">{{ error }}</div><button class="ep-btn ep-btn-primary submit" :disabled="loading||(contexts.length&&!selectedMemberId)">{{ loading?'登录中…':'登录' }}</button></form><p class="invite">首次激活请使用学校发送的企业邀请链接。</p></section></main></template>

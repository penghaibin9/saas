...
<template><section class="ep-page"><div class="ep-page-head"><div><h1 class="ep-title">实习学生</h1><p class="ep-subtitle">这里只展示学校已正式落岗的实习学生；企业管理员和 HR 查看本企业范围，企业导师仅查看学校授权的学生。</p></div></div><div class="ep-toolbar"><input v-model.trim="keyword" class="ep-input" placeholder="搜索学生/岗位/导师" @keyup.enter="applySearch"><select v-model="status" class="ep-select" @change="changeStatus"><option value="ACTIVE">在岗中</option><option value="COMPLETED">已结束</option><option value="ALL">全部正式实习</option></select><button class="ep-btn" :disabled="!collabReady" @click="applySearch">搜索</button></div><div v-if="error" class="ep-error">{{ error }}</div><div v-if="loading" class="ep-card ep-empty">正在读取正式实习记录…</div><div v-else-if="!items.length" class="ep-card ep-empty">当前范围内暂无正式实习学生</div><div v-else class="list"><article v-for="item in items" :key="item.internship_id||item.internshipId||item.id" class="ep-card student"><div class="main"><div class="avatar">{{ String(item.name||'学').slice(0,1) }}</div><div><h3>{{ item.name }}</h3><p>{{ item.position_name||item.positionName||'岗位' }}</p></div></div><dl><div><dt>指导教师</dt><dd>{{ item.advisor_name||item.advisorName||'—' }}</dd></div><div><dt>企业导师</dt><dd>{{ item.mentor_name||item.mentorName||'—' }}</dd></div><div><dt>实习状态</dt><dd><span class="ep-tag ok">{{ item.status_label||item.statusLabel||item.status||'正式实习' }}</span></dd></div><div><dt>实习周期</dt><dd>{{ item.period||`${item.start_date||item.startDate||'—'} ~ ${item.end_date||item.endDate||'—'}` }}</dd></div></dl><RouterLink v-if="item.evaluation_task_id||item.evaluationTaskId" to="/evaluations" class="ep-btn eval">评价任务</RouterLink></article><div class="pager ep-card"><button class="ep-btn" :disabled="page<=1" @click="previousPage">上一页</button><span>{{ pageInfo }}</span><button class="ep-btn" :disabled="!hasNext" @click="nextPage">下一页</button></div></div></section></template>

...
<template>
  <section class="ep-page">
    ...
    <div v-if="receipt" class="submit-receipt ep-card" role="status"><strong>✓ 企业评价已提交学校审核</strong><span>评价 #{{ receipt.id }} · v{{ receipt.version }} · 安置快照 #{{ receipt.placementSnapshotId || '—' }}</span><button type="button" @click="receipt=null">关闭</button></div>
...
.submit-receipt{display:flex;align-items:center;gap:12px;margin-bottom:14px;padding:12px 14px;border-color:var(--ok-bd);background:var(--ok-bg);color:var(--ok-fg)}.submit-receipt span{font-size:11px}.submit-receipt button{margin-left:auto;border:0;background:transparent;color:inherit}

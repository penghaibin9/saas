...
<template>
  <section class="ep-page">
    ...
    <div class="tasks ep-card"><h2 class="ep-section-title">今天要做什么</h2><div v-if="!tasks.length" class="ep-empty">当前没有可确认的待处理任务。</div><div v-for="task in tasks" :key="task.key||task.title" class="task"><div><b>{{ task.title }}</b><p>{{ task.description }}</p></div><RouterLink v-if="task.href" :to="task.href" class="ep-btn">{{ task.actionLabel||'去处理' }}</RouterLink></div></div>
    <div class="history ep-card"><h2 class="ep-section-title">历史招聘季</h2><div v-if="!history.length" class="ep-muted">暂无可确认的历史招聘季。</div><div v-for="item in history" :key="item.id||item.campaignId" class="history-row"><div><b>{{ item.name||item.campaignName||`招聘季 #${item.id||item.campaignId}` }}</b><span>{{ item.status }}</span></div><div>{{ item.period||`${item.startAt||'—'} ~ ${item.endAt||'—'}` }}</div></div></div>
  </section>
</template>

...
const context=useEnterpriseContextStore(); const data=ref(null); const loading=ref(true); const error=ref('')
const metrics=computed(()=>data.value?.metrics||{})
const tasks=computed(()=>data.value?.tasks||[])
onMounted(async()=>{ try{ data.value=await enterpriseInternshipApi.dashboard() }catch(e){error.value=e.message||'首页加载失败'}finally{loading.value=false} })
</script>
<template>
...
<style scoped>.metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}.metric{padding:18px}.metric strong{display:block;font-size:26px;margin-bottom:7px}.metric span{color:var(--t3);font-size:13px}.tasks{padding:20px}.task{display:flex;justify-content:space-between;gap:20px;align-items:center;padding:14px 0;border-top:1px solid var(--line)}.task:first-of-type{border-top:0}.task p{margin:5px 0 0;color:var(--t3);font-size:13px}.task a{text-decoration:none}@media(max-width:900px){.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}</style>

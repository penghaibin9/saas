...
<template>
  <section class="ep-page">
    ...
    <div class="history ep-card"><h2 class="ep-section-title">历史招聘季</h2><div v-if="!history.length" class="ep-muted">暂无可确认的历史招聘季；A01 企业招聘季列表未返回前不构造历史记录。</div><div v-for="item in history" :key="item.id" class="history-row"><div><b>{{ item.name||item.campaignName }}</b><span>{{ item.status }}</span></div><div>{{ item.period||`${item.startAt||'—'} ~ ${item.endAt||'—'}` }}</div></div></div>
  </section>
</template>

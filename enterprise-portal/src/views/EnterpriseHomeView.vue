...
const context=useEnterpriseContextStore(); const data=ref(null); const history=ref([]); const loading=ref(true); const error=ref('')
const metrics=computed(()=>data.value?.metrics||{})
const tasks=computed(()=>data.value?.tasks||[])
onMounted(async()=>{ try{const [dashboardResult,historyResult]=await Promise.allSettled([enterpriseInternshipApi.dashboard(),enterpriseInternshipApi.campaignHistory()]);if(dashboardResult.status==='fulfilled')data.value=dashboardResult.value;else throw dashboardResult.reason;if(historyResult.status==='fulfilled')history.value=Array.isArray(historyResult.value)?historyResult.value:(historyResult.value?.items||[])}catch(e){error.value=e.message||'首页加载失败'}finally{loading.value=false} })
</script>
<template>
  <section class="ep-page">
    ...
    <div class="history ep-card"><h2 class="ep-section-title">历史招聘季</h2><div v-if="!history.length" class="ep-muted">暂无历史招聘季，或 A01 历史读接口尚未开放。</div><div v-for="item in history" :key="item.id" class="history-row"><div><b>{{ item.name }}</b><span>{{ item.status }}</span></div><div>{{ item.period||`${item.start_at||item.startAt||'—'} ~ ${item.end_at||item.endAt||'—'}` }}</div></div></div>
  </section>
</template>

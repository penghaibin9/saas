...
onMounted(async()=>{
  ...
  }catch(e){error.value=e.message||'企业资料加载失败'}finally{loading.value=false}
})
...
<template>
  <section class="ep-page">
    <div class="ep-page-head"><div><h1 class="ep-title">企业资料</h1><p class="ep-subtitle">维护学生能看到的企业公开介绍；学校准入结论始终只读。</p></div><button class="ep-btn ep-btn-primary" :disabled="loading||saving" @click="save">{{ saving?'保存中…':'保存公开资料' }}</button></div>
    ...
    <div class="grid">
      <div class="ep-card panel">
        ...
        <div class="logo-row">
          ...
          <div><label class="ep-btn file-button">选择 Logo<input type="file" accept="image/png,image/jpeg,image/webp" @change="chooseLogo"></label><p class="ep-muted">建议方形 PNG/JPG/WebP，≤5MB。保存时通过正式文件中心安全上传，不要求企业填写文件编号。</p></div>
        </div>
...
<style scoped>.grid{display:grid;grid-template-columns:minmax(0,2fr) minmax(280px,1fr);gap:16px}.panel{padding:20px}label{display:flex;flex-direction:column;gap:7px;font-size:13px;color:var(--t2);margin-bottom:15px}.ep-input,.ep-textarea{width:100%}.logo-row{display:grid;grid-template-columns:96px 1fr;gap:16px;align-items:center;margin-bottom:20px}.logo-preview{width:96px;height:96px;border:1px dashed var(--line);border-radius:14px;background:var(--page);display:grid;place-items:center;overflow:hidden;color:var(--t3);font-size:12px}.logo-preview img{width:100%;height:100%;object-fit:contain;background:#fff}.file-button{display:inline-flex;width:max-content;position:relative;overflow:hidden;margin:0 0 8px}.file-button input{position:absolute;inset:0;opacity:0;cursor:pointer}dl{margin:0}dl div{display:flex;justify-content:space-between;gap:20px;padding:13px 0;border-bottom:1px solid var(--line)}dt{color:var(--t3)}dd{margin:0;font-weight:600}.notice{padding:12px 14px;background:var(--ok-bg);color:var(--ok-fg);border-radius:8px;margin-bottom:14px}@media(max-width:900px){.grid{grid-template-columns:1fr}}@media(max-width:560px){.logo-row{grid-template-columns:1fr}.logo-preview{width:84px;height:84px}}</style>

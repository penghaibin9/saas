...
<template><div class="detail"><div v-if="loading" class="ep-empty">正在加载申请材料…</div><div v-else-if="error" class="ep-error">{{ error }}</div><div v-else-if="!data" class="ep-empty">从左侧选择一名报名学生查看申请详情。</div><template v-else><header class="head"><div><div class="name-line"><h2>{{ data.name }}</h2><span class="ep-tag">第{{ data.volunteerNo??'—' }}志愿</span><span v-if="data.studentVerified!==false" class="ep-tag ok">学校实名学生</span></div><p>{{ data.major||'专业待列表投影' }} · {{ data.grade||'年级待列表投影' }} · {{ data.positionName||'申请岗位' }}</p></div><div v-if="data.matchPercent" class="match">{{ data.matchPercent }}%<small>岗位匹配</small></div></header><div v-if="!campaignWritable" class="history-only">当前招聘写能力未由服务端显式开放：材料保持可读，新的企业 Decision 保持禁用。</div><div v-if="groupStatus==='LOCKED' && decisionStatus==='ACCEPT_INTENT'" class="locked"><strong>拟接收 · 等待学校最终确认</strong><span>学校审核截止时间：{{ data.teacherConfirmDeadline||'—' }}</span></div><div v-if="groupStatus==='NEEDS_REVISION' && (releaseReason==='TEACHER_CONFIRM_TIMEOUT' || data.acceptIntentReleased)" class="released">学校未在确认期限内完成最终确认，本次拟接收已释放，申请状态可能发生变化。历史 Decision 仍保留在处理记录。</div><section class="summary"><h3>学校允许的学生事实</h3><dl><div><dt>专业 / 年级</dt><dd>{{ data.major||'—' }} / {{ data.grade||'—' }}</dd></div><div><dt>申请岗位</dt><dd>{{ data.positionName||'—' }}</dd></div><div><dt>申请时间</dt><dd>{{ data.appliedAt||'—' }}</dd></div></dl></section><section class="summary"><h3>联系方式授权</h3><ContactRevealButton :application-id="id" :contact-policy="data.contactPolicy||{}" /></section><ApplicationMaterialView :material="{...(material||{}),applicationStatement:data.applicationStatement||material?.applicationStatement||''}" /><section class="history"><h3>企业处理记录</h3><div v-if="!(data.decisionHistory||[]).length" class="ep-muted">Decision 历史投影尚未由 A01 facade 返回。</div><div v-for="item in (data.decisionHistory||[])" :key="item.id||`${item.status}-${item.at}`" class="history-row"><span>{{ item.status }}</span><span>{{ item.effectStatus||'—' }}</span><span>{{ item.at||item.createdAt||'—' }}</span></div></section><footer class="sticky"><DecisionActions :application="data" :campaign-writable="campaignWritable" @changed="changed" /></footer></template></div></template>

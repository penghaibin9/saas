<template>
  <div ref="root" class="region-picker" :class="{ 'is-open': open }" @keydown.esc="closePicker">
    ...
    <div v-if="open" class="region-picker__panel" role="dialog" aria-label="选择地区">
      ...
      <footer v-if="!keyword" class="region-picker__footer">
        <span>{{ previewLabel || placeholder }}</span>
...
.region-picker__trigger:disabled { cursor:not-allowed; opacity:.62; background:var(--surface-soft,#f8faff); }
.region-picker__trigger:not(:disabled):hover,.region-picker.is-open .region-picker__trigger { border-color:var(--pri); box-shadow:0 0 0 3px var(--pri-50); }

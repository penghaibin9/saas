...
const lockedAccept=computed(()=>props.application.decisionStatus==='ACCEPT_INTENT' && props.application.volunteerGroupStatus==='LOCKED')
const baseDisabled=computed(()=>Boolean(loading.value)||!props.campaignWritable||lockedAccept.value||props.application.decisionDisabled)
async function decide(status){if(!props.campaignWritable){error.value='招聘季已关闭或当前 RECRUITMENT 权限不可写';return}if(lockedAccept.value){error.value='当前申请已处于拟接收锁定，等待学校最终确认';return}loading.value=status;error.value='';try{await enterpriseInternshipApi.decideApplication(props.application.id||props.application.applicationId,status);emit('changed')}catch(e){error.value=e.message||'处理失败'}finally{loading.value=''}}

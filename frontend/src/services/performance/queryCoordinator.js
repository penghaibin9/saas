...
export function runAdminQuery(key, loader, { ttl = 0, force = false } = {}) {
  ...
  const promise = Promise.resolve()
    ...
    .then((value) => {
      if (Number(ttl) > 0) {
        cache.set(queryKey, { value: cloneValue(value), expiresAt: Date.now() + Number(ttl) })
      }
      return value
    })
    .finally(() => {
...
export function invalidateAdminQueries(match = '') {
...
export function resetAdminQueryCoordinatorForTest() {
  inflight.clear()
  cache.clear()
}

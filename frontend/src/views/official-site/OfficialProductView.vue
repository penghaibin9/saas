<template>
  <div v-if="product" class="yk-site yk-product-page yk-story-site" :style="{ '--yk-product-accent': product.accent, '--yk-product-soft': product.soft }">
    ...
    <footer class="yk-footer"><div class="yk-shell yk-footer-inner"><div><strong>{{ companyName }}</strong><span>职业院校学生全生命周期数字化平台</span></div><div class="yk-footer-links"><router-link to="/">官网首页</router-link><router-link to="/products">产品中心</router-link><router-link to="/platform">平台能力</router-link><router-link to="/privacy">隐私政策</router-link><router-link to="/support">技术支持</router-link><a :href="contact.phoneHref">{{ contact.phone }}</a><a v-if="icpNumber" :href="icpQueryUrl" rel="noopener noreferrer">{{ icpNumber }}</a><span>© {{ year }}</span></div></div></footer>
  </div>
</template>

...
export const NAV_PLAN = [
  ...
  /* ═══════════ 一级②：学工中心 ═══════════
   * 2026-07-10 PC 三级目录重构（refactor(student-affairs)，详见
   * docs/施工记录/13A-学工中心PC前端三级目录重构记录.md 与 docs/modules/13A-学工中心页面树与路由设计.md 附录B）：
   * 原 21 个二级收敛为「12 个学工业务二级 + 1 个数字迎新承接」，按真实学工业务主线落位。
   * 收敛原则：
   *  1) 概览类统一进「学工工作台」；全局统计/归档/档案包统一进「风险预警与统计归档」；
   *  2) 各业务只保留 1 个本业务统计叶子，其余统计切片=页面内统计指标，不再单独挂菜单；
   *  3) 详情页 / 抽屉 / tab / 审批节点 / 归档材料不单独挂菜单（旧叶子归并映射见施工记录映射表）；
   *  4) 已实现旧页面全部保留原路由原入口（/admin/student/*、/admin/campus-service/*、/admin/orientation/*），
   *     本轮只改菜单归属，不动路由、不动权限（moduleCode/permissionKey 均未变，前端隐藏不替代后端校验）；
   *  5) 数字迎新 / 在校服务 = 承接入口（只做入口/摘要/数据回流，不计入学工已完成能力，
   *     对齐 docs/施工记录/历史欠账.md 2026-07-09 冻结口径与 13A-13B 重叠子模块归属裁定）；
   *  6) planned 叶子无 path、不注册路由、普通角色隐藏，杜绝空页面假按钮；施工顺序按 13A B/C/D 任务包。 */
  grp('student-affairs', '学工中心', 'studentAffairs', [
    /* ① 学工工作台（B包）：学工首页/辅导员工作台待施工；原「学工看板」12 个概览叶子收敛为总览页区块；
     * 在校服务 3 个已有页面作为承接快捷入口保留（旧路由不 404，不计入学工完成度） */
    mod('sa-dashboard', '学工工作台', null, [
      ...P('学工总览', '辅导员工作台'),
      I('服务工作台（在校服务·承接）', '/admin/campus-service'),
      I('学生服务（在校服务·承接）', '/admin/campus-service/students'),
      I('服务工单（在校服务·承接）', '/admin/campus-service/work-orders')
    ]),
    /* ② 学生画像：6 个已实现页面；学生360 详情为动态路由（/admin/student/:studentId）不挂菜单；
     * 原「基础信息/家庭与联系人/班级/宿舍/请假/奖助/处分/心理关注摘要/成长档案/数据变更日志」
     * 等三级 = 学生360 页签与「学工摘要」聚合卡（2026-07-07 已实现），不再单独挂菜单 */
    ...
    /* ③ 班级与辅导员（B包）：班级详情/班干部/辅导员/班主任绑定=班级详情页签；
     * 班级风险/请假/宿舍/奖助/活动统计=班级画像统计指标；辅导员考评自原独立二级并入（辅导员维度） */
    mod('sa-classes', '班级与辅导员', null, P(
      '班级列表', '班级学生', '班级画像', '班级材料', '辅导员负责范围', '辅导员考评'
    )),
    /* ④ 宿舍与公寓（B包）：楼栋/房间/床位=房源管理一页三级树；夜不归宿并入宿舍异常（对齐 13A 设计 11-7）；
     * 调宿审批=详情页动作；智能排宿/公寓纪律/文明寝室=能力池暂缓；宿舍维修单走「服务工单」承接；宿舍归档进学工归档 */
    mod('sa-dorm', '宿舍与公寓', null, [
      ...P('房源管理（楼栋/房间/床位）'),
      I('入住管理（现有·宿舍服务）', '/admin/campus-service/dormitory'),
      ...P('调宿与退宿', '宿舍检查', '宿舍异常（含夜不归宿）', '宿舍统计')
    ]),
    /* ⑤ 请假与日常考勤（B包）：长假审批=请假审批节点升级；归寝核验=销假环节动作；外出备案/请假异常=台账筛选；
     * 请假规则配置=低频配置（接平台规则中心，不挂高频菜单）；请假归档进学工归档；
     * 「日常考勤」为本轮新增业务主线规划占位（业务设计冻结前不施工） */
    mod('sa-leave', '请假与日常考勤', null, [
      ...P('请假申请（代录）'),
      I('请假审批', '/admin/campus-service/leave'),
      ...P('销假与续假', '请假台账', '超期未销假', '日常考勤', '请假统计')
    ]),
    /* ⑥ 困难认定与资助（C包）：班级评议/辅导员初审/学院审核/学校复核=认定审核同页分节点（13A 设计 06-5）；
     * 材料上传=申请详情；认定结果/等级调整=困难学生库动作；认定归档进学工归档 */
    mod('sa-difficulty', '困难认定与资助', null, P(
      '认定批次', '认定申请', '认定审核', '公示与异议', '困难学生库', '资助画像', '认定统计'
    )),
    /* ⑦ 奖助勤贷补（C包）：国家/励志/校级奖学金=奖学金管理项目类型（统一资助抽象）；
     * 勤工岗位+报名审核=勤工助学；绿色通道复用迎新 t_green_channel（承接不重建）；奖助归档进学工归档 */
     ...
     * 解除申请/解除审核=处分解除同页；学生申诉/复核处理=申诉复核；处分归档进学工归档 */
    mod('sa-discipline', '违纪处分', null, [
      I('违纪登记（现有）', '/admin/campus-service/discipline'),
      ...P('处分审批', '处分决定与送达', '处分解除', '申诉复核', '违纪台账', '处分统计')
    ]),
    /* ⑨ 心理关注（D包·敏感红线）：心理明细严格受敏感权限控制，普通角色只见「需关注」标记，
     * 不见原始明细（越权 403+审计，导出默认不含心理明细）；咨询预约/记录=谈话转介与回访；
     * 危机预警/干预=危机升级；心理测评=P3 接口位暂缓；心理档案（强权限）进学工归档 */
    mod('sa-mental', '心理关注', null, P(
      '心理关注名单', '心理预警摘要', '谈话转介与回访', '危机升级', '心理权限说明', '心理统计'
    )),
    /* ⑩ 谈心家校（C包）：重点/风险学生谈话=计划与记录筛选；家校联动谈话=联系记录关联；
     * 家校通知/回执/紧急联系/家长授权=能力池暂缓；谈话归档/家校材料归档进学工归档 */
    mod('sa-talks', '谈心家校', null, P(
      '谈话计划', '谈话记录', '重点学生跟进', '家校联系人', '家校联系记录', '谈心统计'
    )),
    /* ⑪ 活动二课与社团（D包）：活动发布/报名审核/签到=活动闭环页内动作；
     * 二课项目库/积分规则/审核/台账/申诉/证书=「第二课堂积分」收敛入口；
     * 社团/学生会/学生干部/党团各收敛 1 个入口；各归档进学工归档 */
    mod('sa-activities', '活动二课与社团', null, P(
      '活动管理', '活动报名与签到', '第二课堂积分', '志愿服务', '社团管理', '学生干部与组织', '党团建设', '活动统计'
    )),
    /* ⑫ 风险预警与统计归档（B包风险 + D包统计归档）：学业/请假/夜不归宿/心理/违纪/经济困难/实习异常
     * 七类风险=风险学生来源筛选（多维合并同列表）；风险跟进/关闭=处置详情动作；
     * 学工统计含学院对比/校级汇总下钻维度；各业务归档在「学工归档+学生档案包」统一收口 */
    mod('sa-risk', '风险预警与统计归档', null, P(
      '风险看板', '风险学生', '风险处置', '风险规则配置', '学工统计', '学工归档', '学生档案包'
    )),
    /* ⑬ 数字迎新（外部承接）：19 个已实现页面原样保留（原路由不 404）；
     * 学工侧只做入口/摘要/数据回流，不在学工内重做、不计入学工完成能力（历史欠账 2026-07-09 冻结口径） */

          ...
          <section v-if="pageKey === 'mapping' && relationBatch" class="mp-card">
            ...
            <div class="mp-card__body">
              ...
              <div v-for="item in relationBatch.candidates" :key="item.candidateId" class="impl-candidate">
                <div><b>{{ item.relationName || '历史业务关系' }} · {{ item.subjectName || item.subjectRef || '对象未记录' }} → {{ item.objectName || item.objectRef || '目标未记录' }}</b><small>{{ item.contextRef ? '业务批次：' + item.contextRef + ' · ' : '' }}来源行 {{ item.sourceRows?.join('、') || '未记录' }} · {{ item.recommendation?.reason || '历史记录未保存候选推荐明细' }}</small></div>
...
export default {
  ...
  methods: {
    ...
    hydrateRelation(raw) { if (!raw?.candidates) return null; raw.candidates.forEach((x) => { const action = x.recommendation?.action; x.uiAction = action === 'REVIEW' ? '' : action || '' }); return raw },
    ...
    async createChange(installation) { const name = await systemPrompt({ title:'填写变更项目名称', message:'请输入本次实施变更项目名称。', defaultValue:`${installation.profileCode} 变更项目`, minLength:1, confirmText:'创建变更项目' }); if (name === null) return; this.action(async () => { this.project = await implementationApi.createChange(installation.id, { projectName: name.trim() }); this.changeAnalysis = null }, '变更项目已创建') },
    ...
    async confirmCheck(check) { const comment = await systemPrompt({ title:'填写责任确认说明', message:'该说明将写入实施审计记录。', defaultValue:'已核实并接受该项风险', minLength:1, confirmText:'确认责任' }); if (comment === null) return; this.action(() => implementationApi.confirmCheck(this.project.id, check.code, { projectVersion: this.project.version, comment, confirmText: '确认责任' }), '责任确认已记录') },

  ...
  <SystemWorkspaceFrame title="访问排查" subtitle="选择老师、功能与具体对象，查看服务端给出的拒绝原因和判定证据。" :ctx="ctx">
    ...
    <div class="sw-inspector-grid">
      <section class="sw-card sw-pad sw-stack">
        ...
        <div v-if="resource" class="sw-alert" data-testid="selected-access-resource"><b>{{ resource.label }}</b><small class="sw-code"> · {{ typeLabel(resource.type) }}</small></div>
      ...
      <section class="sw-card sw-pad sw-stack" aria-live="polite" data-testid="access-conclusion">
        ...
        <template v-else>
          <div class="sw-alert" :class="result.allowed === true && result.finalDecision === 'ALLOW' ? 'sw-alert--success' : 'sw-alert--warning'" data-testid="access-decision"><b>{{ result.allowed === true && result.finalDecision === 'ALLOW' ? '此对象访问通过' : '此对象访问未通过' }}</b><p>{{ result.message || '请核对下方角色与数据范围判定。' }}</p></div>

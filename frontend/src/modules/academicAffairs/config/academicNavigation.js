...
const PRIMARY = {
  ...
  'aa-terms': ['学期管理', '校历管理', '作息时间', '学院管理', '专业管理', '年级管理', '行政班管理', '教学班管理', '组织核对', '组织统计'],
  ...
  'aa-resources': ['教室资源', '实训室资源', '设备资源', '资源占用', '资源冲突', '资源维修', '资源统计'],
...
// 四级对象继续使用原三级入口的权限、页签和收藏身份，不新增菜单入口。
export function academicObjectParentPath(pathname, searchParams) {
  if ((pathname === '/admin/academic-affairs/textbooks' && searchParams?.get('tab') === 'distribution')
    || /^\/admin\/academic-affairs\/textbooks\/distributions\/(?:new|[1-9]\d*)\/?$/.test(pathname)) {
    return '/admin/academic-affairs/textbooks?tab=order'
  }
  return null
}
...
export function buildAcademicNavigation(sourceModules) {
  ...
  for (const source of sourceModules) source.children.forEach((leaf, index) => {
    ...
    target.children.push({ ...leaf, label, sourceKey: leaf.sourceKey || `${source.key}:${index}`, menuSection: source.label,
      legacyWorkspaceIds: [...new Set([...(leaf.legacyWorkspaceIds || []), leaf.sourceKey, leaf.path].filter(Boolean))],
  ...
  // Hidden aliases only restore through an owner with the exact same route and authorization.
  for (const group of result) for (const leaf of group.children) {
    const owner = leaf.hidden ? group.children.find(candidate => !candidate.hidden &&
      ['path', 'permissionKey', 'permissionAny', 'permissionAll', 'status', 'disabled']
        .every(field => JSON.stringify(candidate[field]) === JSON.stringify(leaf[field]))) : leaf
    if (!owner) throw new Error(`Academic alias has no canonical owner: ${leaf.sourceKey}`)
    leaf.canonicalLeafId = owner.leafId
    if (leaf !== owner) owner.legacyWorkspaceIds = [...new Set([
      ...owner.legacyWorkspaceIds, leaf.leafId, ...leaf.legacyWorkspaceIds
    ])]
  }
  return result
}

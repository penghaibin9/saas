  ...
  <ModulePageShell v-else class="classroom-workspace" :title="formVisible ? (editingId ? '编辑教室' : '新建教室') : '教室资源'" :role-name="ctx.currentRole?.roleName" :data-scope-name="ctx.dataScope?.scopeName">
    ...
    <template v-if="!formVisible">
      ...
      <section class="cr-panel" aria-label="教室列表">
        ...
        <template v-else-if="viewMode === 'grid'"><div class="cr-room-grid"><button v-for="row in rows" :key="row.classroomId" class="cr-room-card" @click="openDetail(row)"><div><OfficeBuilding /><StatusTag :type="statusType(row.status)" :label="row.statusLabel" dot /></div><strong>{{ row.roomCode }}</strong><span>{{ row.roomName }}</span><small>{{ row.buildingName }} · {{ row.floorNo ? `${row.floorNo} 层` : '楼层待核对' }} · {{ row.roomTypeLabel }}</small><footer><span><b>{{ row.capacity }}</b> 教学座位</span><span>{{ row.examSeats ?? row.capacity }} 考试座位</span></footer></button></div><div class="cr-grid-pager"><span>共 {{ pagination.total }} 间 · 第 {{ pagination.page }} 页</span><AppButton :disabled="pagination.page <= 1" @click="onPageChange(pagination.page - 1)">上一页</AppButton><AppButton :disabled="pagination.page * pagination.pageSize >= pagination.total" @click="onPageChange(pagination.page + 1)">下一页</AppButton></div></template>
        <DataTable v-else :columns="columns" :rows="rows" row-key="classroomId" :pagination="pagination" @page-change="onPageChange">
          ...
          <template #cell-capacity="{ row }"><strong>{{ row.capacity }}</strong><small class="cr-unit">座</small></template>
      ...
      <div class="cr-editor">
        <div class="cr-panel">
          ...
          <section class="cr-section"><div class="cr-section-head"><h2>教室信息</h2><small>同一楼栋内编号唯一</small></div><div class="cr-grid">
            ...
            <AppFormItem v-slot="{ id }" label="考试座位" hint="留空时考务沿用教学座位；按实际考试布置填写。"><AppNumberInput :id="id" v-model="form.examSeats" :min="0" :max="1000" :precision="0" :disabled="saving" /></AppFormItem><label class="cr-exclusive"><input v-model="form.isExclusive" type="checkbox" :disabled="saving">专用教室（自动排课跳过）</label>
        ...
        <aside class="cr-panel cr-preview" aria-label="教室信息预览"><small>教室信息预览</small><OfficeBuilding class="cr-preview-icon" aria-hidden="true" /><h2>{{ previewName }}</h2><p>{{ form.buildingName || '填写楼栋与编号后自动组合名称' }}</p><dl><div><dt>教室类型</dt><dd>{{ typeOptions.find(item => item.value === form.roomType)?.label }}</dd></div><div><dt>座位容量</dt><dd>{{ form.capacity ?? '待填写' }} 座</dd></div></dl><StatusTag :type="statusType(editingStatus || 'AVAILABLE')" :label="editingId ? statusOptions.find(item => item.value === editingStatus)?.label : '新建后默认可用'" /><p class="cr-preview-note">本页用于少量补充与例外维护；整栋初始化可使用批量建教室或台账导入。</p></aside>
      </div>
...
const EMPTY_FORM = () => ({ buildingId: null, floorNo: null, examSeats: null, isExclusive: false, expectedVersion: null, buildingCode: '', buildingName: '', roomCode: '', roomName: '', capacity: 0, roomType: 'LECTURE', campusCode: '', remark: '' })
...
export default {
  ...
  data() {
    return {
      ...
      columns: [{ key: 'room', title: '教室 / 编号' }, { key: 'building', title: '所在楼栋' }, { key: 'roomType', title: '类型' }, { key: 'capacity', title: '容量' }, { key: 'status', title: '状态' }, { key: 'actions', title: '操作' }],
  ...
  beforeRouteLeave() { return !this.$refs.batchWorkspace?.busy && !this.buildingSaving && !this.saving && (!(this.dirty || this.$refs.batchWorkspace?.dirty || this.buildingForm) || window.confirm('当前教室资料尚未保存，是否离开？')) },
  methods: {
    async loadBuildings(page = 1) {
      ...
      catch (e) { if (rev === this.buildingRevision) { this.buildings = []; this.selectedBuilding = null; this.buildingsError = e.message || '教学楼加载失败' } }
    ...
    async saveBuilding() {
      ...
      catch (e) { if (rev === this.contextRevision) this.buildingError = e.message || '教学楼未保存，请重试' }
    ...
    async closeBatch() { if (this.$refs.batchWorkspace?.busy || (this.$refs.batchWorkspace?.dirty && !window.confirm('清单修改尚未重新预检，是否返回？'))) return; const query = { ...this.$route.query }; delete query.batchNo; await this.$router.replace({ query }); this.batchMode = ''; this.loadBuildings(); this.load() },
    ...
    async updateQuery(page = 1) {
      ...
      const query = { ...this.$route.query }
      for (const key of ['keyword', 'roomType', 'status', 'buildingId', 'floorNo']) { if (this.filters[key]) query[key] = this.filters[key].trim(); else delete query[key] }
    ...
    async load() {
      ...
      try {
        ...
        if (res.code !== 0) throw new Error(res.message || '无法加载教室列表')
      ...
      } catch (error) { if (revision === this.loadRevision) this.error = errorMessage(error, '教室列表加载失败，请重试。') }
    ...
    openEdit(row) {
      ...
      this.editingId = row.classroomId; this.editingStatus = row.status
      this.form = Object.fromEntries(Object.keys(EMPTY_FORM()).map(key => [key, row[key] ?? EMPTY_FORM()[key]]))
      this.form.expectedVersion = row.version
    ...
    closeForm() { if (this.saving || (this.dirty && !window.confirm('当前教室资料尚未保存，是否放弃修改？'))) return; this.formVisible = false; this.form = EMPTY_FORM(); this.formError = ''; this.fieldErrors = {} },
    ...
    validateForm() {
      const errors = {}, ids = { buildingCode: 'cr-building-code', buildingName: 'cr-building-name', roomCode: 'cr-room-code', capacity: 'cr-capacity' }
    ...
    async submitForm(keepBuilding = false) {
      ...
      try {
        ...
        if (res.code !== 0) throw new Error(res.message || '保存失败，请重试。')
        this.receipt = { ...res.data, action: editingId ? '资料已更新' : '教室已创建' }
        if (keepBuilding && !editingId) {
          this.form = { ...EMPTY_FORM(), ...Object.fromEntries(['buildingCode', 'buildingName', 'roomType', 'capacity', 'campusCode'].map(key => [key, body[key]])) }
      ...
      } catch (error) { if (revision === this.contextRevision) this.formError = errorMessage(error, '保存失败，已保留填写内容，请核对后重试。') }
    ...
    async loadDetail() {
      ...
      try {
        ...
        if (res.code !== 0) throw new Error(res.message || '无法读取教室资料')
        this.detail = res.data
      } catch (error) { if (revision === this.detailRevision) this.detailError = errorMessage(error, '教室资料加载失败，请重试。') }
    ...
    async onConfirm() {
      ...
      try {
        ...
        await this.load()
        await this.loadBuildings(this.buildingPage)
      } catch (error) { if (revision === this.contextRevision) this.actionError = errorMessage(error, '操作失败，请重试。') }

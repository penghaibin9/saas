      ...
      <template v-else>
        ...
        <template v-else-if="tab === 'archive'">
          <AppSectionCard title="校历归档">
            <p class="mp-note">仅已发布（PUBLISHED / FROZEN）的校历可归档；归档后学期不再作为当前学期，历史数据只读可查。</p>
            <div v-if="selectedTerm" class="aa-status-row">
              <span>当前学期：{{ selectedTerm.yearCode }} 第 {{ selectedTerm.termNo }} 学期</span>
              <StatusTag :status="selectedTerm.status" />
            </div>
            <AppInlineAlert v-if="!canManageCalendar" type="info" description="仅教务处 / 学校管理员可执行归档，当前身份可查看但按钮已禁用。" />
            <AppInlineAlert v-if="selectedTerm && selectedTerm.status === 'DRAFT'" type="warning" description="校历尚未发布，请先到「校历发布」页发布后再归档。" />
            <AppInlineAlert v-if="selectedTerm && selectedTerm.status === 'ARCHIVED'" type="success" description="校历已归档。" />
            <AppButton variant="primary" :disabled="!canArchive" :loading="archiving" @click="doArchive">
              {{ archiving ? '归档中…' : '归档校历' }}
            </AppButton>
          </AppSectionCard>
        </template>
...
export default {
  ...
  computed: {
    ...
    canArchive() {
      return this.canManageCalendar && !!this.selectedTerm && ['PUBLISHED', 'FROZEN'].includes(this.selectedTerm.status)
    },
    filteredEvents() {
  ...
  methods: {
    ...
    async doArchive() {
      if (this.archiving || !this.canArchive) return
      this.archiving = true
      const res = await academicAffairsApi.archiveTerm(this.termId)

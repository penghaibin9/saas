<template>
  <ModulePageShell
    title="开课差异"
    subtitle="对照培养方案应开课程与本学期教学任务，先处理漏开、重复、多开和课程数据异常"
  ...
  >
    ...
    <div class="mp-stack">
      <AppInlineAlert
        type="info"
        title="本页不建立第二套教学计划"
        description="差异由当前有效培养方案、专业年级/班级绑定和本学期教学任务实时计算；修复动作回到培养方案或教学任务正式页面完成。"
      />
      ...
      <DataTable v-else :columns="columns" :rows="rows" row-key="key">
        ...
        <template #cell-actions="{ row }">
          <button v-if="row.programId" class="mp-link" @click="openProgram(row)">处理方案</button>
          <button v-if="row.taskIds?.length" class="mp-link" @click="$router.push('/admin/academic-affairs/teaching-tasks')">处理任务</button>
        </template>
      </DataTable>
    </div>
  </ModulePageShell>
...
const LABELS = {
  ...
  NO_TEACHER: '缺教师', CREDIT_MISMATCH: '学分不一致', COURSE_UNRESOLVED: '课程未解析',
  TERM_UNRESOLVED: '学期未解析', NO_CLASS: '未匹配班级'
}
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 'actions', title: '处理', width: '150px' }
      ]
    }
  },
  async created() {
  ...
  methods: {
    ...
    statusType(value) {
      ...
      if (['MISSING_TASK', 'DUPLICATE_TASK', 'COURSE_UNRESOLVED', 'TERM_UNRESOLVED', 'NO_CLASS'].includes(value)) return 'danger'
      return 'warning'
    },
    async loadTerms() {
...
@media (max-width: 960px) { .aa-summary-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
</style>

<template>
  ...
  >
    <template #actions>
      <AppButton variant="primary" :disabled="loading || !rows.length" @click="goDetail(rows[0])">打开学生档案</AppButton>
    </template>
    <div class="mp-stack">
      <section class="aa-roster-card">
        <header class="aa-roster-card__head">
          <div><h2>学生权威主档 · {{ pageTitle }}</h2><p>从正式名册进入档案、注册和异动</p></div>
          <span v-if="!loading && !error">共 {{ pagination.total }} 名学生</span>
        </header>
      ...
      <nav class="aa-roster-flow" aria-label="学籍关联办理">
        <span>关联办理 / 四级下钻</span>
        <button type="button" @click="$router.push('/admin/academic-affairs/registration')">注册批次</button>
        <button type="button" @click="$router.push('/admin/academic-affairs/roster/corrections')">学籍信息更正</button>
        <button type="button" @click="$router.push('/admin/academic-affairs/status-changes')">异动台账</button>
        <button type="button" @click="$router.push('/admin/academic-affairs/warnings')">学业预警</button>
      </nav>
...
export default {
  ...
  components: { ModulePageShell, DataTable, LoadingState, ErrorState, EmptyState, AppStatusTag, AdvancedFilter, AppButton },
...
.aa-roster-card { overflow: hidden; border: 1px solid var(--card-b, #dbe4ee); border-radius: 12px; background: var(--bg-card, #fff); }
.aa-roster-card__head { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 15px 17px; border-bottom: 1px solid var(--card-b, #dbe4ee); }
.aa-roster-card__head h2 { margin: 0; font-size: 16px; }
.aa-roster-card__head p, .aa-roster-card__head span { margin: 4px 0 0; color: var(--t3, #65778b); font-size: 12px; }
.aa-roster-card :deep(.advanced-filter) { margin: 12px 16px; }
.aa-roster-card :deep(.dt) { border: 0; border-radius: 0; box-shadow: none; }
.aa-roster-flow { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; padding: 12px 16px; border: 1px solid var(--card-b, #dbe4ee); border-radius: 10px; background: var(--bg-card, #fff); }
.aa-roster-flow span { margin-right: 4px; color: var(--t3, #65778b); font-size: 12px; }
.aa-roster-flow button { padding: 6px 10px; border: 1px solid var(--card-b, #dbe4ee); border-radius: 7px; background: #fff; color: var(--pri, #2f66bd); cursor: pointer; }
</style>

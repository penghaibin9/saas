    ...
    <template v-else-if="ov">
      <section class="pco__welcome" aria-labelledby="pco-focus-title">
        ...
        <section class="pco__priority" aria-labelledby="pco-priority-title">
          ...
          <ul v-if="priorityItems.length" class="pco__priority-list">
            <li v-for="item in priorityItems.slice(0, 4)" :key="item.key">
          ...
          <p v-if="priorityItems.length > 4" class="pco__queue-note">先展示前 4 项；完整学校事项与运行风险见下方。</p>
        </section>
      </section>
      ...
      <div class="pco__main-grid">
        <section class="pco__panel pco__schools" aria-labelledby="pco-schools-title">
          ...
          <EmptyState v-else-if="Array.isArray(ov.expiringTenants) && Array.isArray(ov.abnormalTenants)" text="暂无临期或异常学校" compact />
      ...
      <div class="pco__bottom-grid">
        <section class="pco__panel" aria-labelledby="pco-risks-title">
          ...
          <EmptyState v-else-if="!ov.operationalRisks.length" :text="qualityRows.length ? '部分来源未取得，暂不能判断全部运行风险' : '本次未发现运行风险'" compact />
...
.pco__welcome-main { padding: var(--space-6); background: linear-gradient(120deg, var(--pri-bg), var(--bg-card)); display: flex; flex-direction: column; align-items: flex-start; }
.pco__welcome-main h2 { margin: var(--space-4) 0 var(--space-2); font-size: var(--font-size-2xl); line-height: 1.55; letter-spacing: -.03em; text-wrap: balance; }
.pco__welcome-main > p { margin: 0; max-width: 36em; font-size: var(--font-size-sm); line-height: 1.8; color: var(--t2); }
.pco__welcome-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); margin: var(--space-5) 0; }
...
.pco__activity-lead { display: flex; flex-wrap: wrap; align-items: baseline; column-gap: var(--space-2); margin: var(--space-5) 0; }.pco__activity-lead strong { font-size: var(--font-size-3xl); font-variant-numeric: tabular-nums; letter-spacing: -.03em; }.pco__activity-lead span,.pco__activity-lead small { font-size: var(--font-size-xs); color: var(--t2); }.pco__activity-lead small { margin-left: auto; }

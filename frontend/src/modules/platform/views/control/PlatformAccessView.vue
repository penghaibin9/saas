<template>
  <ModulePageShell title="平台访问治理" subtitle="职责分离、二次认证临时提升、真实工单受控协助与访问复核" role-name="平台负责人 / 安全审计" data-scope-name="平台控制面">
    <div class="access-page">
      <section class="hero">
        <div>
          <h3>平台人员不再共用超级管理员</h3>
          <p>职责、临时提升、受控协助和访问复核统一由后端授权中心控制；受控协助只能读取会话批准的学校上下文与审计摘要。</p>
        </div>
      ...
      <section class="panel table-panel">
        ...
        <p class="hint">平台主管职责是固定受控模板；本工作区不允许任意新建职责或直接勾选平台权限。</p>
        <table>
          <thead><tr><th>职责</th><th>业务能力</th><th>平台权限</th><th>生产菜单</th><th>高危保证</th></tr></thead>
          <tbody>
            <tr v-for="item in dutyMatrix" :key="item.dutyCode"><td><strong>{{ dutyLabel(item.dutyCode) }}</strong></td><td>{{ capabilityLabels(item.capabilities) }}</td><td class="matrix-list">{{ (item.permissionCodes || []).length ? `${item.permissionCodes.length} 项平台权限` : '无直接权限投影' }}</td><td class="matrix-list">{{ (item.menuPreview || []).map((menu) => menu.label).join(' / ') || '无生产菜单' }}</td><td>{{ (item.highRiskActions || []).length ? `${item.highRiskActions.length} 项需近期登录复核；严重操作需二次认证` : '无高危动作' }}</td></tr>
          </tbody>
        </table>
      </section>
      ...
      <section class="forms">
        <form class="panel" @submit.prevent="saveAssignment">
          <h3>分配平台职责</h3>
          <label>平台用户编号<input v-model.trim="assignmentForm.userId" required /></label>
          <label>职责
          ...
          <p class="hint">请求编号在失败重试时保持不变；成功后才生成新编号。</p>
        ...
        <form class="panel" @submit.prevent="createElevation">
          <h3>临时权限提升</h3>
          <label>平台用户编号<input v-model.trim="elevationForm.userId" required /></label>
          <fieldset class="choice-field">
            <legend>具体能力</legend>
            <label v-for="item in capabilityOptions" :key="item.value" class="choice-option">
              <input v-model="elevationForm.capabilities" type="checkbox" :value="item.value" />{{ item.label }}
            </label>
          </fieldset>
          ...
          <p class="hint">创建临时提升时，生产环境会强制执行近期登录复核和二次认证。</p>
        ...
        <form class="panel" @submit.prevent="createSupportSession">
          <h3>受控学校协助</h3>
          <label>学校租户编号<input v-model.trim="supportForm.tenantId" required inputmode="numeric" /></label>
          <label>支持工单编号<input v-model.trim="supportForm.ticketId" required inputmode="numeric" /></label>
          <fieldset class="choice-field">
            <legend>批准范围</legend>
            <label v-for="item in scopeOptions" :key="item.value" class="choice-option">
              <input v-model="supportForm.scopes" type="checkbox" :value="item.value" />{{ item.label }}
            </label>
          </fieldset>
          <p class="hint">可批准范围包括学校概况、学校审计、身份元数据、文件元数据和敏感身份读取；默认只申请本工作区所需的最小权限，读取审计还需二次认证。</p>
        ...
        <form class="panel" @submit.prevent="createReview">
          ...
          <p class="hint">创建时冻结当前有效职责、临时提升与受控协助快照；关闭时必须逐项决策。</p>
      ...
      <section v-if="actionTarget" class="panel action-panel">
        <div>
          <h3>{{ actionTitle }}</h3>
          <p class="hint">目标 {{ actionTarget.id }} · 版本 {{ actionTarget.version }}</p>
        </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in assignments" :key="item.id">
              <td>{{ item.userId }}</td><td>{{ dutyLabel(item.dutyCode) }}</td><td>{{ platformStatusLabel(item.status || 'ACTIVE') }}</td><td>{{ item.expiresAt || '长期' }}</td><td>{{ item.version }}</td>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in elevations" :key="`e-${item.id}`">
              <td>临时提升</td><td>{{ item.userId }}</td><td>{{ capabilityLabels(item.capabilities) }}</td><td>{{ item.reason }}</td><td>{{ item.expiresAt }}</td><td>{{ platformStatusLabel(item.status) }}</td>
              <td><AppButton v-if="isActive(item)" @click="beginAction('elevation', item)">撤销</AppButton></td>
            </tr>
            <tr v-for="item in sessions" :key="`s-${item.id}`">
              <td>受控协助</td><td>{{ item.operatorUserId }}</td><td>{{ item.tenantId }} · {{ scopeLabels(item.scopes) }}</td><td>工单 #{{ item.ticketId }}</td><td>{{ item.expiresAt }}</td><td>{{ platformStatusLabel(isActive(item) ? item.status : 'EXPIRED') }}</td>
              <td class="access-ops">
      ...
      <section v-if="supportWorkspace.session" class="panel support-workspace">
        <div class="review-head">
          <div>
            <h3>受控协助工作区 · 工单 #{{ supportWorkspace.session.ticketId }}</h3>
        ...
        <template v-else-if="supportWorkspace.context">
          ...
          <div v-if="supportHasScope('tenant.audit.read')" class="support-audit">
            <div class="support-audit__head">
              <div><h3>学校审计摘要</h3><p class="hint">属于更敏感的支持数据，必须使用短时二次认证凭证；凭证只保存在本页内存并按服务端有效期主动清除。</p></div>
              <div v-if="!supportWorkspace.mfaToken" class="support-mfa">
                <input v-model.trim="supportWorkspace.mfaCode" inputmode="numeric" maxlength="6" placeholder="6 位动态码" />
                <AppButton variant="primary" :loading="saving === 'support-mfa'" @click="stepUpAndLoadAudit">二次认证并读取</AppButton>
              </div>
            ...
            <table v-if="supportWorkspace.mfaToken">
              ...
              <tbody>
                <tr v-for="row in supportWorkspace.audits" :key="row.auditId || row.id">
                  <td>{{ row.occurredAt || row.time || '—' }}</td><td>{{ auditActionLabel(row.action) }}</td><td>{{ row.resource || row.target || '—' }}</td><td>{{ row.operatorName || row.who || '—' }}</td><td>{{ platformStatusLabel(row.result) }}</td>
                </tr>
                <tr v-if="!supportWorkspace.audits.length"><td colspan="5">当前没有审计记录</td></tr>
              </tbody>
            </table>
          </div>
          <p v-else class="hint">本次会话未批准学校审计读取权限，因此不展示学校审计摘要。</p>
        </template>
      </section>
      ...
      <section class="panel table-panel">
        ...
        <div v-if="selectedReview" class="review-editor">
          <div class="review-head">
            <div><strong>{{ selectedReview.name || selectedReview.id }}</strong><p class="hint">每个冻结项必须明确保留或撤销；缺项或多项后端都会拒绝关闭。</p></div>
          ...
          <div v-for="item in selectedReview.items || []" :key="item.itemKey" class="review-item">
            <span>{{ reviewItemLabel(item) }}</span>
            <select v-model="reviewDecisions[item.itemKey]"><option value="KEEP">保留</option><option value="REVOKE">撤销</option></select>
          </div>
...
const CAPABILITY_LABELS = Object.freeze({
  'commercial.manage': '商务管理', 'delivery.manage': '交付管理',
  'customerSuccess.manage': '客户成功管理', 'operations.manage': '运行管理',
  'incident.manage': '事件管理', 'security.audit': '安全审计', 'support.manage': '客户支持管理'
})
const SCOPE_LABELS = Object.freeze({
  'tenant.context.read': '学校概况读取', 'tenant.audit.read': '学校审计读取',
  'identity.metadata.read': '身份元数据读取', 'file.metadata.read': '文件元数据读取',
  'sensitive.identity.read': '敏感身份读取'
})
...
export default {
  ...
  computed: {
    capabilityOptions() { return Object.entries(CAPABILITY_LABELS).map(([value, label]) => ({ value, label })) },
    scopeOptions() { return Object.entries(SCOPE_LABELS).map(([value, label]) => ({ value, label })) },
  ...
  methods: {
    platformStatusLabel,
    dutyLabel(value) {
      return this.dutyOptions.find((item) => item.value === value)?.label || '其他平台职责'
    },
    capabilityLabels(values) {
      return (values || []).map((value) => CAPABILITY_LABELS[value] || '其他受控能力').join(' / ') || '无'
    },
    scopeLabels(values) {
      return (values || []).map((value) => SCOPE_LABELS[value] || '其他批准范围').join(' / ') || '无'
    },
    ...
    reviewItemLabel(item) {
      ...
      if (key.includes('assignment')) return '平台职责分配'
      if (key.includes('elevation')) return '临时权限提升'
      if (key.includes('support')) return '受控协助会话'
      return '受控访问项目'
    },
    isActive(item) {
    ...
    async createElevation() {
      if (!this.elevationForm.capabilities.length) return toast.error('请至少选择一项具体能力')
      this.saving = 'elevation'; const res = await platformPamApi.createElevation({ ...this.elevationForm, capabilities: [...this.elevationForm.capabilities] }); this.saving = ''
    ...
    async createSupportSession() {
      ...
      if (!Number.isInteger(tenantId) || tenantId <= 0 || !Number.isInteger(ticketId) || ticketId <= 0) return toast.error('请输入有效租户编号与支持工单编号')
      if (!this.supportForm.scopes.length) return toast.error('请至少选择一项批准范围')
      this.saving = 'support'; const res = await platformPamApi.createSupportSession({ ...this.supportForm, tenantId, ticketId, scopes: [...this.supportForm.scopes] }); this.saving = ''
    ...
    async stepUpAndLoadAudit() {
      if (!/^\d{6}$/.test(this.supportWorkspace.mfaCode)) return toast.error('请输入 6 位动态码')
      ...
      try {
        ...
        if (!this.supportWorkspace.mfaToken) return toast.error('二次认证未返回访问凭证')
    ...
    async loadSupportAudit() {
      ...
      if (this.supportWorkspace.mfaExpiresAt && this.supportWorkspace.mfaExpiresAt <= Date.now()) {
        this.clearSupportMfaGrant()
        return toast.error('二次认证临时授权已过期，请重新验证')
      }
    ...
    async closeReview() {
      ...
      if (decisions.some((item) => !['KEEP', 'REVOKE'].includes(item.decision))) return toast.error('每个复核项都必须明确保留或撤销')
...
.choice-field { display: flex; flex-wrap: wrap; gap: 8px 12px; margin: 0; padding: 10px; border: 1px solid var(--card-b,#e5e6eb); border-radius: 8px; }
.choice-field legend { padding: 0 4px; color: var(--text-secondary,#646a73); font-size: 13px; }
.choice-option { display: flex; grid-template-columns: none; align-items: center; gap: 5px; color: var(--text-primary,#1f2329); }
.choice-option input { width: 16px; height: 16px; margin: 0; padding: 0; }

<template>
  <section class="finance-workspace" aria-label="商业财务工作区">
    <header class="finance-heading">
      <div><span class="eyebrow">FINANCE CONTROL</span><h3>应收、退款与发票</h3><p>应收直接读取原订单；退款与发票只记录人工办理事实。系统不自动退款、不自动开票，也不自动停用学校已购模块。</p></div>
      <div class="heading-actions"><span class="badge">M8 · 人工凭据闭环</span><button :disabled="loading||!tenantId" @click="reloadCurrent">刷新财务事实</button></div>
    </header>

    <div v-if="!tenantId" class="empty">先在上方销售工作区选择学校，再办理该校财务事项。</div>
    <template v-else>
      <div class="tabs" role="tablist" aria-label="财务办理步骤"><button v-for="item in tabs" :key="item.key" role="tab" :aria-selected="tab===item.key" @click="selectTab(item.key)">{{ item.label }}</button></div>
      <p v-if="error" class="error" role="alert">{{ error }}</p><p v-if="notice" class="success" role="status">{{ notice }}</p>
      <div v-if="pendingCommand" class="pending" role="status"><strong>上次{{ pendingCommand.kind==='refund'?'退款':'发票' }}申请结果仍需核对</strong><p>原请求与幂等键已保留。请重试原请求，不要重新填写第二份申请。</p><button class="primary" :disabled="sending||!canManage" @click="retryPending">{{ sending?'正在核对原请求…':'重试原请求' }}</button></div>

      <section v-show="tab==='overview'" class="tab-panel">
        <div class="section-head"><div><h4>订单实时资金投影</h4><p>没有第二张“应收余额表”；金额始终从 PlatformOrder 及退款/发票控制事实派生。币种只认订单冻结分项，历史缺失不会自动猜成 CNY。</p></div></div>
        <div v-if="loading" class="empty">正在读取真实订单资金事实…</div>
        <div v-else class="table-scroll"><table><thead><tr><th>订单</th><th>成交 / 已收</th><th>未收应收</th><th>退款</th><th>发票</th><th>可办理余额</th><th>操作</th></tr></thead><tbody>
          <tr v-for="row in orders" :key="row.orderId" :class="{conflict:row.currencyConflict}"><td><strong>{{ row.orderNo }}</strong><small>{{ orderStatus(row.orderStatus) }} · {{ row.orderType }}</small><small>{{ currencyStateLabel(row) }}</small></td><td>{{ amountText(row,row.amount) }}<small>已收 {{ amountText(row,row.paidAmount) }}</small></td><td><strong :class="Number(row.outstandingAmount)>0?'warn-text':'ok-text'">{{ amountText(row,row.outstandingAmount) }}</strong></td><td>已结算 {{ amountText(row,row.refundSettledAmount) }}<small>已批准 {{ amountText(row,row.refundApprovedAmount) }}</small></td><td>已开 {{ amountText(row,row.invoiceIssuedAmount) }}<small>申请中 {{ amountText(row,row.invoiceRequestedAmount) }}</small></td><td><strong>{{ amountText(row,row.availableFinanceCapacity) }}</strong><small>退款/发票共享额度</small></td><td class="row-actions"><button :disabled="!canManage||locked||!row.financeActionAllowed" @click="startRefund(row)">申请退款</button><button :disabled="!canManage||locked||!row.financeActionAllowed" @click="startInvoice(row)">申请发票</button></td></tr>
      ...
      <section v-show="tab==='refunds'" class="tab-panel">
        <div class="split">
          <article class="form-card"><h4>发起退款申请</h4><p>这里只申请退款额度；批准后仍需在外部资金渠道完成退款，再回来登记结算凭据。</p><fieldset :disabled="!canManage||sending||locked||!!pendingCommand"><label>订单 ID<input v-model.trim="refundForm.orderId" inputmode="numeric" placeholder="从资金总览点“申请退款”自动带入"></label><div class="two-col"><label>退款金额<input v-model.trim="refundForm.amount" inputmode="decimal" placeholder="0.00"></label><label>币种<input v-model.trim="refundForm.currency" maxlength="3" placeholder="如 CNY；历史订单需人工核对"></label></div><label>退款原因<textarea v-model.trim="refundForm.reason" minlength="10" maxlength="500" placeholder="至少10个字符，写清合同、未使用服务或其他依据"></textarea></label><button class="primary" :disabled="!refundReady" @click="submitRefund">{{ sending?'正在登记…':'登记退款申请（不执行退款）' }}</button></fieldset></article>
          <article class="form-card"><h4>退款办理</h4><div v-if="!selectedRefund" class="empty">从下方退款台账选择一条记录。</div><template v-else>
            <div class="case-summary"><strong>{{ selectedRefund.caseNo }}</strong><span>{{ refundStatus(selectedRefund.status) }} · {{ selectedRefund.currency }} {{ selectedRefund.amount }}</span></div>
            <template v-if="selectedRefund.status==='REQUESTED'"><label>审批说明<input v-model.trim="refundAction.note" maxlength="500" placeholder="批准时写合同/财务复核结论"></label><label>驳回原因<input v-model.trim="refundAction.rejectReason" maxlength="500" placeholder="驳回时至少5个字符"></label><div class="button-row"><button class="primary" :disabled="sending||refundAction.note.length<5" @click="approveSelectedRefund">批准额度</button><button :disabled="sending||refundAction.rejectReason.length<5" @click="rejectSelectedRefund">驳回申请</button></div></template>
            <template v-else-if="selectedRefund.status==='APPROVED'"><label>外部退款凭据<input v-model.trim="refundAction.settlementRef" maxlength="160" placeholder="银行/微信/支付宝退款流水号"></label><button class="primary" :disabled="sending||refundAction.settlementRef.length<6" @click="settleSelectedRefund">登记外部退款已完成</button><p class="notice">登记结算不会自动修改订单支付状态，也不会自动停止模块授权。</p></template>
            <template v-else-if="selectedRefund.status==='SETTLED'"><div class="after-sales-box"><strong>下一步：核对退款后的模块授权</strong><p>只有仍存在有效模块来源时，后端才会创建现有客户成功工单；没有有效来源不会乱造工单，也不会自行停用模块授权。</p><label>工单优先级<select v-model="afterSalesForm.severity"><option value="">请明确选择</option><option value="P0">P0 最高</option><option value="P1">P1 高</option><option value="P2">P2 中</option><option value="P3">P3 低</option></select></label><label>授权复核说明<input v-model.trim="afterSalesForm.reason" minlength="5" maxlength="500" placeholder="说明合同、退款范围与授权处理重点"></label><button class="primary" :disabled="sending||!afterSalesForm.severity||afterSalesForm.reason.length<5" @click="createRefundAfterSales">生成 / 找回售后授权复核工单</button></div></template>
            <div v-else class="empty">此退款申请已驳回，无需继续办理。</div>
          </template></article>
        </div>
      ...
      <section v-show="tab==='invoices'" class="tab-panel">
        <div class="split"><article class="form-card"><h4>发起发票申请</h4><p>申请只占用可办理金额；系统不会调用税控或第三方开票服务。</p><fieldset :disabled="!canManage||sending||locked||!!pendingCommand"><label>订单 ID<input v-model.trim="invoiceForm.orderId" inputmode="numeric" placeholder="从资金总览点“申请发票”自动带入"></label><div class="two-col"><label>开票金额<input v-model.trim="invoiceForm.amount" inputmode="decimal" placeholder="0.00"></label><label>币种<input v-model.trim="invoiceForm.currency" maxlength="3" placeholder="如 CNY；历史订单需人工核对"></label></div><label>发票抬头<input v-model.trim="invoiceForm.invoiceTitle" minlength="2" maxlength="200" placeholder="学校或合同约定开票主体"></label><button class="primary" :disabled="!invoiceReady" @click="submitInvoice">{{ sending?'正在登记…':'登记发票申请（不自动开票）' }}</button></fieldset></article>
          <article class="form-card"><h4>发票办理</h4><div v-if="!selectedInvoice" class="empty">从下方发票台账选择一条记录。</div><template v-else><div class="case-summary"><strong>{{ selectedInvoice.requestNo }}</strong><span>{{ invoiceStatus(selectedInvoice.status) }} · {{ selectedInvoice.currency }} {{ selectedInvoice.amount }}</span></div><template v-if="selectedInvoice.status==='REQUESTED'"><label>外部发票凭据<input v-model.trim="invoiceAction.externalRef" maxlength="160" placeholder="税控/第三方平台返回的发票号或业务凭据"></label><button class="primary" :disabled="sending||invoiceAction.externalRef.length<6" @click="issueSelectedInvoice">登记外部发票已开具</button><p class="notice">不要求手填数据库文件 ID；发票文件后续应通过文件中心正式关联。</p></template><template v-else-if="selectedInvoice.status==='ISSUED'"><label>外部作废依据<input v-model.trim="invoiceAction.voidReason" maxlength="500" placeholder="至少5个字符，确认外部发票已经作废后再登记"></label><button :disabled="sending||invoiceAction.voidReason.length<5" @click="voidSelectedInvoice">登记外部发票已作废</button></template><div v-else class="empty">此发票记录已处于终态，无需重复办理。</div></template></article></div>
  ...
  <ModuleOperationsWorkspace :key="`${tenantId}:${opsEpoch}`" :tenant-id="tenantId" :locked="locked||sending" />
</template>
...
const tab=ref('overview'),access=ref(null),error=ref(''),notice=ref(''),sending=ref(false),pendingCommand=ref(null),opsEpoch=ref(0)

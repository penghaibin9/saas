    ...
    <template v-else>
      ...
      <section v-show="tab==='overview'" class="tab-panel">
        ...
        <div v-else class="table-scroll"><table><thead><tr><th>订单</th><th>成交 / 已收</th><th>未收应收</th><th>退款</th><th>发票</th><th>可办理余额</th><th>操作</th></tr></thead><tbody>
          <tr v-for="row in orders" :key="row.orderId" :class="{conflict:row.currencyConflict}">
            <td><strong>{{ row.orderNo }}</strong><small>{{ orderStatus(row.orderStatus) }} · {{ row.orderType }}</small><small>{{ currencyStateLabel(row) }}</small></td>
            <td>{{ amountText(row,row.amount) }}<small>已收 {{ amountText(row,row.paidAmount) }}</small></td>
            <td><strong :class="Number(row.outstandingAmount)>0?'warn-text':'ok-text'">{{ amountText(row,row.outstandingAmount) }}</strong></td>
            <td>已结算 {{ amountText(row,row.refundSettledAmount) }}<small>已批准 {{ amountText(row,row.refundApprovedAmount) }}</small></td>
            <td>已开 {{ amountText(row,row.invoiceIssuedAmount) }}<small>申请中 {{ amountText(row,row.invoiceRequestedAmount) }}</small></td>
            <td><strong>{{ amountText(row,row.availableFinanceCapacity) }}</strong><small>退款/发票共享额度</small></td>
            <td class="row-actions"><button :disabled="!canManage||locked||!row.financeActionAllowed" @click="startRefund(row)">申请退款</button><button :disabled="!canManage||locked||!row.financeActionAllowed" @click="startInvoice(row)">申请发票</button></td>
          </tr>
      ...
      <section v-show="tab==='refunds'" class="tab-panel">
        <div class="split">
          <article class="form-card"><h4>发起退款申请</h4><p>这里只申请退款额度；批准后仍需在外部资金渠道完成退款，再回来登记结算凭据。</p>
            <fieldset :disabled="!canManage||sending||locked||!!pendingCommand">
              ...
              <div class="two-col"><label>退款金额<input v-model.trim="refundForm.amount" inputmode="decimal" placeholder="0.00"></label><label>币种<input v-model.trim="refundForm.currency" maxlength="3" placeholder="如 CNY；历史订单需人工核对"></label></div>
...
const refundAction=ref({note:'',rejectReason:'',settlementRef:''}),invoiceAction=ref({externalRef:'',voidReason:''})
let epoch=0
...
watch(()=>props.tenantId,()=>{epoch++;orders.value=[];refunds.value=[];invoices.value=[];selectedRefund.value=null;selectedInvoice.value=null;refundForm.value={orderId:'',amount:'',currency:'',reason:''};invoiceForm.value={orderId:'',amount:'',currency:'',invoiceTitle:''};clearMessage();loadPending();if(props.tenantId)loadOrders(1)})

<template>
  <ModulePageShell title="服务目录、依赖与租户影响地图" subtitle="一级核心服务 · 降级 · 无负责人 · 单点依赖 · 服务目标风险"
    ...
    <template v-else>
      <div class="psc__grid">
        ...
        <AppCard class="psc__stat"><div class="psc__stat-num">{{ overview.p0Count }}</div><div class="psc__stat-label">一级核心服务</div></AppCard>
        ...
        <AppCard class="psc__stat" :class="{ 'psc__stat--warn': overview.noOwnerCount }"><div class="psc__stat-num">{{ overview.noOwnerCount }}</div><div class="psc__stat-label">无负责人</div></AppCard>
      ...
      <AppCard class="psc__panel">
        ...
        <DataTable :columns="serviceColumns" :rows="services" row-key="serviceCode">
          <template #cell-scope="{ row }">
            <div class="psc__cell-main">{{ serviceLabel(row.serviceCode, row.serviceName) }}</div>
             <div class="psc__cell-sub">{{ serviceTierLabel(row.tier) }}</div>
          </template>
          <template #cell-status="{ row }">
            <StatusTag :type="statusTone(row.status)" :label="platformStatusLabel(row.status)" dot />
            <StatusTag v-if="row.releaseBlocked" type="danger" label="发布阻断：缺少负责人或运维手册" />
          </template>
      ...
      <AppCard class="psc__panel">
        ...
        <div class="psc__form">
          <input v-model.trim="form.serviceCode" class="psc__input" placeholder="服务编码" :disabled="!!form._editing" />
          <input v-model.trim="form.serviceName" class="psc__input" placeholder="服务名称" />
          <select v-model="form.tier" class="psc__input">
            <option value="P0">一级（核心）</option><option value="P1">二级（重要）</option>
            <option value="P2">三级（一般）</option><option value="P3">四级（辅助）</option>
          </select>
          ...
          <input v-model.trim="form.ownerName" class="psc__input" placeholder="负责人姓名" />
          <input v-model.trim="form.runbookUrl" class="psc__input" placeholder="运维手册地址" />
      ...
      <AppCard class="psc__panel">
        ...
        <div class="psc__form">
          <input v-model.trim="depForm.serviceCode" class="psc__input" placeholder="依赖方服务编码" />
          <input v-model.trim="depForm.dependsOnServiceCode" class="psc__input" placeholder="被依赖方服务编码" />
          <button class="mp-link" @click="addDependency">新增依赖</button>
        </div>
        <DataTable :columns="depColumns" :rows="dependencies" row-key="id">
          <template #cell-scope="{ row }">{{ serviceLabel(row.serviceCode) }} → {{ serviceLabel(row.dependsOnServiceCode) }}（{{ dependencyTypeLabel(row.dependencyType) }}）</template>
      ...
      <AppCard v-if="impact" class="psc__panel">
        <AppSectionHeader :title="`故障影响面：${serviceLabel(impact.serviceCode)}`" />
        <ul class="psc__kv">
          <li><span>直接受影响租户</span><b>{{ impact.directTenants.join('、') || '无' }}</b></li>
          <li><span>间接受影响租户</span><b>{{ impact.indirectTenants.join('、') || '无' }}</b></li>
          <li><span>受影响服务</span><b>{{ serviceLabels(impact.affectedServices) }}</b></li>
        </ul>
      </AppCard>
    </template>
...
export default {
  ...
  methods: {
    platformStatusLabel,
    serviceLabel(value, fallback) { return platformServiceLabel(value, fallback || '其他服务') },
    serviceLabels(values) { return (values || []).map((value) => this.serviceLabel(value)).join('、') || '无' },
    serviceTierLabel(value) {
      return ({ P0: '一级核心服务', P1: '二级重要服务', P2: '三级一般服务', P3: '四级辅助服务' })[value] || '服务等级待确认'
    },
    dependencyTypeLabel(value) {
      return ({ HARD: '强依赖', SOFT: '弱依赖', RUNTIME: '运行依赖', DATA: '数据依赖' })[value] || '普通依赖'
    },
    statusTone(s) {
    ...
    editService(row) {
      this.form = {
        serviceCode: row.serviceCode, serviceName: this.serviceLabel(row.serviceCode, row.serviceName), tier: row.tier,
        status: row.status, ownerName: row.ownerName || '', runbookUrl: row.runbookUrl || '',
        _editing: true, expectedVersion: row.version
      }
    },
    async saveService() {
      if (!this.form.serviceCode || !this.form.serviceName) return toast.error('服务编码与服务名称必填')

...
export const PLATFORM_MANAGEMENT_ROADMAP_CATALOG = [
  {
    ...
    items: [
      ...
      { key: 'plt-customer-success', label: '客户健康、工单、培训与续费', path: '/admin/platform/customer-success', permissionKey: 'platform.customerSuccess.manage', view: 'customer-success', actions: [action('ticket:manage', '处理工单'), action('training:manage', '登记培训'), action('renewal:manage', '跟进续费')] },
      ...
      { key: 'plt-disaster-recovery', label: '备份恢复验证与灾备', path: '/admin/platform/disaster-recovery', permissionKey: 'platform.control.view', view: 'disaster-recovery', actions: [action('backup:evidence:record', '登记备份/演练证据', 'HIGH'), action('schema:check:run', '运行结构自检')] }
    ]
  },
  {

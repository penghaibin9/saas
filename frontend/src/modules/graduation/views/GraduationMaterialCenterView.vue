<template>
  <section class="mc-page">
      ...
      <section class="mc-panel">
        ...
        <div v-else ref="tableWrap" class="mc-table-wrap">
          <table v-if="tab !== 'students'">
            <thead><tr><th>学生</th><th>材料</th><th>当前文件</th><th>提交信息</th><th>文件安全</th><th>审核结论</th><th>归档结论</th><th>操作</th></tr></thead>
            <tbody><tr v-for="row in rows" :key="row.materialId">
              <td><strong>{{ row.studentName }}</strong><small>{{ row.studentNo }} · {{ row.className || row.classId || '班级待补' }}</small><small>指导教师：{{ row.advisorName || '待分配' }}</small></td>
              <td><span>{{ stageLabel(row.stage) }}</span><strong>{{ row.materialName }}</strong><details><summary>技术标识</summary><small>{{ row.materialCode || '—' }}</small></details></td>
              <td :title="row.fileName"><strong>{{ row.fileName || '尚未提交' }}</strong><small>当前第 {{ row.currentVersion || 0 }} 版 · 历史 {{ row.historyVersionCount || 0 }} 版</small></td>
              <td><span>{{ row.uploader || '提交人待同步' }}</span><small>{{ row.uploadedAt || '尚未提交' }} · {{ sizeText(row.sizeBytes) }}</small></td>
              <td><b :class="tone(row.scanStatus)">{{ scanLabel(row.scanStatus) }}</b></td>
              <td>{{ reviewLabel(row.reviewStatus) }}</td><td>{{ archiveLabel(row.archiveStatus) }}</td>
    ...
    <aside v-if="actionReceipt" class="mc-receipt" role="status">
      ...
      <button type="button" @click="actionReceipt = null">知道了</button>
    </aside>
  </section>
</template>
...
function stageLabel(value) { return stages.find(item => item.value === value)?.label || '其他材料' }
function scanLabel(value) { return ({ CLEAN: '安全，可用于业务', PASSED: '安全，可用于业务', PENDING: '安全检查中', ERROR: '安全检查失败', INFECTED: '发现风险文件', NOT_REQUIRED: '无需检查' })[String(value || '').toUpperCase()] || '安全状态待确认' }
function reviewLabel(value) { return ({ PENDING: '等待审核', RETURNED: '已退回，等待补交', APPROVED: '审核通过', NOT_REQUIRED: '无需审核' })[String(value || '').toUpperCase()] || '审核结论待确认' }
function archiveLabel(value) { return ({ READY: '可归档', ARCHIVED: '已归档', MISSING: '缺少材料', BLOCKED: '暂不可归档', PENDING: '等待归档' })[String(value || '').toUpperCase()] || '归档结论待确认' }
...
async function confirmReview(action, payload) { if (!reviewRow.value) return; const target = { ...reviewRow.value }; reviewing.value = true; try { await api.reviewMaterial(target.materialId, { fileVersionId: target.currentVersionId, expectedVersion: target.version, action, comment: payload?.reason || '' }); reviewVisible.value = false; rejectVisible.value = false; await load(); const latest = rows.value.find(row => String(row.materialId) === String(target.materialId)); actionReceipt.value = { title: `${target.studentName} · ${target.materialName} 已处理`, result: `服务器最新结论：${reviewLabel(latest?.reviewStatus || (action === 'APPROVE' ? 'APPROVED' : 'RETURNED'))}`, next: action === 'APPROVE' ? '该版本已进入后续完整性与归档校验。' : '下一步由学生按退回原因补交新版本。' } } catch (e) { error.value = e?.message || '审核失败' } finally { reviewing.value = false } }
...
<style scoped>
.mc-page{display:grid;gap:16px;min-width:0}.mc-hero,.mc-panel,.mc-filters{background:#fff;border:1px solid #dfe7f3;border-radius:14px}.mc-hero{display:flex;justify-content:space-between;align-items:center;padding:20px 24px;background:linear-gradient(135deg,#f7fbff,#eef5ff)}.mc-hero span{color:#1769e0;font-size:12px;font-weight:700;letter-spacing:.1em}.mc-hero h2{margin:5px 0}.mc-hero p{margin:0;color:#69768b}.mc-page button{border:1px solid #ccd8e8;border-radius:8px;background:#fff;color:#29415f;padding:7px 11px;cursor:pointer}.mc-page button:disabled{opacity:.45}.mc-page .primary{background:#1769e0;border-color:#1769e0;color:#fff}.mc-error,.mc-empty{padding:24px;text-align:center;color:#69768b}.mc-error{display:flex;gap:12px;align-items:center;text-align:left;border-radius:12px;background:#fff1f1;color:#a61b1b}.mc-error button{margin-left:auto}.mc-summary{display:grid;grid-template-columns:repeat(6,minmax(110px,1fr));gap:10px}.mc-summary article{display:grid;gap:4px;padding:14px;border:1px solid #e2e8f2;border-radius:12px;background:#fff}.mc-summary strong{font-size:24px}.mc-summary small,td small{display:block;color:#7b8798}.mc-tabs{display:flex;gap:4px;border-bottom:1px solid #dfe7f3}.mc-tabs button{border:0;border-radius:8px 8px 0 0;padding:10px 18px}.mc-tabs button.active{background:#1769e0;color:#fff}.mc-filters{display:grid;grid-template-columns:1.6fr repeat(3,1fr) auto;gap:12px;align-items:end;padding:14px}.mc-filters label{display:grid;gap:5px;color:#526176;font-size:13px}.mc-filters input,.mc-filters select{min-width:0;border:1px solid #ccd8e8;border-radius:8px;padding:8px;background:#fff}.mc-filters>div{display:flex;gap:7px}.mc-panel{overflow:hidden}.mc-table-wrap{overflow:auto;max-height:66vh}table{width:100%;min-width:1080px;border-collapse:collapse}th,td{padding:10px 12px;border-bottom:1px solid #edf1f7;text-align:left;vertical-align:top;font-size:13px}th{position:sticky;top:0;z-index:1;background:#f7f9fc;color:#526176;white-space:nowrap}td strong,td span{display:block}td details{margin-top:4px;color:#7b8798;font-size:11px}td summary{cursor:pointer}.actions{white-space:nowrap}.actions button{border:0;padding:4px 6px;color:#1769e0}.ok,.warn,.danger{display:inline-block!important;padding:2px 7px;border-radius:999px}.ok{color:#137a43;background:#eafaf1}.warn{color:#9a5b00;background:#fff7df}.danger{color:#b42318;background:#fff0f0}.mc-pagebar{display:flex;justify-content:flex-end;align-items:center;gap:10px;padding:12px 16px}.mc-pagebar span{margin-right:auto;color:#69768b}.mc-modal-mask{position:fixed;inset:0;z-index:1000;display:flex;align-items:center;justify-content:center;padding:24px;background:rgba(15,23,42,.42)}.mc-modal{width:min(680px,100%);max-height:80vh;overflow:auto;padding:20px;border-radius:14px;background:#fff;box-shadow:0 18px 60px rgba(15,23,42,.2)}.mc-modal>header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:18px}.mc-modal>header div{display:grid;gap:4px}.mc-modal>header span{color:#69768b;font-size:13px}.mc-reader{position:fixed;inset:0;z-index:1200;display:grid;grid-template-rows:auto auto minmax(0,1fr);background:#f4f7fb}.mc-reader__head{display:flex;align-items:center;gap:14px;padding:10px 14px;border-bottom:1px solid #dfe7f3;background:#fff;box-shadow:0 2px 10px rgba(15,23,42,.06)}.mc-reader__back{flex:none}.mc-reader__identity{min-width:0;display:flex;align-items:center;gap:8px;flex:1}.mc-reader__identity strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.mc-reader__identity span,.mc-reader__identity b{flex:none;padding:3px 7px;border-radius:999px;background:#eef4ff;color:#315986;font-size:12px}.mc-reader__identity b{background:#fff7df;color:#8a5b00}.mc-reader__error{padding:7px 14px;background:#fff1f1;color:#a61b1b;font-size:13px}.mc-reader__body{min-height:0;padding:12px;overflow:hidden}.mc-reader__body :deep(.document-viewer){height:100%}.mc-reader__body :deep(.document-viewer__body){min-height:0}.mc-reader__empty{height:100%;display:grid;place-items:center;border:1px dashed #ccd8e8;border-radius:12px;background:#fff;color:#69768b}.mc-receipt{position:fixed;right:24px;bottom:24px;z-index:1250;display:flex;align-items:center;gap:18px;width:min(440px,calc(100vw - 32px));padding:14px 16px;border:1px solid #b7ebc6;border-radius:12px;background:#f0fff4;box-shadow:0 12px 36px rgba(15,23,42,.16)}.mc-receipt div{display:grid;gap:3px;flex:1}.mc-receipt strong{color:#137a43}.mc-receipt span{color:#29415f;font-size:13px}.mc-receipt small{color:#69768b}@media(max-width:1100px){.mc-summary{grid-template-columns:repeat(3,1fr)}.mc-filters{grid-template-columns:repeat(2,1fr)}.mc-reader__identity{flex-wrap:wrap}.mc-reader__identity strong{width:100%}}@media(max-width:700px){.mc-hero{display:grid;gap:12px}.mc-summary{grid-template-columns:repeat(2,1fr)}.mc-filters{grid-template-columns:1fr}.mc-tabs{overflow:auto}.mc-reader__head{align-items:flex-start;flex-wrap:wrap}.mc-reader__identity{order:3;width:100%}.mc-receipt{right:16px;bottom:16px}}
</style>

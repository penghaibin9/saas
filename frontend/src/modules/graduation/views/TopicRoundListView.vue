    ...
    <div v-if="activePanel !== 'conflicts'" class="mp-stack">
      ...
      >
        ...
        <template #cell-actions="{ row }">
          ...
          <button v-if="canTopicMatch && (row.status === 'DRAFT' || row.status === 'CLOSED')" class="mp-link" style="margin-left: var(--space-2)" @click="askOpen(row)">开启</button>
          <button v-if="canTopicMatch && row.status === 'OPEN'" class="mp-link" style="margin-left: var(--space-2)" @click="askClose(row)">关闭</button>
          <button v-if="canTopicMatch && (row.status === 'OPEN' || row.status === 'CLOSED')" class="mp-link" style="margin-left: var(--space-2)" @click="askMatch(row)">匹配</button>
          <button v-if="canTopicMatch && (row.status === 'CLOSED' || row.status === 'MATCHED')" class="mp-link" style="margin-left: var(--space-2)" @click="askArchive(row)">归档</button>
        </template>
      </DataTable>
      ...
      >
        ...
        <template #cell-actions="{ row }">
          <template v-if="row.status === 'PENDING'">
            <button v-if="canTopicReview" class="mp-link" @click="askConfirmChoice(row)">确认</button>
            <button v-if="canTopicReview" class="mp-link mp-link--danger" style="margin-left: var(--space-2)" @click="askRejectChoice(row)">驳回</button>
            <button v-if="canTopicAssign" class="mp-link" style="margin-left: var(--space-2)" @click="askWithdraw(row)">退选</button>
            <span v-if="!canTopicReview && !canTopicAssign" class="mp-cell-sub">—</span>
          </template>
...
export default {
  ...
  computed: {
    ...
    canTopicMatch() { return matchPermission(this.permissionPatterns, 'graduationDesign.topic.match') },
    ...
    filterFields() {
      return [{ key: 'status', label: '状态', type: 'select', options: GD_ROUND_STATUS }]
    },
    toolbarActions() {
      if (this.activePanel === 'rounds') {
        return this.canTopicMatch
          ? [{ key: 'create', label: '＋ 新建轮次', variant: 'primary', disabled: !this.hasBatch || this.ctx.writeEnabled === false }]
          : []
      }
      const actions = [{ key: 'back', label: '返回轮次列表' }]
      if (this.activePanel === 'conflicts') return actions
      if (this.selectedRoundId) {
        actions.push({ key: 'conflicts', label: '容量冲突复核' })
        if (this.canTopicCreate) actions.push({ key: 'import', label: '导入 Excel', disabled: this.ctx.writeEnabled === false })
      }
      return actions
    },
    exportVisible() {
      return this.canTopicExport && this.hasBatch && (this.activePanel === 'rounds' || !!this.selectedRoundId)
    },
    pageSubtitle() {
    ...
    emptyDesc() {
      ...
      return this.activePanel === 'rounds'
        ? (this.canTopicMatch ? '点「新建轮次」创建一轮选题互选' : '当前角色仅可查看选题轮次')
        : '学生提交志愿后将在此展示'
    }
  },
  watch: {
  ...
  methods: {
    ...
    async load() {
      if (!this.canTopicView) {
        this.loading = false; this.error = ''; this.rows = []; this.total = 0; this.conflicts = []; this.stats = null; return
      }
    ...
    onToolbar(key) {
      if (key === 'create') {
        if (!this.canTopicMatch) return
        this.$router.push('/admin/graduation/topic-rounds/create')
      }
      if (key === 'import') {
    ...
    async onConfirm({ reason } = {}) {
      ...
      const allowed = ['open', 'close', 'match', 'archive'].includes(action) ? this.canTopicMatch
        : ['confirmChoice', 'rejectChoice'].includes(action) ? this.canTopicReview
          : action === 'withdraw' ? this.canTopicAssign : false
      if (!allowed) { this.confirm.visible = false; return }
      this.submitting = true
      let r = { code: 1 }
      if (action === 'open') r = await gdTopicRoundApi.openRound(row.id)
      if (action === 'close') r = await gdTopicRoundApi.closeRound(row.id)
      if (action === 'match') r = await gdTopicRoundApi.matchRound(row.id)
      if (action === 'confirmChoice') r = await gdTopicRoundApi.confirmChoice(row.id)
      if (action === 'rejectChoice') r = await gdTopicRoundApi.rejectChoice(row.id, reason || '')
      if (action === 'archive') r = await gdTopicRoundApi.archiveRound(row.id)
      if (action === 'withdraw') r = await gdTopicRoundApi.withdrawChoices(this.selectedRoundId, row.gdStudentId)
      this.submitting = false
      if (r.code !== 0) { toast.error(r.message); return }
      toast.success(action === 'match' ? `已匹配 ${r.data?.matched ?? 0} 人` : '操作成功')
    ...
    exportRoundsFn() {
      if (!this.canTopicExport) return Promise.resolve({ code: 403001, message: '当前角色无选题导出权限' })
      if (this.activePanel === 'rounds') {

  ...
  <section class="file-evidence-panel">
    ...
    <details v-if="canonical" class="file-evidence-panel__history">
      ...
      <div class="file-evidence-panel__technical">
        <code>文件版本编号 {{ canonical.fileVersionId ?? canonical.versionId ?? '—' }}</code>
      ...
      <div v-for="item in versions" :key="String(item.fileVersionId ?? item.versionId ?? item.id)" class="file-evidence-panel__row">
        <span>第 {{ item.versionNo ?? '—' }} 版</span><span>{{ versionStatusLabel(item.versionStatus || item.status) }}</span><code>{{ shortHash(item.sourceSha256 || item.sha256) }}</code>
      </div>
    </details>
  </section>
</template>
...
function versionStatusLabel(value) { return ({ UPLOADED: '已上传', SCANNING: '安全检查中', READY: '已就绪', SUBMITTED: '已提交', APPROVED: '已通过', REJECTED: '已退回', SUPERSEDED: '已被新版本替代', INVALIDATED: '已失效', ARCHIVED: '已归档' })[value] || (value ? `状态待确认（${value}）` : '—') }
</script>
<style scoped>

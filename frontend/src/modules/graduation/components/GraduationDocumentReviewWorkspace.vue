  ...
  <div class="gd-review-workspace" :class="{ 'is-narrow': narrow }">
    <aside class="gd-review-workspace__queue">
      ...
      <button
        ...
        :class="{ 'is-active': index === currentIndex }" @click="$emit('select', item)"
      >
    ...
    <main class="gd-review-workspace__document">
      <div class="gd-review-workspace__business-bar">
        ...
        <div class="gd-review-workspace__nav">
          <button type="button" :disabled="currentIndex <= 0" @click="$emit('previous')">← 上一条</button>
          <button type="button" :disabled="currentIndex >= queue.length - 1" @click="$emit('next')">下一条 →</button>
        </div>
      </div>

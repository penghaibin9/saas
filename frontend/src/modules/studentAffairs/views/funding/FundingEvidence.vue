<template>
  <section class="fund-evidence" aria-label="申请佐证材料">
    <header><strong>申请佐证材料</strong><button type="button" :disabled="loading" @click="refresh">刷新</button></header>
    <p v-if="loading">正在读取申请材料…</p>
    <p v-else-if="error" class="fund-evidence__error" role="alert">{{ error }}</p>
    <p v-else-if="!items.length">该申请未附佐证材料。</p>
    ...
    <section class="fund-evidence__requirements" aria-label="补交与验收">
      <header><strong>补交与验收</strong><button type="button" @click="openMaterials()">管理补交与验收</button></header>
...
import { affairsOperationsApi } from '../../api/operations.api'
...
let seq = 0
function showError(e) { error.value = e.message || '材料加载失败，请重试' }
...
function openMaterials(id) {
  router.push({ path: '/admin/student-affairs/material-operations', query: { bizType: 'FUNDING', bizId: props.applicationId, ...(id ? { materialRequirementId: String(id) } : {}) } })
}
async function loadRequirements(page = 1) {
  ...
  try {
    const data = await (affairsOperationsApi.listRequirements({ ...context, page, pageSize: 10 }))
...
<style scoped>
.fund-evidence { display: grid; gap: 8px; margin: 16px 0; padding-top: 14px; border-top: 1px solid var(--border-color, #e2e8f0); }
.fund-evidence header { display: flex; align-items: center; justify-content: space-between; font-size: 14px; }
.fund-evidence button { border: 0; background: transparent; color: var(--color-primary, #2765bd); cursor: pointer; min-height: 32px; }
.fund-evidence p { margin: 0; font-size: 13px; color: var(--text-secondary, #64748b); }.fund-evidence__error { color: var(--color-danger, #b45309) !important; }

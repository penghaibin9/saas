...
export function isPlatformPrincipal() {
  const user = identity()
  return !!user.userId && (user.userType.startsWith('PLATFORM_') || user.roleCode.startsWith('PLATFORM_'))
}
...
export function platformDutyPatterns(duties) {
  const normalized = new Set((Array.isArray(duties) ? duties : []).map((item) => String(item || '').trim()).filter(Boolean))
  if (normalized.has('*')) return ['platform.*']

  const patterns = new Set(Array.from(normalized, (duty) => `platform.${duty}`))
  // Historical route keys are UI aliases only. They mirror backend authority;
  // aliases never grant a capability that is absent from /platform/context.
  if (normalized.has('commercial.view')) {
    patterns.add('platform.package.view')
    patterns.add('platform.order.view')
  }
  if (normalized.has('provisioning.manage')) {
    patterns.add('platform.provision.run.view')
    patterns.add('platform.onboarding.view')
  }
  if (normalized.has('incident.manage')) {
    patterns.add('platform.incident.view')
  }
...
export function resolvePlatformHome(context = cachedContext) {
  ...
  if (duties.has('provisioning.manage')) return '/admin/platform/provisioning'
  if (duties.has('customerSuccess.manage')) return '/admin/platform/customer-success'
  if (duties.has('incident.manage')) return '/admin/platform/incidents'
...
export function toPlatformUiContext(context = cachedContext) {
  ...
  return {
    tenantBrandConfig: {
      tenantId: '0', operatorName, schoolName: '', platformDisplayName: '高校学生全生命周期管理平台',
      schoolLogo: '', schoolBadge: '', brandColor: '#2563eb', watermarkText: '平台运营数据 · 严禁外传'
    },
    currentRole: { userId: String(context.subjectId || user.userId), userName: operatorName, roleCode, roleName: roleCode },
    dataScope: {
...
export async function ensurePlatformAccessContext({ force = false } = {}) {
  if (!isPlatformPrincipal()) {
    cachedSubjectId = ''; cachedAt = 0; cachedContext = null
    return null
  }
  const subjectId = identity().userId
  const patterns = getPermissionPatterns()
  if (!force && cachedContext && cachedSubjectId === subjectId && Date.now() - cachedAt < CACHE_MS && Array.isArray(patterns) && patterns.some((item) => String(item).startsWith('platform.'))) {
    return cachedContext
  }
  try {
    const context = await request('/platform/context', { forceProbe: true })
    if (String(context?.principalPlane || '').toUpperCase() !== 'PLATFORM') throw new Error('平台上下文 principalPlane 非 PLATFORM，拒绝进入控制面')
    if (String(context?.subjectId || '') !== subjectId) throw new Error('平台上下文主体与当前 access token 不一致')
    ...
    cachedSubjectId = subjectId; cachedAt = Date.now(); cachedContext = context
    return context
  } catch (error) {
    cachedSubjectId = ''; cachedAt = 0; cachedContext = null

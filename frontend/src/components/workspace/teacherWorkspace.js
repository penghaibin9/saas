...
export function workspacePages(modules) {
  ...
  return items.map(item => {
    if (item.moduleKey.startsWith('aa-') && item.leafId) {
      if (!item.path || counts.get(item.path) === 1) return { ...item, id: item.leafId }
      const url = new URL(item.path, 'http://workspace.local')
      url.searchParams.set('_workspace', item.leafId)
      return { ...item, id: item.leafId, destination: url.pathname + url.search }
    }
...
export function resolveWorkspacePageId(id, pages) {
  ...
  const matches = pages.filter(page => page.moduleKey?.startsWith('aa-') && page.leafId && page.legacyWorkspaceIds?.includes(id))
...
export function restoreWorkspace(value, pages) {
  ...
  const usable = pages.filter(item => item.path && !item.disabled)
  const allowed = new Set(usable.map(item => item.id))
  const safeIds = (ids, max) => Array.isArray(ids) ? [...new Set(ids.map(id => resolveWorkspacePageId(id, usable)).filter(id => allowed.has(id)))].slice(0, max) : []
  const appearance = {}
  const migratedAppearance = {}
  for (const [oldId, entry] of Object.entries(saved.appearance || {})) {
    const id = resolveWorkspacePageId(oldId, usable)
    if (id && entry && typeof entry === 'object' && !Object.hasOwn(migratedAppearance, id)) migratedAppearance[id] = entry
  }
  for (const id of allowed) {
    const entry = saved.appearance?.[id] || migratedAppearance[id]
    if (!entry || typeof entry !== 'object') continue
    appearance[id] = shortcutAppearance(pages.find(page => page.id === id), { [id]: entry })
  }
  return {
    appearance,

...
class DocxArchive {
  ...
  parseDirectory() {
    ...
    for (let index = 0; index < count; index += 1) {
      if (offset + 46 > this.bytes.byteLength || u32(this.view, offset) !== 0x02014b50) {
        fail('PREVIEW_DOCX_MALFORMED', 'DOCX 中央目录损坏')
      }
  ...
  async read(name) {
    ...
    if (offset + 30 > this.bytes.byteLength || u32(this.view, offset) !== 0x04034b50) {
      fail('PREVIEW_DOCX_MALFORMED', 'DOCX 本地文件头损坏')
    }
    ...
    if (typeof DecompressionStream !== 'function') {
      fail('PREVIEW_DOCX_DECOMPRESSION_UNSUPPORTED', '当前浏览器不支持安全 DOCX 解压，请升级 Chrome/Edge 后重试')
    }
    let output
    try {
    ...
    if (output.byteLength > MAX_ENTRY_BYTES || output.byteLength > Math.max(entry.uncompressedSize * 2 + 4096, entry.uncompressedSize + 4096)) {
      fail('PREVIEW_DOCX_TOO_COMPLEX', 'DOCX 解压内容异常，已停止渲染')
    }
    return output
  }

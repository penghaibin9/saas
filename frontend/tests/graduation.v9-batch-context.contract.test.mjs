...
const domainBatchApis = [
  'frontend/src/modules/graduation/api/graduation.api.js',
  'frontend/src/modules/graduation/api/graduation-more.api.js',
  'frontend/src/modules/graduation/api/graduation-student.api.js',
  'frontend/src/modules/graduation/api/graduation-risk-archive.api.js',
]
...
test('graduation domain has one batch resolver implementation', () => {
  domainBatchApis.forEach((path) => {
    const source = read(path)
    assert.match(source, /graduation-batch-context/, `${path} must import the shared batch helper`)
    assert.doesNotMatch(source, /function\s+withBatch\s*\(/, `${path} must not define a local withBatch`)
    assert.doesNotMatch(source, /useGraduationBatchStore/, `${path} must not resolve batch from the store directly`)
  })
})

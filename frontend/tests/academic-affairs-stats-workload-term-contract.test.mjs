...
test('workload detail keeps the selected term filter', () => {
  const handlerStart = source.indexOf('async viewWorkloadDetail(row)')
  ...
  assert.match(handler, /this\.workloadTeacherKey = row\.teacherKey/)
  assert.match(handler, /await this\.loadDetail\(\)/)
  assert.match(loader, /getStatsWorkloadDetail\(\{[^}]*teacherKey: this\.workloadTeacherKey/)
  assert.match(loader, /\.\.\.this\.baseParams\(\)/)
  assert.match(params, /termId: this\.filters\.termId \|\| undefined/)
  assert.match(params, /collegeId: this\.filters\.collegeId \|\| undefined/)
})

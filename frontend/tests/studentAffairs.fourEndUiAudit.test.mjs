...
test('leave UIs reject reversed dates and non-extension dates', () => {
  ...
  assert.match(portal, /newEndTime > leaveDate\(item\.endTime\)/)
...
test('credit claims are required, bounded, decimal-safe and server paginated', () => {
  const portal = read('student-portal/src/views/affairs/AffairsFourEndView.vue')
  const portalApi = read('student-portal/src/services/affairsFourEndApi.js')
  ...
  for (const source of [portal, portalApi, mini, miniApi, pc]) {
...
test('teacher high-risk actions require evidence and confirmation', () => {
  ...
  assert.doesNotMatch(mentalFollowPc, /ESCALATED:\s*\['CLOSE'\]/)
  assert.match(mentalFollowPc, /allowed\(row\)\.includes\('CLOSE'\)/)
  assert.doesNotMatch(mentalFollowPc, /pageSize: 100/)
  assert.match(talk, /followContent\.trim\(\)\.length < 5/)
  assert.match(talk, /确认办结谈话/)
  assert.match(leave, /picker mode="date"/)
  assert.match(leave, /picker mode="time"/)
  assert.match(leave, /不能晚于当前时间/)
  assert.match(review, /allowedActions/)
  assert.match(review, /调整通过/)
  assert.match(review, /this\.chooseAdjustLevel\(x\)/)

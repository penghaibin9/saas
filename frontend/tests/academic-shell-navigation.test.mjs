...
test('details and filtered query pages retain their actual module and selected leaf', () => {
  ...
  assert.equal(workspaceRouteOwner(route.path, route.fullPath).modKey, 'aa-terms')
...
test('business object and return parameters preserve the academic query entry owner', () => {
  for (const page of pages.filter(page => page.path?.includes('?') && !page.disabled)) {
    const route = routeFor(page)
    route.fullPath += '&batchId=990201&returnToken=opaque-token&page=3'
    const owner = workspaceRouteOwner(route.path, route.fullPath)
    assert.equal(activeWorkspacePage(pages, route, owner.modKey)?.id, page.id, page.path)
  }
})
test('textbook distribution deep links retain the order workspace and cannot inherit a stale overview tab', () => {
  const order = pages.find(page => page.path === '/admin/academic-affairs/textbooks?tab=order')
  const overview = pages.find(page => page.path === '/admin/academic-affairs')
  const queue = { path: '/admin/academic-affairs/textbooks', fullPath: '/admin/academic-affairs/textbooks?tab=distribution&page=3', query: { tab: 'distribution', page: '3' } }
  assert.equal(workspaceRouteOwner(queue.path, queue.fullPath).modKey, 'aa-textbooks')
  assert.equal(activeWorkspacePage(pages, queue, 'aa-textbooks')?.id, order.id)
  for (const suffix of ['new?orderBatchId=50', '6', '6/?returnToken=queue-page-3']) {
    const url = new URL(`/admin/academic-affairs/textbooks/distributions/${suffix}`, 'http://workspace.local')
    url.searchParams.set('_workspace', overview.id)
    const route = { path: url.pathname, fullPath: url.pathname + url.search, query: Object.fromEntries(url.searchParams) }
    assert.equal(workspaceRouteOwner(route.path, route.fullPath).modKey, 'aa-textbooks')
    assert.equal(activeWorkspacePage(pages, route, 'aa-textbooks')?.id, order.id)
    const restricted = workspacePages(getVisibleNavPlan({ permissionPatterns: ['academicAffairs.dashboard.view'] })
      .find(group => group.key === 'academic-affairs')?.children || [])
    assert.equal(activeWorkspacePage(restricted, route, 'aa-textbooks'), undefined)
  }
})

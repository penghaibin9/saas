...
const source = parse(fs.readFileSync(new URL('../src/modules/internship/views/ProcessReportDetailView.vue', import.meta.url), 'utf8'))
...
test('process report rejects duplicate and conflicted submissions', async () => {
  const vm = view({ reviewProcessReport: () => assert.fail('must not submit') })
...
test('late process report approval cannot create receipt or advance a different report', async () => {
  let finish
  const vm = view({ reviewProcessReport: () => new Promise(resolve => { finish = resolve }) })

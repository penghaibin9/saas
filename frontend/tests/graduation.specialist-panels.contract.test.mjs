...
test('route meta wins over a stale or forged panel query and API loading fails closed', () => {
  assert.match(view, /const requested = this\.\$route\.meta\.defaultPanel \|\| this\.\$route\.query\.panel/)
  assert.match(view, /this\.tab = this\.canPanel\(requested\) \? requested : \(this\.firstAllowedPanel\(\) \|\| 'plagiarism'\)/)
  assert.match(view, /if \(!this\.current \|\| !this\.canPanel\(this\.tab\)\) return/)
  assert.match(view, /switchTab\(t\) \{ if \(!this\.canPanel\(t\)\) return;/)
})
...
test('grade batch mode is invisible and non-callable without grade view permission', () => {
  ...
  assert.match(view, /if \(!this\.canUseGradeBatch \|\| !this\.batchStore\.selectedBatchId\)/)
})

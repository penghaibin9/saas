...
test('colorful shell overrides are page-scoped and preserve the approved geometry', () => {
  ...
  assert.match(css, /--hc-topbar-text:\s*var\(--text-primary\)/)
  assert.match(css, /--hc-topbar-muted:\s*var\(--text-tertiary\)/)
  assert.match(css, /--hc-topbar-control-border:\s*var\(--border-base\)/)
  assert.match(css, /--hc-topbar-control-text:\s*var\(--text-secondary\)/)

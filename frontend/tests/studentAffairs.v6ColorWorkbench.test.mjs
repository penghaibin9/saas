...
test('A1 uses a light conclusion surface, compact flow locator and semantic work rows', () => {
  ...
  assert.match(css, /grid-template-columns:\s*40px\s+minmax\(0,\s*1fr\)\s+90px\s+92px/)
  assert.match(css, /@media \(max-width: 1450px\)[\s\S]*?grid-template-columns:\s*38px\s+minmax\(0,\s*1fr\)\s+88px\s+86px/)

...
test('foundation keeps grade and organization anchors visible in the compact tertiary menu', () => {
  const modules = getVisibleNavPlan({ permissionPatterns: ['*'] }).find(group => group.key === 'academic-affairs').children
  const pages = workspacePages(modules)
  const visibleLabels = workspaceMenuItems(pages, 'aa-terms', '').map(item => item.label)
  for (const label of ['年级管理', '组织核对', '组织统计']) assert.ok(visibleLabels.includes(label), label)
})

...
test('M5 UI submits server references instead of arbitrary hashes',()=>{assert.match(api,/module-offboarding\/\$\{jobId\}\/export/);assert.match(api,/export-acceptance/);assert.match(view,/ExportJob ID/);assert.match(view,/Manifest ID/);assert.match(view,/scopeHash/);assert.doesNotMatch(view,/v-model[^>]*(sha|hash)/i)})

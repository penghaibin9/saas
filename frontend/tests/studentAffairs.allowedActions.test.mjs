...
test('student clients use the server allowedActions contract without runtime adapters', () => {
  ...
  assert.match(leave, /allows\('SUBMIT_EXTENSION'\)/)
...
test('admin student-affairs actions fail closed when allowedActions is absent', () => {
  ...
  assert.match(workStudy, /Array\.isArray\(row\?\.allowedActions\) && row\.allowedActions\.includes\(action\)/)
  assert.match(loan, /Array\.isArray\(row\?\.allowedActions\) && row\.allowedActions\.includes\(action\)/)

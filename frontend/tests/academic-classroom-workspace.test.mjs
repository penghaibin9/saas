...
import { isDeniedResult, isConflictResult } from '../src/modules/academicAffairs/components/parallel-a/resultState.js'
...
function page(api = {}, catalog = {}) {
  ...
  const sandbox = { dependencies: { matchPermission, isDeniedResult, isConflictResult, academicAffairsApi: api, classroomCatalogApi: catalog }, window: { confirm: () => true } }
...
test('individual edit retains input and expected version after conflict', async () => {
  let sent
  const formal = { classroomId: '9007199254740999', buildingCode: 'A', buildingName: '教学楼', roomCode: '0101', capacity: 60, version: 7, status: 'AVAILABLE', allowSchedule: true, allowExam: true, allowBorrow: false }
  const { state } = page({
    getClassroom: async id => ({ code: 0, data: { ...formal, classroomId: id } }),
    updateClassroom: async (id, body) => { sent = { id, body }; return { code: 409, message: '资料已变化' } },
  })
  await state.openEdit(formal)
  ...
  assert.match(state.formError, /资料已变化/)
  assert.match(state.formError, /保留输入/)
})

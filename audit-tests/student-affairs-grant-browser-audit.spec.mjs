...
test.describe.serial('Student Affairs strict browser audit · SA-005 grant', () => {
  test.describe.configure({ retries: 0 })

  test('approved difficult-library fact -> grant apply -> return/resubmit -> review -> publicity -> appeal -> time gate', async ({ page }) => {
    test.setTimeout(300_000)

    const prefix = `E2E-SA005-${Date.now()}-${process.pid}`
    let projectName = ''
    let schoolYear = ''
    const initialStatement = `${prefix}-APPLY 已完成本学年困难认定，现按真实困难资格申请助学金。`
    const revisedStatement = `${prefix}-RESUBMIT 已补充困难情况与助学申请用途说明，请重新审核。`
    const returnReason = `${prefix}-RETURN 请补充困难情况及助学金使用说明后重新提交。`
    const appealReason = `${prefix}-APPEAL 对助学金公示信息有疑问，申请复核困难资格与本次申请。`
    const appealOpinion = `${prefix}-APPEAL-REVIEW 已核对困难认定结果与助学资格快照，申诉不成立，维持公示资格。`
    ...
    if (!browser) throw new Error('SA-005 journey requires a real browser for the persistent student session')
    const studentContext = await browser.newContext()
    const studentPage = await studentContext.newPage()
    observePage(studentPage)

    await test.step('reuse GRANT project and one-day batch created by preceding real Staff PC configuration gate', async () => {
      const evidence = JSON.parse(await fs.readFile(path.resolve('student-affairs-grant-config-v3-evidence.json'), 'utf8'))
    ...
    await test.step('student submits GRANT in real PC portal; current SA-002 approved fact must allow the write', async () => {
      ...
      await studentPage.getByLabel('类型').selectOption('GRANT')
      ...
      expect(submitted.ok(), `grant apply HTTP ${submitted.status()}`).toBeTruthy()
      ...
      await expect(studentPage.locator('article.record').filter({ hasText: '辅导员初审' }).first()).toBeVisible({ timeout: 15_000 })
    })

    await test.step('same-tenant wrong counselor cannot read or review the grant application', async () => {
    ...
    await test.step('assigned counselor sees actual grant statement and returns it through real workbench', async () => {
      await openFundingWorkbench(page, staff.counselorA)
      await chooseFundingItem(page)
      const detail = page.locator('.fd-detail')
      await expect(detail.getByText(initialStatement, { exact: true })).toBeVisible({ timeout: 15_000 })
      ...
      expect(returned.ok(), `grant return HTTP ${returned.status()}`).toBeTruthy()
      expect((await jsonBody(returned)).code).toBe(0)
    })

    await test.step('student sees exact return reason, edits and resubmits through browser', async () => {
      await studentPage.goto(`${config.studentBaseUrl}/campus-service?tab=funding`)
      ...
      await modal.locator('textarea').first().fill(revisedStatement)
      ...
      expect(resubmitted.ok(), `grant resubmit HTTP ${resubmitted.status()}`).toBeTruthy()
      expect((await jsonBody(resubmitted)).code).toBe(0)
    })

    await test.step('assigned counselor sees revised statement and approves', async () => {
      await openFundingWorkbench(page, staff.counselorA)
      await chooseFundingItem(page)
      const detail = page.locator('.fd-detail')
    ...
    await test.step('college admin approves through real workbench', async () => {
    ...
    await test.step('SA admin approves school review and grant enters publicity', async () => {
    ...
    await test.step('tenant B cannot read the grant application', async () => {
    ...
    expect(api500, 'no unhandled API 5xx during SA-005 journey').toEqual([])
    ...
    expect(unexpectedConsoleErrors, 'no unexpected browser console errors during SA-005 journey').toEqual([])

    await fs.writeFile(path.resolve('student-affairs-grant-audit-evidence.json'), JSON.stringify({
      ...
      qualificationPrecondition: 'SA002_APPROVED_DIFFICULT_LIBRARY',

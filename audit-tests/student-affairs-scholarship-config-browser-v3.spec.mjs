...
test.describe.serial('Student Affairs V3 Browser First · SA-004 scholarship configuration', () => {
  test.describe.configure({ retries: 0 })

  test('staff creates scholarship project and one-day batch through real UI, then reads server truth', async ({ page }) => {
    test.setTimeout(180_000)

    const prefix = `E2E-SA004-CONFIG-${Date.now()}-${process.pid}`
    ...
    let projectId = ''
    await test.step('create scholarship project from Staff PC button and modal', async () => {
      await page.getByRole('button', { name: '建项目', exact: true }).click()
      const dialog = page.getByRole('dialog', { name: '新建资助项目' })
      await expect(dialog).toBeVisible()

      await field(dialog, '项目类型').locator('select').selectOption('SCHOLARSHIP')
      await field(dialog, '项目名称').locator('input').fill(projectName)
      await field(dialog, '金额（元）').locator('input').fill('3000')
    ...
    let batchId = ''
    await test.step('create published one-day batch from Staff PC button and modal', async () => {
    ...
    await test.step('read project and batch server truth after UI writes', async () => {
      ...
      expect(project, 'created project must be readable from server truth').toBeTruthy()
      expect(project.projectType).toBe('SCHOLARSHIP')
      expect(project.projectName).toBe(projectName)
      expect(String(project.amount)).toBe('3000.00')
      ...
      expect(batch, 'created batch must be readable from server truth').toBeTruthy()
      expect(batch.status).toBe('OPEN')
      expect(Number(batch.publicityDays)).toBe(1)
      expect(batch.schoolYear).toBe(schoolYear)
    })

    expect(api500, 'no API 5xx during Staff PC project/batch creation').toEqual([])
    expect(consoleErrors, 'no unexpected browser console errors during Staff PC project/batch creation').toEqual([])
    ...
    await fs.writeFile(
      path.resolve('student-affairs-scholarship-config-v3-evidence.json'),
